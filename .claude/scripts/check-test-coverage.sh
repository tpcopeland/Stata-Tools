...
#
# Exit codes:
#   0 - Report generated (coverage may be incomplete)
#
...
for pkg in $PACKAGES; do
    ...
    if [ -f "$TEST_FILE" ]; then
        ...
        # Also check for comprehensive test files
...
echo "---------------------"
TOTAL_TEST_FILES=$(find "$REPO_ROOT/_testing" -name "test_*.do" 2>/dev/null | wc -l)
TOTAL_VAL_FILES=$(find "$REPO_ROOT/_validation" -name "validation_*.do" 2>/dev/null | wc -l)
...
if [ -d "$REPO_ROOT/tvtools" ]; then
    ...
    echo "----------------"
    TVTOOLS_CMDS=$(grep -l "^program define tv" "$REPO_ROOT/tvtools/"*.ado 2>/dev/null | wc -l || echo "0")
    echo "tvtools commands: $TVTOOLS_CMDS"

    # Check for tvtools-specific tests
    TV_TESTS=$(find "$REPO_ROOT/_testing" -name "test_tv*.do" 2>/dev/null | wc -l)
    TV_VALS=$(find "$REPO_ROOT/_validation" -name "validation_tv*.do" 2>/dev/null | wc -l)
