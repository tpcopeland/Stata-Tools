...
{synopt:{opt sch:eme(schemename)}}graph scheme for Love plot{p_end}
{synopt:{opt graphopt:ions(twoway_options)}}additional options passed to Love plot{p_end}
...
Weights must be non-negative. May not be combined with {opt matched}.
...
assessed on the matched sample. May not be combined with {opt wvar()}.
...
Vertical lines mark the threshold bounds. If {opt wvar()} was specified,
...
{phang}
{opt scheme(schemename)} specifies the graph scheme for the Love plot.
For example, {cmd:scheme(plotplainblind)} provides a clean,
colorblind-friendly appearance.

{phang}
{opt graphoptions(twoway_options)} specifies additional options passed directly
to the {helpb twoway} command that draws the Love plot. Any valid twoway option
may be specified, including {opt xsize()}, {opt ysize()},
{opt note()}, and {opt plotregion()} among others. For example,
{cmd:graphoptions(note("Source: RCT data"))}.
...
{phang2}{stata "balancetab index_age female education diabetes hypertension, treatment(treated)":. balancetab index_age female education diabetes hypertension, treatment(treated)}{p_end}
...
{phang2}{stata "balancetab index_age female education diabetes hypertension, treatment(treated) wvar(ipw)":. balancetab index_age female education diabetes hypertension, treatment(treated) wvar(ipw)}{p_end}

{pstd}
{bf:Example 3: With Love plot and Excel export}

{phang2}{cmd:. balancetab index_age female education diabetes hypertension, ///}{p_end}
{phang2}{cmd:     treatment(treated) wvar(ipw) ///}{p_end}
{phang2}{cmd:     xlsx(balancetab/examples/balance.xlsx) loveplot saving(balancetab/examples/loveplot.png) scheme(plotplainblind)}{p_end}

{pstd}
{bf:Example 4: Custom threshold}

{phang2}{stata "balancetab index_age female education, treatment(treated) threshold(0.25)":. balancetab index_age female education, treatment(treated) threshold(0.25)}{p_end}
...
{synopt:{cmd:r(max_smd_adj)}}maximum absolute SMD after adjustment; only stored when {opt wvar()} is specified{p_end}
...
{pstd}Version 1.1.1, 2026-02-26{p_end}
