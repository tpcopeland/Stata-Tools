*! consort Version 1.0.3  2026/02/27
...
program define _consort_init, rclass
    ...
    * Set up file path
    if "`file'" == "" {
        * Create a temp file that persists across program calls
        ...
        local rand = int(runiform() * 100000)
        local file "`tmpdir'/consort_`ts'_`rand'.csv"
    ...
    * Initialize CSV file
    ...
    file write `fh' `""`safe_initial'",`n',"' _n
    file close `fh'
...
program define _consort_exclude, rclass
    ...
    * gets consumed as Stata's compound-quote close, leaving a stray ")
    ...
    file write `fh' `""`safe_label'",`n_excl',"' _char(34) `"`safe_remaining'"' _char(34) _n
    file close `fh'
    ...
    * Rewrite with final label on last data line
    ...
    forvalues i = 1/`linenum' {
        if `i' == `linenum' {
            ...
            * Check if remaining field is empty: line ends with ,"" or ,
            ...
            if `needs_final' {
                * Strip trailing ,"" or , and rewrite with final label
                ...
                file write `fh' `"`macval(trimmed)'"' "," _char(34) "`final_label'" _char(34) _n
            }
            else {
