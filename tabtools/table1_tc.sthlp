...
{synopt:{opt gurmeet}}equivalent to specifying:  percformat(%5.1f) percent_n percsign("") iqrmiddle(",") sdleft(" [±") sdright("]") gsdleft(" [×/") gsdright("]") extraspace{p_end}
...
{pstd}{bf:SSRI vs SNRI baseline characteristics:}{p_end}
...
{phang2}{stata "replace hypertension = 0 if missing(hypertension)":. replace hypertension = 0 if missing(hypertension)}{p_end}
{phang2}{stata "replace anxiety = 0 if missing(anxiety)":. replace anxiety = 0 if missing(anxiety)}{p_end}
{phang2}{cmd:. }{p_end}
{phang2}{cmd:. table1_tc, by(treated) ///}{p_end}
{phang3}{cmd:vars(index_age contn %5.1f \ female bin \ education cat \ ///}{p_end}
{phang3}{cmd:     income_quintile cat \ born_abroad bin \ ///}{p_end}
{phang3}{cmd:     diabetes bin \ hypertension bin \ anxiety bin) ///}{p_end}
