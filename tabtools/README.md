...
- **1.7.1** (2026-06-14): Bug fix for `table1_tc` weighted display. In `wt()` analyses the displayed count is now the effective count (weighted % × group N) instead of the raw unweighted count, so the count and percentage are internally consistent (n/N = weighted %) and weighted columns differ from crude ones. This corrects `wtcompare` tables where the crude and weighted count columns were previously identical; `r(categorical)` still returns raw counts for programmatic use.
