...
- **2.1.0** (2026-09-03): Stopped `regtab` labelling every constrained coefficient `Reference`. A base category, a term dropped for collinearity, and a factor cell identifying no observations all arrive from `collect` as a zero — a one after `eform` — with an empty confidence interval and an empty p-value, and the level key `collect` exposes has had its `b.`/`o.` marker stripped, so the rendered table cannot tell them apart. Every one of them was labelled `Reference` and merged across its model's estimate/CI/p triplet: a model with `i.grp` and a collinear fourth level printed two reference categories, `ibn.` printed a reference category for a model that has none, and a factor with a non-default base printed the true base and the dropped level identically. `regtab` now reads the constraint class the collection records for each coefficient and prints `Reference`, `Omitted`, or `Empty`, per model, so a level one model dropped keeps its estimate in the models that retained it; the new `omitlabel()` and `emptylabel()` set those words as `refcat()` sets the first. A dropped or empty cell contributes no estimate to `r(table)` and no row to the `eplotframe()` companion; a reference row is still posted there as before. A non-factor term dropped for collinearity is labelled by its variable rather than by `collect`'s raw `o.varname` key. The same reading now covers the multi-equation and multilevel layouts: a `mixed`/`melogit` table takes the class through the `coleq` row keys, and a multinomial base-outcome equation, in which no coefficient is estimated at all, is dropped whole rather than left standing as a row of pseudo-reference categories beside the estimated equations. An interaction term now heads one parent row for all its level combinations instead of repeating a header — built from the first component only, the parent of `1.grp#0.sex` was `grp#0.sex`, which matched nothing else. `effecttab` printed the literal `omit` as a point estimate and `(omit, omit)` as its confidence interval whenever `margins` reported a row as not estimable, and `empty`/`(empty, empty)` for a factor cell identifying no observations. The shared collection renderer read whichever member of a saved cell came first, and for those cells `collect` writes the constraint class ahead of the value — so the metadata was rendered as the number. It now asks for the value member by name. `effecttab` reads the same constraint class `regtab` does and writes `Reference`, `Omitted`, or `Empty`, with matching `omitlabel()` and `emptylabel()` options; constrained cells contribute nothing to `r(table)` or to the `eplotframe()` companion. Factor rows are now labelled the way `regtab` labels them — the variable's label heads the block, levels are indented beneath it under their value labels — instead of arriving as the raw term `1.grp`. New `qa/test_effecttab_omitted.do` pins eight contracts, seven of which fail on 2.0.3. New `qa/test_regtab_omitted.do` pins fifteen contracts; fourteen fail on 2.0.3 and the fifteenth holds the output of a model with nothing dropped unchanged.
- **2.0.3** (2026-08-30): Extended `comptab` rate-scaffold transaction cleanup through the forest-plot and staged-frame commit tail. If an `eplot` dependency returns success after invalidating its input frame, `comptab` and the `hrcomptab` compatibility wrapper now restore the caller's dataset and leave existing destination frames unchanged before returning the dependency-induced error. Added a fail-first regression for that post-dependency cleanup path.
