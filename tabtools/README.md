...
- `markdown(filename)` writes GitHub-Flavored Markdown and replaces an existing file. `mdappend` appends when the target exists and creates it otherwise. Cell, header, title, and footnote text is written literally: it is never macro-expanded, and `\ | * _`, backtick, `< > & [ ] ~` are backslash-escaped so data never render as emphasis, HTML, entities, links, strikethrough, or code. Leading spaces in the row-label (first) column are written as `&nbsp;` entities, so indented categorical and factor-level rows keep their hierarchy after GFM trims cell whitespace; value cells are trimmed.
...
- **2.1.13** (2026-09-27): `regtab` shows a covariate and another model's ancillary parameter or cutpoint of the same name (a covariate `alpha` beside an `nbreg`, a covariate `cut1` beside an `ologit`) as two rows, as it already did within one model; they used to share one row. Two such ancillary rows next to each other (a Weibull model with covariates `ln_p` and `p`) now follow the covariates in their usual order instead of floating up among them. Added regressions.
- **2.1.12** (2026-09-26): `regtab` decides intercept, cutpoint and ancillary rows from each coefficient's equation, not its label, so a covariate named or labelled `p`, `alpha`, `constant` or `cut1` is kept and exponentiated, a covariate sharing its model's ancillary name (`nbreg y alpha`) shows both rows, and lognormal/loglogistic `lnsigma`/`sigma`/`lngamma`/`gamma` keep their own scale. `bootstrap:`, `jackknife:` and `mi estimate:` fits are classified like their plain counterparts (`mi` follows the [MI] eform rule and its t-based intervals), and `xtgee` follows `glm`'s family/link rule (binomial/logit OR, poisson/log IRR). New `stats()` tokens `events`, `r2_a`, `rmse`, `F`, `mi_m` and `fmi`. AIC, BIC and QICu count the estimated free parameters, which is `e(rank)` (base and omitted levels, `mlogit`'s base equation and linear constraints excluded, as `estat ic`) under every vce, except that after a robust-type vce over too few clusters, GEE panels or replications to reach full rank they count the coefficients with a nonzero standard error instead of the capped `e(rank)`. `bs:`, `bstrap:` and `jknife:` fits are classified like `bootstrap:`/`jackknife:` ones, and `regtab` leaves the labels and levels of the collection's result dimension as it found them. The methods sentence names the fitted model (probit, negative binomial, GEE, mixed-effects, Fine-Gray, heteroskedastic probit, quantile, instrumental-variables, ...) and says univariable or multivariable, counting variance-equation covariates, and `coef()`/`cdisc` no longer change it. `mlogit`/`ologit` factor covariates get a header row per equation, `mestreg, time` shows its random intercept as a variance, and duplicate `r(table)` row names get `_2`, `_3`, ... in `regtab` and `effecttab`. CI levels such as 99.9 print without floating-point noise. Every command refuses two output files that resolve to the same path before writing anything (a colliding `csv()` used to overwrite the workbook), and `csv()` must end in `.csv`. The Markdown writer is macro-safe (backticks, quotes and `$` are written literally, as are `title()`/`footnote()` text in every command), replaces an existing file unless `mdappend`, and escapes `` ` ``, `<`, `>`, `&`, `[`, `]` and `~`. `table1_tc`/`desctab` weighted statistics stay correct when weighted sums overflow. `stratetab` refuses missing `pyscale()`/`ratescale()`. `survtab` notes and returns `r(beyond_support)` for `times()` past a group's follow-up. `effecttab` `from()` headers read `95% CI`/`p-value`, row names with spaces stay on one row, and its methods text no longer borrows the active `e(subcmd)`. `puttab` leaves a `matrix()` label header blank in Markdown, loads `frame()` quietly, and skips a one-cell footnote merge. `stacktab` draws `borders(bottom(row #))` on that row, reads `rows()` alone in sheet coordinates, and accepts `style()` option names in any case. Added regressions.
- **2.1.11** (2026-09-26): `regtab` keeps rows that only a later model estimates: a covariate one model lacks, `zinb`'s ancillary rows beside `zip`, and whole equations absent from model 1 (`zip` + `poisson` of another outcome, `ologit` + `mlogit`). Multilevel models keep a factor's header row. A transformed interval that overflows (a boundary-variance MOR, `exp()` of a separated coefficient) is left blank instead of showing collect's untransformed text. `relabel` names me* slope variances and covariances like `mixed`. `r(table)` row names no longer fall back to `r1, r2, ...` or name a covariance `var(...)`. `svy:`-prefixed fits are classified like their plain counterparts (OR/IRR/RRR, intercept suppressed). `effecttab` top-aligns every body cell, not just the label column, and merges a Reference/Omitted/Empty row only in the model block that holds it, so another model's CI and p-value on that row are kept. Added regressions.
- **2.1.10** (2026-09-25): Correctness fixes from an independent review. `regtab` reads each model's scale only from its own options (after the command's top-level comma, with each estimator's abbreviations), so `glm ..., ef` is no longer exponentiated twice, a covariate named `or` no longer suppresses exponentiation, `stcox, nohr`, `logistic, coef`, and time-metric `streg` without `tr` display on the ratio scale their header states, PH-metric `streg` is labelled HR, and collections mixing confidence levels are rejected instead of mislabelled. `hrcomptab` places each model estimate in the rate-table row whose label it matches and errors when a selection cannot be matched exactly, instead of filling rows by position. `table1_tc`/`desctab` accept double quotes in variable and value labels, default the GSD to two decimals, compute categorical SMDs over the levels of the two compared groups, return one `r(table)` row per variable with no 200-row cap, and join the header descriptor consistently using the active `iqrmiddle()`. `stratetab` and `hrcomptab` preserve the caller's data with `preserve` (so `c(filename)` and `c(changed)` are untouched), `stratetab` commits `frame()` only after every export succeeds, supports rate files with no grouping variable, and returns `r(csv)`; its help documents that `ratescale()`/`pyscale()` assume `strate`'s default `per(1)`. `puttab` evaluates `if`/`in` inside a `frame()` source, exports `%t` dates as dates, accepts `matrix(r(table))`, keeps a genuine first data row, and reports the sheet name actually written. Added regressions.
- **2.1.9** (2026-09-25): `regtab` labels the base level of a factor *Reference* after `nbreg`, `zinb`, `intreg`, and `streg` with an ancillary parameter (e.g. `dist(weibull)`), where it printed *Empty*. Stata 17's `collect` stamps every constrained cell of these fits "empty" even though `e(b)` marks the level `b.` and the estimator's own table reports `(base)`. A model whose only recorded class is "empty" is now treated as unclassified, so its constrained factor levels take the `refcat()` label; models with genuine empty cells (which always also carry a base or omitted cell) keep `emptylabel()`, per model within one collection. The collection cannot separate a base cell from a genuinely empty or collinear one under these estimators, so their interaction terms read *Reference* on every constrained cell. The unclassified fallback also now requires an empty p-value as well as an empty CI, so an estimated level whose interval bound is missing (a near-separated fit printing `(0, .)`) shows its estimate instead of *Reference*. Added `regtab` regressions.
...
- **2.0.2** (2026-08-30): Preserved caller-owned Mata objects across Excel exports, made `stacktab` multi-sink output transactional, posted documented results without silent capture, and limited `survtab` RMST-difference returns to two-group analyses.
...
- **1.16.2** (2026-08-19): Fixed Windows style compaction by normalizing archive paths from Mata's platform-specific `dir()` output before manifest comparison and rebuilding.
...
- **1.15.0** (2026-08-13): Closed a reconstruction leak in `table1_tc, smallcells()` by suppressing percentages whenever a protected variable contains a primary suppression, rejected incompatible percent-only displays, and made the `corrtab` star legend independent of how thresholds were supplied.
