...
{synopthdr}
{synoptline}
{synopt:{opt sheet(name)}}Excel sheet name; default is {cmd:"Table"}{p_end}
...
{synopt:{opt fra:me(name)}}Store output in a named Stata frame for programmatic access{p_end}
{synoptline}
{p 4 6 2}{opt sheet()} defaults to "Table" if omitted.{p_end}
...
{phang}
{opt sheet(name)} specifies the Excel sheet name. Default is {cmd:"Table"}.
