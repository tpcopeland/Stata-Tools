*! stratetab Version 1.0.5  25feb2026
*! Author: Tim Copeland
...
* Save user data before processing (tempfile since inner loop uses preserve/restore)
tempfile _userdata
qui save "`_userdata'"
...
if _rc {
	noi di as err "Excel cell formatting failed with error `=_rc'"
	exit _rc
}

* Restore user data
qui use "`_userdata'", clear
