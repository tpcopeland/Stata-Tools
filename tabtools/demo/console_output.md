...
Simulation Studies
  simtab       - Monte Carlo performance table (pairs with simsum/siman)
...
Simulation Studies
  ------------------------------------------------------------
  simtab       Render and export a Monte Carlo simulation
               performance table (bias, empirical/model SE,
               coverage, ...) from replication-level results,
               or ingest a simsum/siman summary. Pairs with
               simsum and siman for full analysis and graphs.
...
```
  +---------------------------------------------------------------------------------------------------------------------
> ----------+
  |                                                         SSRI (N=8934)                SNRI (N=6066)         Differenc
```
...
```
  |                       Median survival, d                       5699.0                       5660.0               39.
> 0   0.094 |
...
> )         |
  |                RMST (1460-d), d (95% CI)   1448.87 (1447.02, 1450.72)   1449.75 (1447.55, 1451.96)              -0.8
> 8         |
...
```
puttab: wrote 3 data rows x 3 cols (data source) to sheet Block Primary in /home/tpcopeland/Stata-Tools/tabtools/demo/_p
```
...
```
puttab: wrote 2 data rows x 3 cols (data source) to sheet Block Dose in /home/tpcopeland/Stata-Tools/tabtools/demo/_pipe
```
...
```
note: coverage 85.8% is off-nominal (estimator Unweighted, scenario A, estimand Marginal slope)
note: coverage 85.2% is off-nominal (estimator Unweighted, scenario A, estimand Treatment contrast)
note: coverage 74.7% is off-nominal (estimator Unweighted, scenario B, estimand Marginal slope)
note: coverage 79.5% is off-nominal (estimator Unweighted, scenario B, estimand Treatment contrast)
note: coverage 97.0% is off-nominal (estimator IIW, scenario B, estimand Treatment contrast)
note: coverage 92.3% is off-nominal (estimator IIW + log(test), scenario B, estimand Treatment contrast)
note: coverage 73.9% is off-nominal (estimator Unweighted, scenario C, estimand Marginal slope)
note: coverage 72.1% is off-nominal (estimator Unweighted, scenario C, estimand Treatment contrast)
note: coverage 91.5% is off-nominal (estimator IIW + log(test), scenario C, estimand Marginal slope)
note: coverage 89.5% is off-nominal (estimator IIW + log(test), scenario C, estimand Treatment contrast)
Markdown exported to tabtools/demo/demo_markdown_report.md
. }
```
...
| Scenario | Estimator | Marginal slope: Mean | Marginal slope: Bias | Marginal slope: Coverage | Marginal slope: N | Tr
```

```stata
> eatment contrast: Mean | Treatment contrast: Bias | Treatment contrast: Coverage | Treatment contrast: N |
```
