...
**# Sheet 47: Desctab Events -- events / N (%) from a table collect
use `analysis', clear
collect clear
collect: table education treated, ///
    statistic(sum cv_event) statistic(count cv_event) statistic(mean cv_event)

desctab, xlsx("`xlsx_desctab'") sheet("Events") ///
    title("Table X. Cardiovascular Events by Education and Treatment") ///
    compose(events_n_pct) pctdigits(1) ///
    footnote("Cells show events / N (%). Statistics are generated by Stata table and formatted by desctab.")

**# Sheet 48: Desctab Styled Events -- explicit shaded fills
desctab, xlsx("`xlsx_desctab'") sheet("Styled Events") ///
    title("Table X. Cardiovascular Events by Education and Treatment") ///
    compose(events_n_pct) pctdigits(1) headershade zebra ///
    footnote("Cells show events / N (%). Shading is explicitly requested with headershade and zebra.")

**# Sheet 49: Desctab Mean SD -- mean (SD) from a table collect
collect clear
collect: table (var) (treated), ///
    statistic(mean index_age crp prior_hosp) ///
    statistic(sd index_age crp prior_hosp)

desctab, xlsx("`xlsx_desctab'") sheet("Mean SD") ///
    title("Table X. Descriptive Characteristics by Treatment") ///
    compose(mean_sd) digits(1) ///
    footnote("Cells show mean (SD) with statistic-specific formats.")

**# Sheet 50: Desctab Median IQR -- median (IQR) from percentiles
collect clear
collect: table treated, ///
    statistic(p25 crp) statistic(p50 crp) statistic(p75 crp)

desctab, xlsx("`xlsx_desctab'") sheet("Median IQR") ///
    title("Table X. CRP by Treatment") ///
    compose(median_iqr) digits(1) ///
    footnote("Cells show median (p25-p75), composed from table percentile statistics.")

**# Sheet 51: Desctab Separate Stats -- per-statistic columns and labels
collect clear
collect: table education treated, ///
    statistic(count index_age) statistic(mean index_age) statistic(sd index_age)

desctab, xlsx("`xlsx_desctab'") sheet("Separate Stats") ///
    title("Table X. Age by Education and Treatment") ///
    statorder(count mean sd) ///
    statlabels("count=N \ mean=Mean \ sd=SD") ///
    nformats("count %8.0fc mean %8.1f sd %8.1f") ///
    footnote("Each statistic keeps its own number format.")

**# Sheet 52: Desctab Custom -- custom compose() template
collect clear
collect: table education, ///
    statistic(sum cv_event) statistic(count cv_event) statistic(mean cv_event)

desctab, xlsx("`xlsx_desctab'") sheet("Custom") ///
    title("Table X. Custom Descriptive Composition") ///
    compose("{total} of {count} ({mean})") pctscale(0to100) pctsign ///
    pctdigits(1) ///
    footnote("Custom compose() templates substitute formatted {stat} values.")
