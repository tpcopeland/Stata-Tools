...
{synoptset 24 tabbed}{...}
{synoptline}
{synopt:{opt spe:arman}}use Spearman rather than Pearson correlations{p_end}
{synopt:{opt low:er}}display the lower triangle only (default){p_end}
{synopt:{opt upp:er}}display the upper triangle only{p_end}
{synopt:{opt full}}display the full symmetric matrix{p_end}
{synopt:{opt star(numlist)}}Significance thresholds for stars{p_end}
{synopt:{opt pval:ues}}show p-values in parentheses instead of stars{p_end}
{synopt:{opt dig:its(#)}}decimal places for correlation coefficients{p_end}
{synoptline}

{dlgtab:Output}

{synoptset 24 tabbed}{...}
{synopt:{opt xlsx(filename)}}export to Excel{p_end}
...
{opt full} may be specified{p_end}
...
nonlinear monotonic data).{p_end}
...
{phang}
{opt upp:er} display the upper triangle only{p_end}

{phang}
{opt xlsx(filename)} export to Excel; filename must end in {cmd:.xlsx}; if the file exists, only the
named sheet is replaced{p_end}
