/*******************************************************************************
* audit_effecttab_gcomptab.do
*
* Purpose: Expert-level audit of effecttab and gcomptab Excel output
...
* Add gcomp package to adopath (gcomptab moved from tabtools to gcomp)
adopath ++ "${STATA_TOOLS_PATH}/gcomp"

display as text _n "{hline 70}"
display as text "EXPERT AUDIT: effecttab and gcomptab"
...
display as text _n "SECTION 2: Creating gcomptab test files for Python analysis"

* Mock gcomp program
capture program drop mock_gcomp
program define mock_gcomp, rclass
    version 16.0
...
* -----------------------------------------------------------------------------
* Test 1: Basic gcomptab
* -----------------------------------------------------------------------------
display as text "  Test 1: Basic gcomptab"
mock_gcomp, tce(0.150) nde(0.100) nie(0.050) pm(0.333) cde(0.085) ///
    se_tce(0.030) se_nde(0.025) se_nie(0.015) se_pm(0.080) se_cde(0.025)

gcomptab, xlsx("${AUDIT_DIR}/gcomptab_01_basic.xlsx") sheet("Mediation") ///
    title("Test 1: Basic Mediation Analysis")

display as result "    Created: gcomptab_01_basic.xlsx"

* -----------------------------------------------------------------------------
* Test 2: Custom labels
* -----------------------------------------------------------------------------
display as text "  Test 2: Custom labels"
mock_gcomp, tce(0.180) nde(0.120) nie(0.060) pm(0.333) cde(0.100)

gcomptab, xlsx("${AUDIT_DIR}/gcomptab_02_labels.xlsx") sheet("Custom") ///
    labels("Total \ Direct \ Indirect \ % Mediated \ Controlled") ///
    title("Test 2: Custom Labels")

display as result "    Created: gcomptab_02_labels.xlsx"

* -----------------------------------------------------------------------------
* Test 3: Percentile CI
* -----------------------------------------------------------------------------
display as text "  Test 3: Percentile CI"
mock_gcomp, tce(0.200) nde(0.130) nie(0.070) pm(0.350) cde(0.110)

gcomptab, xlsx("${AUDIT_DIR}/gcomptab_03_percentile.xlsx") sheet("Pct") ///
    ci(percentile) title("Test 3: Percentile CI")

display as result "    Created: gcomptab_03_percentile.xlsx"

* -----------------------------------------------------------------------------
* Test 4: High precision (4 decimals)
* -----------------------------------------------------------------------------
display as text "  Test 4: High precision"
mock_gcomp, tce(0.1523) nde(0.1012) nie(0.0511) pm(0.3355) cde(0.0856)

gcomptab, xlsx("${AUDIT_DIR}/gcomptab_04_precision.xlsx") sheet("Dec4") ///
    decimal(4) title("Test 4: 4 Decimal Precision")

display as result "    Created: gcomptab_04_precision.xlsx"

* -----------------------------------------------------------------------------
* Test 5: Negative effects
* -----------------------------------------------------------------------------
display as text "  Test 5: Negative effects"
mock_gcomp, tce(-0.120) nde(-0.080) nie(-0.040) pm(0.333) cde(-0.070) ///
    se_tce(0.040) se_nde(0.030) se_nie(0.020) se_pm(0.100) se_cde(0.030)

gcomptab, xlsx("${AUDIT_DIR}/gcomptab_05_negative.xlsx") sheet("Negative") ///
    title("Test 5: Negative (Protective) Effects")

display as result "    Created: gcomptab_05_negative.xlsx"

* -----------------------------------------------------------------------------
* Test 6: Large effects
* -----------------------------------------------------------------------------
display as text "  Test 6: Large effects"
mock_gcomp, tce(0.450) nde(0.280) nie(0.170) pm(0.378) cde(0.250) ///
    se_tce(0.080) se_nde(0.060) se_nie(0.050) se_pm(0.100) se_cde(0.060)

gcomptab, xlsx("${AUDIT_DIR}/gcomptab_06_large.xlsx") sheet("Large") ///
    title("Test 6: Large Effects")

display as result "    Created: gcomptab_06_large.xlsx"

* -----------------------------------------------------------------------------
* Test 7: Small effects (near zero)
* -----------------------------------------------------------------------------
display as text "  Test 7: Small effects"
mock_gcomp, tce(0.020) nde(0.015) nie(0.005) pm(0.250) cde(0.012) ///
    se_tce(0.030) se_nde(0.025) se_nie(0.015) se_pm(0.150) se_cde(0.025)

gcomptab, xlsx("${AUDIT_DIR}/gcomptab_07_small.xlsx") sheet("Small") ///
    decimal(4) title("Test 7: Small Effects")

display as result "    Created: gcomptab_07_small.xlsx"

* -----------------------------------------------------------------------------
* Test 8: All options combined
* -----------------------------------------------------------------------------
display as text "  Test 8: All options combined"
mock_gcomp, tce(0.175) nde(0.115) nie(0.060) pm(0.343) cde(0.095) ///
    se_tce(0.035) se_nde(0.028) se_nie(0.018) se_pm(0.085) se_cde(0.028)

gcomptab, xlsx("${AUDIT_DIR}/gcomptab_08_full.xlsx") sheet("Complete") ///
...
display as result "    Created: gcomptab_08_full.xlsx"
