...
def main() -> None:
    ...
    # console_output.md is a documented, gitignored inspection artifact.  A
    # leftover copy in the expected tree must not turn it into a tracked-golden
    # input merely because the release test runs from a used checkout.
    transient_markdown = {"console_output.md"}
    exp_md = {
        p.name: p
        for p in args.expected_dir.glob("*.md")
        if p.name not in transient_markdown
    }
    act_md = {
        p.name: p
        for p in args.actual_dir.glob("*.md")
        if p.name not in transient_markdown
    }
    if not act_md and exp_md:
        ...
        # from the tracked tree. Comparing the two inventories for equality
        # wrongly failed on that.
