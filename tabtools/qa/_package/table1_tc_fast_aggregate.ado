*! table1_tc_fast_aggregate Version 0.1.0  2026/05/23
*! Fast pre-finalization aggregation helper prototype for table1_tc
*! Author: Timothy P Copeland, Karolinska Institutet
*! Program class: rclass

program define table1_tc_fast_aggregate, rclass
    version 16.0
    ...
    capture noisily {
        ...
        syntax [if] [in] [fweight], BY(varname numeric) VARS(string asis) ///
            ...
            [ REPLACE STUB(name) TOTAL(string) TOTALCode(real -1) ///
              WT(varname numeric) SMD TEST STATistic NOPvalue ///
        ...
        local has_wt = "`wt'" != ""
        if `has_wt' & "`percent_n'" == "" local percent "percent"
        ...
        forvalues i = 1/`nvars' {
            ...
            if !`suppress_p' {
                if inlist("`typ'", "contn", "contln") & `nglevels' >= 2 {
                    ...
                    if "`typ'" == "cat" {
                        capture quietly tab `v' `by' [`weight'`exp'] if `touse' & `by' < ., chi2 m
                        if _rc == 0 {
                            local p`i' = r(p)
                        ...
                        capture quietly tab `v' `by' [`weight'`exp'] if `touse' & `by' < ., exact m
                        if _rc == 0 {
                            local p`i' = r(p_exact)
                ...
                if inlist("`typ'", "contn", "contln", "conts") {
                    ...
                    foreach _clv of local _smd_lvls {
                        if `has_wt' {
                            ...
                            quietly summarize `fwvar' if `touse' & `by' == `level1' & `v' < .
                            local _tot1 = r(sum)
                            quietly summarize `fwvar' if `touse' & `by' == `level2' & `v' < .
                            local _tot2 = r(sum)
                            quietly summarize `fwvar' if `touse' & `by' == `level1' & `v' == `_clv'
                            local _num1 = r(sum)
                            quietly summarize `fwvar' if `touse' & `by' == `level2' & `v' == `_clv'
                            local _num2 = r(sum)
                        }
                        else {
        ...
        mata: _table1_tc_fast_aggregate_mata("`touse'", "`by'", ///
        ...
        quietly gen double smd_val = .
        quietly gen str48 test = ""
        quietly gen str48 statistic = ""
...
capture mata: mata drop _t1tcfa_group_index()
capture mata: mata drop _t1tcfa_level_index()
capture mata: mata drop _t1tcfa_wquantile()
capture mata: mata drop _table1_tc_fast_aggregate_mata()

mata:
real scalar _t1tcfa_group_index(real scalar value, real colvector levels)
{
    real scalar i
...
real scalar _t1tcfa_level_index(real scalar value, real colvector levels)
{
    real scalar i
...
real scalar _t1tcfa_wquantile(real colvector x, real colvector w, real scalar p)
{
    real colvector ord, xs, ws
    real scalar i, target, running, total
...
void _table1_tc_fast_aggregate_mata(
    ...
    string scalar cat_name)
{
    ...
    for (i = 1; i <= n; i++) {
        ...
        gi = _t1tcfa_group_index(group[i], group_levels)
        if (gi >= .) continue
    ...
    for (j = 1; j <= nv; j++) {
        ...
        if (types[j] == "conts") {
            for (g = 1; g <= ngout; g++) {
                ...
                if (rows(xvals) > 0) {
                    cont_a[j, g] = _t1tcfa_wquantile(xvals, wvals, .50)
                    cont_b[j, g] = _t1tcfa_wquantile(xvals, wvals, .25)
                    cont_c[j, g] = _t1tcfa_wquantile(xvals, wvals, .75)
                }
            }
            continue
        }
    ...
    for (j = 1; j <= nv; j++) {
        ...
        for (i = 1; i <= n; i++) {
            ...
            if (types[j] == "bin" | types[j] == "bine") {
                ...
                li = _t1tcfa_level_index(x, levels)
                value_ok = li < .
            }
            if (!value_ok) continue
            gi = _t1tcfa_group_index(group[i], group_levels)
