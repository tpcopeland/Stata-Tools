...
**## 5b. All 7 categories are returned
local ++test_count
capture noisily {
    tabtools
...
if _rc == 0 {
    display as result "  PASS: tabtools returns all 7 categories"
    local ++pass_count
}
else {
...
local ++test_count
local t5c_pass = 1
foreach cat in descriptive models rates survival diagnostics composite general {
    capture noisily {
...
local ++test_count
local t5d_pass = 1
foreach cat in all descriptive models rates survival diagnostics composite general {
    capture noisily {
