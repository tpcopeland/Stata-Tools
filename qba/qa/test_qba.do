...
if _rc == 0 {
    display as result "  PASS: T0.1 Fresh install auto-loads helper programs"
    local ++pass_count
}
else {
    display as error "  FAIL: T0.1 Helper auto-load after install (error `=_rc')"
    local ++fail_count
}

* Load bundled helper programs only after the fresh-install path is verified.
...
capture restore
capture noisily {
    tempfile qba_test_save
    qba_misclass, a(136) b(297) c(1432) d(6738) seca(.85) spca(.95) ///
        reps(200) seed(42) saving("`qba_test_save'", replace)
    preserve
    use "`qba_test_save'", clear
    assert _N == 200
...
capture noisily {
    qba_multi, a(136) b(297) c(1432) d(6738) reps(500) ///
        seca(.85) spca(.95) ///
        sela(.9) selb(.85) selc(.7) seld(.8) ///
        order(selection misclass) seed(12345)
...
local ++test_count
capture noisily {
    tempfile qba_mc_for_plot
    qba_misclass, a(136) b(297) c(1432) d(6738) seca(.85) spca(.95) ///
        reps(500) dist_se("uniform .75 .95") seed(42) ///
        saving("`qba_mc_for_plot'", replace)
...
* T9.8: Default scheme inherits the current Stata scheme
local ++test_count
capture noisily {
    qba_plot, tornado a(136) b(297) c(1432) d(6738) ///
        param1(se) range1(.7 1.0) steps(5) ///
...
* T9.9: Tornado leaves infeasible misclassification ranges missing
local ++test_count
capture noisily {
...
if _rc == 0 {
    ...
    display as error "  FAIL: T9.9 Tornado infeasible range (error `=_rc')"
    local ++fail_count
}

* T9.10: Graph saving exports non-empty SVG payloads with expected labels
local ++test_count
capture noisily {
    local plot_dir "`c(tmpdir)'/qba plot exports"
    capture mkdir "`plot_dir'"
    local tornado_svg "`plot_dir'/tornado export.svg"
    local dist_svg "`plot_dir'/distribution export.svg"
    local tipping_svg "`plot_dir'/tipping export.svg"
    tempfile mc_for_svg
    ...
    qba_misclass, a(136) b(297) c(1432) d(6738) seca(.85) spca(.95) ///
        ...
        saving("`mc_for_svg'", replace)
    qba_plot, distribution using("`mc_for_svg'") observed(2.15) ///
    ...
    capture erase "`dist_svg'"
    capture erase "`tipping_svg'"
    capture rmdir "`plot_dir'"
}
if _rc == 0 {
    display as result "  PASS: T9.10 Graph saving exports expected SVG payloads"
    local ++pass_count
}
else {
...
* T10.3: from_model preserves active estimation results
local ++test_count
capture noisily {
    sysuse auto, clear
    quietly logistic foreign mpg weight
    local ecmd_before "`e(cmd)'"
    local b_mpg_before = _b[mpg]
    local se_mpg_before = _se[mpg]

    qba_confound, from_model coef(mpg) p1(.3) p0(.1) rrcd(2.0) evalue

    assert "`e(cmd)'" == "`ecmd_before'"
    local b_mpg_after = _b[mpg]
    local se_mpg_after = _se[mpg]
    _assert_close `b_mpg_after' `b_mpg_before' 0.0000000001
    _assert_close `se_mpg_after' `se_mpg_before' 0.0000000001
}
if _rc == 0 {
    display as result "  PASS: T10.3 from_model preserves e() state"
    local ++pass_count
}
else {
    display as error "  FAIL: T10.3 e() preservation (error `=_rc')"
    local ++fail_count
}

* T10.4: qba_plot preserves the caller's data
local ++test_count
capture noisily {
    tempfile plot_mc
    qba_misclass, a(100) b(200) c(300) d(400) seca(.9) spca(.95) ///
        reps(120) dist_se("constant .9") dist_sp("constant .95") ///
        seed(123) saving("`plot_mc'", replace)
...
if _rc == 0 {
    display as result "  PASS: T10.4 qba_plot preserves data"
    local ++pass_count
}
else {
...
if `fail_count' > 0 {
    ...
    display "RESULT: test_qba tests=`test_count' pass=`pass_count' fail=`fail_count'"
    exit 1
}
else {
