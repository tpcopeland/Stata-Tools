...
local ++test_count
capture noisily {
    qba_misclass, a(215) b(1449) c(668) d(4296) seca(.78) spca(.99) measure(RR) ///
        reps(100000) dist_se("beta 50.6 14.3") dist_sp("beta 70 1") ///
        totalerror seed(12345)
    assert r(reps) == 100000
}
local rc = _rc
if `rc' local ++fail_count
else local ++pass_count
...
local ++test_count
capture noisily {
    qba_misclass, a(215) b(1449) c(668) d(4296) seca(.78) spca(.99) secb(.75) spcb(.98) ///
...
local ++test_count
capture noisily {
    qba_plot, distribution using(mc_sel) observed(2.15)
    assert "`r(plot_type)'" == "distribution"
}
local rc = _rc
if `rc' local ++fail_count
...
foreach spec in ///
    ...
    capture noisily {
        qba_plot, `spec'
        assert inlist("`r(plot_type)'", "tornado", "tipping")
    }
    local rc = _rc
    if `rc' local ++fail_count
    else local ++pass_count
