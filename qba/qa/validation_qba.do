...
* V5.5: All three biases with default order match hand calculation
*   Misclass(.8,.9) -> Selection(.8,.5,.6,.9) -> Confound(p1=.4,p0=.1,rrcd=3)
*   Cell-level corrected OR after misclass+selection = 55/48 = 1.1458333
*   Confounding BF = (0.4*(3-1)+1)/(0.1*(3-1)+1) = 1.8/1.2 = 3/2
*   Final corrected OR = (55/48)/(3/2) = 55/72 = 0.7638889
local ++test_count
capture noisily {
    qba_multi, a(80) b(120) c(200) d(600) reps(500) ///
        seca(.8) spca(.9) ///
        ...
        dist_sela("constant .8") dist_selb("constant .5") ///
        dist_selc("constant .6") dist_seld("constant .9") ///
        dist_p1("constant .4") dist_p0("constant .1") dist_rr("constant 3.0") ///
        seed(42)
    _assert_close `=r(corrected)' 0.7638889 0.001
    assert "`r(order)'" == "misclass selection"
}
if _rc == 0 {
    display as result "  PASS: V5.5 All three biases default order = hand solution"
    local ++pass_count
}
else {
    display as error "  FAIL: V5.5 Three-bias hand solution (error `=_rc')"
    local ++fail_count
}

* V5.6: Reversed cell-level order matches hand calculation
*   Selection first gives OR = 5/6; then misclass gives OR = 33/43
*   Final corrected OR = (33/43)/(3/2) = 22/43 = 0.5116279
local ++test_count
capture noisily {
    qba_multi, a(80) b(120) c(200) d(600) reps(500) ///
        seca(.8) spca(.9) ///
        ...
        dist_p1("constant .4") dist_p0("constant .1") dist_rr("constant 3.0") ///
        order(selection misclass) seed(42)
    _assert_close `=r(corrected)' 0.5116279 0.001
    assert "`r(order)'" == "selection misclass"
}
if _rc == 0 {
    display as result "  PASS: V5.6 Reversed order = hand solution"
    local ++pass_count
}
else {
    display as error "  FAIL: V5.6 Reversed-order hand solution (error `=_rc')"
    local ++fail_count
}
...
* Load helper explicitly for direct helper validation. These tests must not rely
* on earlier qba_* commands having already run the helper into memory.
...
local ++test_count
	capture noisily {
	    ...
	    qba_confound, from_model p1(.3) p0(.1) confeffect(2.0)
...
* V8.3: from_model with multivariable logistic requires coef() and applies BF exactly
local ++test_count
capture noisily {
    sysuse auto, clear
    quietly logistic foreign mpg weight
    local expected_or = exp(_b[mpg])
    local expected_bf = (0.3 * (2.0 - 1) + 1) / (0.1 * (2.0 - 1) + 1)
    qba_confound, from_model coef(mpg) p1(.3) p0(.1) rrcd(2.0)
    _assert_close `=r(observed)' `expected_or' 0.001
    _assert_close `=r(bias_factor)' `expected_bf' 0.001
    _assert_close `=r(corrected)' `=`expected_or'/`expected_bf'' 0.01
}
if _rc == 0 {
    display as result "  PASS: V8.3 from_model coef() selects multivariable logistic term"
    local ++pass_count
}
else {
    display as error "  FAIL: V8.3 from_model coef(logit) (error `=_rc')"
    local ++fail_count
}

* V8.4: from_model linear models use subtractive correction exactly
local ++test_count
capture noisily {
    sysuse auto, clear
	    quietly regress price mpg weight
	    local expected_coef = _b[weight]
	    local expected_corr = `expected_coef' - (.4 - .2) * 500
	    qba_confound, from_model coef(weight) p1(.4) p0(.2) confeffect(500)
	    _assert_close `=r(observed)' `expected_coef' 0.001
	    _assert_close `=r(corrected)' `expected_corr' 0.01
	    assert "`r(correction_type)'" == "subtractive"
}
if _rc == 0 {
    display as result "  PASS: V8.4 from_model linear subtractive correction"
    local ++pass_count
}
else {
    display as error "  FAIL: V8.4 from_model linear subtractive (error `=_rc')"
    local ++fail_count
}

* V8.5: from_model with poisson auto-detects RR and exponentiates
local ++test_count
capture noisily {
    sysuse auto, clear
    quietly poisson rep78 mpg if rep78 < ., nolog
    local expected_rr = exp(_b[mpg])
    local expected_bf = (0.3 * (2.0 - 1) + 1) / (0.1 * (2.0 - 1) + 1)
    qba_confound, from_model p1(.3) p0(.1) rrcd(2.0)
    assert "`r(measure)'" == "RR"
    _assert_close `=r(observed)' `expected_rr' 0.001
    _assert_close `=r(bias_factor)' `expected_bf' 0.001
    _assert_close `=r(corrected)' `=`expected_rr'/`expected_bf'' 0.01
}
if _rc == 0 {
    display as result "  PASS: V8.5 from_model poisson auto-detects RR"
    local ++pass_count
}
else {
    display as error "  FAIL: V8.5 from_model poisson (error `=_rc')"
    local ++fail_count
}

* V8.6: from_model with glm link(logit/log) auto-detects OR/RR
local ++test_count
capture noisily {
    sysuse auto, clear
    quietly glm foreign mpg, family(binomial) link(logit) nolog
    local expected_or = exp(_b[mpg])
    local expected_bf = (0.3 * (2.0 - 1) + 1) / (0.1 * (2.0 - 1) + 1)
    qba_confound, from_model p1(.3) p0(.1) rrcd(2.0)
    assert "`r(measure)'" == "OR"
    ...
    sysuse auto, clear
    quietly glm rep78 mpg if rep78 < ., family(poisson) link(log) nolog
    local expected_rr = exp(_b[mpg])
    qba_confound, from_model p1(.3) p0(.1) rrcd(2.0)
    assert "`r(measure)'" == "RR"
    _assert_close `=r(observed)' `expected_rr' 0.001
...
if _rc == 0 {
    display as result "  PASS: V8.6 from_model glm link auto-detection"
    local ++pass_count
}
else {
    display as error "  FAIL: V8.6 from_model glm links (error `=_rc')"
    local ++fail_count
}
...
capture restore
capture noisily {
    tempfile qba_val_misclass
    qba_misclass, a(80) b(120) c(200) d(600) seca(.8) spca(.9) ///
        reps(200) seed(42) saving("`qba_val_misclass'", replace)
    preserve
    use "`qba_val_misclass'", clear
    assert _N == 200
...
capture restore
capture noisily {
    tempfile qba_val_sel
    qba_selection, a(200) b(100) c(300) d(400) ///
        sela(.8) selb(.5) selc(.6) seld(.9) ///
        reps(200) seed(42) saving("`qba_val_sel'", replace)
    preserve
    use "`qba_val_sel'", clear
    assert _N == 200
    confirm variable corrected_or
    confirm variable a_corr
...
capture restore
capture noisily {
    tempfile qba_val_conf
    qba_confound, estimate(2.5) p1(.4) p0(.1) rrcd(3.0) ///
        reps(200) seed(42) saving("`qba_val_conf'", replace)
    preserve
    use "`qba_val_conf'", clear
    assert _N == 200
...
* V10.1: Negative corrected cells make corrected measure undefined
local ++test_count
capture noisily {
    qba_misclass, a(0) b(120) c(200) d(600) seca(.8) spca(.9)
    * a_corr = (0 - 0.1*120)/0.7 = -120/7 = -17.1428571
    _assert_close `=r(corrected_a)' -17.1428571 0.001
    _assert_close `=r(corrected_b)' 137.1428571 0.001
    _assert_close `=r(corrected_c)' 171.4285714 0.001
    _assert_close `=r(corrected_d)' 628.5714286 0.001
    assert r(corrected) == .
}
if _rc == 0 {
    display as result "  PASS: V10.1 Negative corrected cells return undefined measure"
    local ++pass_count
}
else {
...
* V10.1b: Positive corrected cells still yield finite corrected measure
local ++test_count
capture noisily {
    qba_misclass, a(80) b(120) c(200) d(600) seca(.8) spca(.9)
...
if _rc == 0 {
    display as result "  PASS: V10.1b Positive corrected cells yield finite measure"
    local ++pass_count
}
else {
    display as error "  FAIL: V10.1b Positive corrected cells (error `=_rc')"
    local ++fail_count
}

* V10.2: Very large table preserves scale invariance
local ++test_count
capture noisily {
    qba_misclass, a(80000) b(120000) c(200000) d(600000) seca(.8) spca(.9)
    * This is V1.1 scaled by 1000, so ORs should be unchanged.
    _assert_close `=r(observed)' 2.0 0.0001
    _assert_close `=r(corrected)' 2.75 0.001
}
if _rc == 0 {
    display as result "  PASS: V10.2 Large cell counts preserve OR scale invariance"
    local ++pass_count
}
else {
