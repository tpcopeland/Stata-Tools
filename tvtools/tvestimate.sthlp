...
{phang2}{stata "set seed 12345":. set seed 12345}{p_end}
{phang2}{stata "set obs 1000":. set obs 1000}{p_end}
{phang2}{stata "gen age = 50 + rnormal(0, 10)":. gen age = 50 + rnormal(0, 10)}{p_end}
{phang2}{stata "gen sex = runiform() > 0.5":. gen sex = runiform() > 0.5}{p_end}
{phang2}{stata "gen confounder = rnormal()":. gen confounder = rnormal()}{p_end}
{phang2}{stata "gen pr_treat = invlogit(-1 + 0.02*age + 0.5*sex + 0.3*confounder)":. gen pr_treat = invlogit(-1 + 0.02*age + 0.5*sex + 0.3*confounder)}{p_end}
{phang2}{stata "gen treatment = runiform() < pr_treat":. gen treatment = runiform() < pr_treat}{p_end}
{phang2}{stata "gen outcome = 50 + 2*treatment + 0.5*age + 1*sex + 2*confounder + rnormal(0, 5)":. gen outcome = 50 + 2*treatment + 0.5*age + 1*sex + 2*confounder + rnormal(0, 5)}{p_end}

{pstd}Basic G-estimation{p_end}
{phang2}{stata "tvestimate outcome treatment, confounders(age sex confounder)":. tvestimate outcome treatment, confounders(age sex confounder)}{p_end}

{pstd}With robust standard errors{p_end}
{phang2}{stata "tvestimate outcome treatment, confounders(age sex confounder) robust":. tvestimate outcome treatment, confounders(age sex confounder) robust}{p_end}

{pstd}With bootstrap standard errors{p_end}
{phang2}{stata "tvestimate outcome treatment, confounders(age sex confounder) bootstrap reps(500)":. tvestimate outcome treatment, confounders(age sex confounder) bootstrap reps(500)}{p_end}

{pstd}With time-varying data and clustering{p_end}
{phang2}{cmd:. * After tvexpose creates time-varying dataset}{p_end}
{phang2}{stata "tvestimate outcome tv_exposure, confounders(age sex) cluster(id)":. tvestimate outcome tv_exposure, confounders(age sex) cluster(id)}{p_end}
