...
program define tvcalendar, rclass
    ...
    * Simplified approach: merge on date
    ...
    if "`startvar'" == "" & "`stopvar'" == "" {
        ...
        * Warn if product is large (cross creates N_master * N_periods rows)
        local cross_size = `n_master' * `n_periods'
        if `cross_size' > 10000000 {
            display as error "Range merge would create `=string(`cross_size', "%12.0fc")' rows (master * periods)."
            display as error "This may exhaust memory. Consider reducing master data or using point-in-time merge."
            exit 198
        }
        else if `cross_size' > 1000000 {
            display as text "  Note: cross join creates `=string(`cross_size', "%12.0fc")' rows before filtering"
        }
        ...
        * Handle multiple period matches per observation: keep earliest
        tempvar _mseq
        quietly bysort `_obs_id' (`startvar'): gen double `_mseq' = _n
        quietly count if `_mseq' > 1
        local n_multi = r(N)
        if `n_multi' > 0 {
            display as text "  Note: `n_multi' obs matched multiple periods; keeping earliest"
            quietly keep if `_mseq' == 1
        }
        quietly drop `_mseq'
