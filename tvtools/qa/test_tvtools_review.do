...
log using "../../_devkit/_testing/test_tvtools_review.log", replace name(_test)
...
* =========================================================================
* TEST 4: tvsensitivity (cilow option, E-value)
* =========================================================================

display as text _dup(70) "="
display as text "{bf:TEST 4: tvsensitivity}"
display as text _dup(70) "="

cap program drop tvsensitivity
run tvtools/tvsensitivity.ado

* Test 4a: Basic E-value
capture {
    tvsensitivity, rr(2.0)
    assert r(evalue) > 0
    assert abs(r(evalue) - (2.0 + sqrt(2.0 * 1.0))) < 0.001
    assert r(rr) == 2.0
}
if _rc == 0 {
    display as result "  PASS: tvsensitivity e-value"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvsensitivity e-value (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvsensitivity_evalue"
}

* Test 4b: E-value with cilow
capture {
    tvsensitivity, rr(2.0) cilow(1.5)
    assert r(evalue) > 0
    assert r(evalue_ci) > 0
    assert r(evalue_ci) < r(evalue)
}
if _rc == 0 {
    display as result "  PASS: tvsensitivity cilow"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvsensitivity cilow (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvsensitivity_cilow"
}

* Test 4c: E-value for protective effect (RR < 1)
capture {
    tvsensitivity, rr(0.5)
    assert r(evalue) > 0
}
if _rc == 0 {
    display as result "  PASS: tvsensitivity protective"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvsensitivity protective (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvsensitivity_protective"
}

* Test 4d: Bias analysis method
capture {
    tvsensitivity, rr(1.8) method(bias)
    assert r(rr) == 1.8
    assert "`r(method)'" == "bias"
}
if _rc == 0 {
    display as result "  PASS: tvsensitivity bias"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvsensitivity bias (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvsensitivity_bias"
}

* Test 4e: Invalid RR should error
capture noisily tvsensitivity, rr(-1)
if _rc == 198 {
    display as result "  PASS: tvsensitivity invalid RR error"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvsensitivity should error on negative RR (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvsensitivity_invalid"
}

* =========================================================================
* TEST 5: tvtable (if/in, marksample)
* =========================================================================

display as text _dup(70) "="
display as text "{bf:TEST 5: tvtable}"
display as text _dup(70) "="

* Create table test data
clear
set obs 500
gen byte exposure = floor(runiform() * 3)
gen byte outcome = (runiform() > 0.9)
gen double pt = 30 + floor(runiform() * 335)
gen byte subset = (_n <= 200)

cap program drop tvtable
run tvtools/tvtable.ado

* Test 5a: Basic
capture {
    tvtable, exposure(exposure)
    assert r(n_levels) == 3
    assert r(total_n) == 500
}
if _rc == 0 {
    display as result "  PASS: tvtable basic"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvtable basic (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvtable_basic"
}

* Test 5b: With outcome and persontime
capture {
    tvtable, exposure(exposure) outcome(outcome) persontime(pt)
    assert r(total_events) >= 0
    assert r(total_pt) > 0
}
if _rc == 0 {
    display as result "  PASS: tvtable with outcome"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvtable with outcome (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvtable_outcome"
}

* Test 5c: With if condition
capture {
    tvtable if subset == 1, exposure(exposure)
    assert r(total_n) <= 200
}
if _rc == 0 {
    display as result "  PASS: tvtable with if"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvtable with if (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvtable_if"
}

* Test 5d: With in range
capture {
    tvtable in 1/100, exposure(exposure)
    assert r(total_n) <= 100
}
if _rc == 0 {
    display as result "  PASS: tvtable with in"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvtable with in (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvtable_in"
}

* =========================================================================
* TEST 6: tvreport (if/in, marksample)
* =========================================================================

display as text _dup(70) "="
display as text "{bf:TEST 6: tvreport}"
display as text _dup(70) "="
...
gen double age = 50 + rnormal(0, 10)
gen byte subset = (id <= 50)

cap program drop tvreport
run tvtools/tvreport.ado

* Test 6a: Basic report
capture {
...
if _rc == 0 {
    display as result "  PASS: tvreport basic"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvreport basic (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvreport_basic"
}

* Test 6b: With if
capture {
    tvreport if subset == 1, id(id) start(start) stop(stop) exposure(tv_exposure)
    assert r(n_ids) <= 50
}
if _rc == 0 {
    display as result "  PASS: tvreport with if"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvreport with if (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvreport_if"
}

* Test 6c: With covariates and event
capture {
    tvreport, id(id) start(start) stop(stop) exposure(tv_exposure) ///
        covariates(age) event(_event)
    assert r(n_events) >= 0
}
if _rc == 0 {
    display as result "  PASS: tvreport with covariates and event"
    local n_pass = `n_pass' + 1
}
else {
    display as error "  FAIL: tvreport with covariates/event (rc=`=_rc')"
    local n_fail = `n_fail' + 1
    local failures "`failures' tvreport_covariates"
}

* =========================================================================
* TEST 7: tvcalendar (range merge, tempvar fix)
* =========================================================================
