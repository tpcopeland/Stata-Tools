...
capture log close
quietly log using "test_options.log", replace nomsg
...
display as result "tvtools QA: option coverage -- $S_DATE $S_TIME"


**# ===== merged from test_tvtools.do L8087-9620: TVEXPOSE option groups + tvmerge/tvevent/person-time/error/edge coverage =====
...
label define drug2_lbl 0 "Unexposed" 1 "Drug_A" 2 "Drug_B"
label values drug drug2_lbl
tempfile _combine_exp
save "`_combine_exp'"
restore
capture noisily tvexpose using "`_combine_exp'", ///
    id(id) start(rx_start) stop(rx_stop) ///
    exposure(drug) reference(0) ///
    entry(study_entry) exit(study_exit) ///
...
display as result _newline "tvtools QA option coverage Results -- $S_DATE $S_TIME"
