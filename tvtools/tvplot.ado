*! tvplot Version 1.0.1  2026/02/26
    ...
    **************************************************************************
    if "`swimlane'" != "" {
        _tvplot_swimlane, id(`id') start(`start') stop(`stop') ///
            ...
            scheme(`scheme') graphoptions(`graphoptions')
    ...
    **************************************************************************
    if "`persontime'" != "" {
        ...
        _tvplot_persontime, id(`id') start(`start') stop(`stop') ///
            ...
            `replace' colors(`colors') scheme(`scheme') ///
...
program define _tvplot_swimlane
    syntax , id(varname) start(varname) stop(varname) ///
        [exposure(varname) sample(integer 30) sortby(string) ///
         ...
         scheme(string) graphoptions(string asis)]

    preserve

    * Declare tempvars for this subprogram
    tempvar _sortval _days _first _ypos _exp_num

    * Determine sort order for ID selection
    if "`sortby'" == "" | "`sortby'" == "entry" {
        * Sort by earliest start date
        by `id': egen `_sortval' = min(`start')
    }
    else if "`sortby'" == "exit" {
        * Sort by latest stop date
        by `id': egen `_sortval' = max(`stop')
    }
    else if "`sortby'" == "persontime" {
        * Sort by total person-time
        gen double `_days' = `stop' - `start' + 1
        by `id': egen `_sortval' = total(`_days')
        drop `_days'
    }
    else {
        * Sort by specified variable
        ...
        by `id': egen `_sortval' = mean(`sortby')
    }

    * Keep unique IDs with sort value
    by `id': gen `_first' = (_n == 1)
    tempfile _full_data
    quietly save `_full_data'

    quietly keep if `_first' == 1
    gsort `_sortval' `id'
    ...
    * Create exposure numeric if labeled
    if "`exposure'" != "" {
        ...
        if _rc == 0 {
            ...
            encode `exposure', gen(`_exp_num')
            local exposure "`_exp_num'"
            quietly levelsof `exposure', local(exp_levels)
            local n_exposures: word count `exp_levels'
        }
    }
    ...
    * Build twoway rbar command for each exposure level
    ...
    foreach lev of local exp_levels {
        ...
        local rbar_cmds `"`rbar_cmds' (rbar `start' `stop' `_ypos' if `exposure' == `lev', horizontal barwidth(0.6) color(`color'))"'
    }
...
program define _tvplot_persontime
    ...
    * Calculate person-time by exposure
    tempvar _pdays _pyears
    quietly gen double `_pdays' = `stop' - `start' + 1
    quietly gen double `_pyears' = `_pdays' / 365.25

    * Collapse to get person-time by exposure
    quietly collapse (sum) person_years = `_pyears', by(`exposure')
