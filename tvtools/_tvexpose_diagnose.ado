*! _tvexpose_diagnose Version 1.7.1  2026/07/17
...
program define _tvexpose_check, rclass
    version 16.0
    syntax , [id(name) start(name) stop(name) VERBose]
    ...
    * Calculate coverage metrics per person
    tempvar period_days total_covered expected_days gap_ind
    ...
    * Calculate number of gaps
    quietly by `id' (`start'): gen double `gap_ind' = (`start' > `stop'[_n-1] + 1) if _n > 1 & `id' == `id'[_n-1]
    quietly by `id': egen double n_gaps = total(`gap_ind')
...
program define _tvexpose_overlaps, rclass
    version 16.0
    syntax , exposure(name) [id(name) start(name) stop(name) skip_main_var(integer 0) VERBose]
    ...
    * Identify overlapping periods (start before previous period ends)
    tempvar overlap
    quietly by `id' (`start'): gen double `overlap' = (`start' <= `stop'[_n-1]) if _n > 1 & `id' == `id'[_n-1]

    quietly keep if `overlap' == 1
    ...
    if _N > 0 {
        ...
        * Count unique IDs with overlaps
        tempvar first_overlap
        quietly by `id': gen double `first_overlap' = (_n == 1)
        quietly count if `first_overlap' == 1
        local n_ids = r(N)
    ...
    if "`exp_type'" != "continuous" {
        * Get collapse by variables
        ...
        quietly collapse (sum) cat_time = `period_length' (count) n_periods = `period_length', ///
            by(`collapse_by_vars')
        quietly gen double cat_pct = 100 * cat_time / `total_time'
...
program define _tvexpose_validate, rclass
    ...
    * Create comprehensive validation dataset with per-person metrics
    ...
    tempvar period_days exposed_val trans_ind gap_val first_exp_val last_exp_val
    quietly generate double `period_days' = `stop' - `start' + 1
    quietly by `id': egen double total_covered = total(`period_days')
    quietly by `id': generate double expected_days = study_exit[1] - study_entry[1] + 1
    quietly generate double pct_covered = 100 * total_covered / expected_days
    ...
    * Calculate exposed time
    quietly gen double `exposed_val' = (`exposure' != `reference')
    quietly generate double exp_days = `period_days' * `exposed_val'
    quietly by `id': egen double total_exposed_days = total(exp_days)
    quietly by `id': egen double n_periods = count(`id')

    * Calculate number of transitions
    quietly by `id' (`start'): gen double `trans_ind' = (`exposure' != `exposure'[_n-1]) if _n > 1 & `id' == `id'[_n-1]
    quietly by `id': egen double n_transitions = total(`trans_ind')

    * Calculate gaps
    quietly by `id': gen double `gap_val' = (`start' > `stop'[_n-1] + 1) if _n > 1 & `id' == `id'[_n-1]
    quietly by `id': egen double any_gaps = max(`gap_val')
    quietly by `id': egen double n_gaps = total(`gap_val')

    * First and last exposure dates
    quietly by `id': egen double `first_exp_val' = min(`start') if `exposed_val'
    quietly by `id': egen double `last_exp_val' = max(`stop') if `exposed_val'
    quietly by `id': egen double first_exposure = min(`first_exp_val')
    quietly by `id': egen double last_exposure = max(`last_exp_val')
