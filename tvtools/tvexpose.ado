...
Data handling and cleaning options:
  ...
  merge(#)             - Days within which to merge same-type periods (default: 120)
...
program define tvexpose, rclass
    ...
    * These parameters control time adjustments and should not be negative
    if `merge' <= 0 {
        noisily display as error "merge() must be positive (days)"
        exit 198
    }
    if `lag' < 0 {
...
* Check for invalid exposure records AFTER all date processing
    ...
    * Check if any exposures exist in the dataset
    quietly count
    if r(N) == 0 {
        noisily display as error "No exposure periods found in `using' for persons in master dataset"
        noisily display as error "Possible causes:"
        noisily display as error "  - No matching IDs between master and exposure datasets"
        noisily display as error "  - All exposure periods fall outside study entry/exit dates"
        noisily display as error "  - Exposure dataset is empty"
        exit 2000
    }
