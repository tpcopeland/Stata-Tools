*! tvexpose Version 1.10.2  2026/07/31
...
program define tvexpose, rclass
    ...
    * check/gaps/overlaps/summarize/validate are report-only: each one saves the
    * finished dataset, computes, prints, and reloads it. None of them feeds the
    * committed schema and none of their locals is read below, which is what let
    * the whole span move to _tvexpose_diagnostics.ado.
    *
    * It had to move. A Stata program may hold at most 3500 statements; loading
    * a longer one fails with r(1000) and leaves the command undefined, with no
    * message naming the cause. tvexpose.ado had two statements of headroom at
    * 1.9.1, so wrapping any span here in a single further `if' broke the whole
    * command at load time. qa/test_program_limits.do now measures the margin.
    if "`check'`gaps'`overlaps'`summarize'`validate'" != "" {
        _tvexpose_diagnostics , exptype("`exp_type'") totaltime(`total_time') ///
            skipmainvar(`skip_main_var') generate(`generate') ///
            stubname(`stub_name') reference(`reference') ///
            saveas(`"`saveas'"') ///
            `check' `gaps' `overlaps' `summarize' `validate' ///
            `verbose' `bytype' `replace'
    }
