    ...
    if "`bootstrap'" != "" {
        ...
        * IF for psi: (Y - psi*A)*(A - ps) / E[A*(A-ps)]
        ...
        if "`cluster'" != "" {
            ...
            * Robust (HC0 heteroskedasticity-consistent) SE
            * HC0: (1/N^2) * sum(IF_i^2) — does not assume homoskedasticity
            tempvar influence_sq
            quietly gen double `influence_sq' = `influence'^2 if `touse'
            quietly summarize `influence_sq' if `touse'
            local se_psi = sqrt(r(sum) / (`N'^2))
        }
        else {
            * Model-based SE (assuming homoskedasticity)
            * Uses sample variance of IF divided by N
