*! tvbalance Version 1.0.2  2026/02/26
...
program define tvbalance, rclass
    ...
    * Mark sample
    ...
    if "`weights'" != "" markout `touse' `weights'
    ...
    * Count observations by exposure
    quietly count if `exposure' == `ref_level' & `touse'
    local n_ref = r(N)
    quietly count if `exposure' != `ref_level' & `touse'
    local n_exp = r(N)
    ...
    * Calculate SMD for each covariate
    ...
    foreach var of local varlist {
        ...
        * Unweighted statistics
        quietly sum `var' if `exposure' == `ref_level' & `touse'
        ...
        * Weighted SMD if weights provided
        if "`weights'" != "" {
            quietly sum `var' [aw=`weights'] if `exposure' == `ref_level' & `touse'
    ...
    * Summary statistics
    ...
    if "`weights'" != "" {
        ...
        * Effective sample size
        tempvar w2_temp
        quietly sum `weights' if `exposure' == `ref_level' & `touse'
        local sum_w_ref = r(sum)

        quietly gen double `w2_temp' = `weights'^2 if `exposure' == `ref_level' & `touse'
        quietly sum `w2_temp'
        local sumsq_w_ref = r(sum)
        quietly drop `w2_temp'
