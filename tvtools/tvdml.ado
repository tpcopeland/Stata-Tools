...
program define tvdml, eclass
    ...
    * Create fold variable
    tempvar fold resid_y resid_d pscore y_hat_tmp d_hat_tmp
        ...
        if `has_lasso' & "`method'" == "lasso" {
            * Use Stata's lasso for outcome model
            quietly capture {
                ...
                predict double `y_hat_tmp' if `touse' & `fold' == `k'
                replace `resid_y' = `depvar' - `y_hat_tmp' if `touse' & `fold' == `k'
                drop `y_hat_tmp'
            }

            * Fall back to OLS if lasso failed for outcome model
            quietly count if `touse' & `fold' == `k' & missing(`resid_y')
            if r(N) > 0 {
                display as text "  {bf:Note:} Lasso failed for outcome in fold `k'; using OLS fallback"
                quietly regress `depvar' `covariates' if `touse' & `fold' != `k'
                quietly predict double `y_hat_tmp' if `touse' & `fold' == `k'
                quietly replace `resid_y' = `depvar' - `y_hat_tmp' if `touse' & `fold' == `k'
                drop `y_hat_tmp'
            }
            ...
            * Use lasso for propensity score
            quietly capture {
                ...
                predict double `d_hat_tmp' if `touse' & `fold' == `k', pr
                replace `pscore' = `d_hat_tmp' if `touse' & `fold' == `k'
                replace `resid_d' = `treatment' - `d_hat_tmp' if `touse' & `fold' == `k'
                drop `d_hat_tmp'
            }

            * Fall back to logit if lasso failed for treatment model
            quietly count if `touse' & `fold' == `k' & missing(`resid_d')
            if r(N) > 0 {
                display as text "  {bf:Note:} Lasso failed for treatment in fold `k'; using logit fallback"
                quietly logit `treatment' `covariates' if `touse' & `fold' != `k'
                quietly predict double `d_hat_tmp' if `touse' & `fold' == `k', pr
                quietly replace `pscore' = `d_hat_tmp' if `touse' & `fold' == `k'
                quietly replace `resid_d' = `treatment' - `d_hat_tmp' if `touse' & `fold' == `k'
                drop `d_hat_tmp'
            }
        }
        else {
