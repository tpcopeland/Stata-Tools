...
{pstd}{bf:6. Hand off to diagnostics and survival setup} using the names
{cmd:tvpipe} returns rather than names typed twice. Copy the returns into
locals first: each of these commands replaces {cmd:r()} with its own
results.{p_end}
...
{pstd}{bf:7. Recurring events from a separate frame.} Under {opt gaptime} the
analysis clock is the gap clock, so {cmd:stset} the gap-time bounds rather than
the calendar bounds. The defaults {cmd:_t0} and {cmd:_t} are already
{cmd:stset}'s own names, which is deliberate; give {opt gapstart()} and
{opt gapstop()} other names if you intend to {cmd:stset} on calendar time
instead.{p_end}
