...
to the graph command. Any valid twoway option may be specified, including
{opt note()}, {opt xsize()}, {opt ysize()}, and {opt plotregion()} among others.
...
{phang2}{stata "tvplot, id(id) start(start) stop(stop) exposure(tv_exposure) sample(50) sortby(persontime)":. tvplot, id(id) start(start) stop(stop) exposure(tv_exposure) sample(50) sortby(persontime)}{p_end}

{pstd}Person-time bar chart{p_end}
{phang2}{stata "tvplot, id(id) start(start) stop(stop) exposure(tv_exposure) persontime scheme(plotplainblind)":. tvplot, id(id) start(start) stop(stop) exposure(tv_exposure) persontime scheme(plotplainblind)}{p_end}
...
{phang2}{stata "tvplot, id(id) start(start) stop(stop) exposure(tv_exposure) colors(blue red green orange)":. tvplot, id(id) start(start) stop(stop) exposure(tv_exposure) colors(blue red green orange)}{p_end}
