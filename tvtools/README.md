...
4. **tvcalendar** - Merge calendar-time external factors (policy periods, seasons)
5. **tvage** - Generate time-varying age intervals for survival analysis

### Diagnostics and Visualization
6. **tvdiagnose** - Assess data quality with coverage, gap, and overlap diagnostics
7. **tvbalance** - Calculate standardized mean differences for covariate balance
8. **tvplot** - Visualize exposure patterns with swimlane and person-time plots
...
All examples use synthetic data from `_examples/`. Intermediate files are saved to `_examples/`.
...
tvexpose using _examples/tv_antidep_episodes.dta, ///
...
tvexpose using _examples/tv_antidep_episodes.dta, ///
    ...
    saveas(_examples/tv_antidep.dta) replace

* Step 2: Load events as master, integrate with TV data
use _examples/tv_events.dta, clear
tvevent using _examples/tv_antidep.dta, id(id) ///
...
use _examples/tv_antidep.dta, clear
rename tv_exposure drug_class
save _examples/tv_antidep.dta, replace

* Step 2: Create time-varying benzodiazepine dataset
use _examples/cohort.dta, clear
tvexpose using _examples/tv_benzo_episodes.dta, ///
    id(id) start(rx_start) stop(rx_stop) ///
    exposure(benzo_use) reference(0) ///
    entry(study_entry) exit(study_exit) ///
    saveas(_examples/tv_benzo.dta) replace

use _examples/tv_benzo.dta, clear
rename tv_exposure benzo
save _examples/tv_benzo.dta, replace

* Step 3: Merge the two time-varying datasets
tvmerge _examples/tv_antidep _examples/tv_benzo, id(id) ///
...
tvexpose using _examples/tv_antidep_episodes.dta, ///
    ...
    exposure(drug_class) reference(0) ///
    entry(study_entry) exit(study_exit) ///
    keepvars(index_age female) saveas(_examples/tv_antidep.dta) replace
...
tvexpose using _examples/tv_benzo_episodes.dta, ///
    ...
    exposure(benzo_use) reference(0) ///
    entry(study_entry) exit(study_exit) ///
    keepvars(education income_quintile) saveas(_examples/tv_benzo.dta) replace
...
tvexpose using _examples/tv_antidep_episodes.dta, ///
    ...
    exposure(drug_class) reference(0) ///
    entry(study_entry) exit(study_exit) ///
    saveas(_examples/tv_antidep_temp.dta) replace

use _examples/tv_events.dta, clear
tvevent using _examples/tv_antidep_temp.dta, id(id) ///
    date(cv_event_date) ///
    compete(death_date) ///
...
save _examples/tv_merged.dta, replace
use _examples/tv_events.dta, clear
tvevent using _examples/tv_merged.dta, id(id) ///
...
```stata
* Basic balance check after tvexpose
use _examples/cohort.dta, clear
tvexpose using _examples/tv_antidep_episodes.dta, ///
    id(id) start(rx_start) stop(rx_stop) ///
    exposure(drug_class) reference(0) ///
...
tvbalance index_age female education, exposure(tv_exposure)

* Balance with IPTW weights
tvweight tv_exposure, covariates(index_age female education)
tvbalance index_age female education, ///
    exposure(tv_exposure) weights(iptw)

* Generate Love plot with stricter threshold
tvbalance index_age female education, ///
    exposure(tv_exposure) weights(iptw) ///
    threshold(0.05) loveplot
```
...
```stata
* Basic swimlane plot of antidepressant exposure
tvplot, id(id) start(rx_start) stop(rx_stop) exposure(tv_exposure)
...
tvplot, id(id) start(rx_start) stop(rx_stop) exposure(tv_exposure) persontime

* Save plot with custom title
tvplot, id(id) start(rx_start) stop(rx_stop) exposure(tv_exposure) ///
    title("SSRI/SNRI Treatment Patterns") ///
    saving(tvtools/demo/treatment_patterns.png) replace
```
...
tvage, idvar(id) dobvar(birth_date) ///
    entryvar(study_entry) exitvar(study_exit) ///
    groupwidth(10) saveas(_examples/tv_age_data) replace noisily
```
...
tvage, idvar(id) dobvar(birth_date) ///
    entryvar(study_entry) exitvar(study_exit) ///
    groupwidth(5) saveas(_examples/tv_age.dta) replace
...
tvmerge _examples/tv_age _examples/tv_antidep, id(id) ///
...
tvbalance index_age female, exposure(drug_class) threshold(0.1)

* Step 7: Visualize exposure patterns
tvplot, id(id) start(start) stop(stop) ///
    exposure(drug_class) sample(50) sortby(persontime)
