...
| `tvpipe` | Build a committed interval frame from a cohort and its sources | `help tvpipe` |
...
`tvpipe` is the recommended path: it runs the construction half of the pipeline as one validated call, checks the entire plan before touching any data, and commits its destination frames as a single transaction. The primitives remain available and unchanged for anything `tvpipe` deliberately does not decide for you — an advanced `tvexpose` exposure definition, a bespoke overlap policy, or a non-standard alignment. Both paths call the same engines and produce the same result.
