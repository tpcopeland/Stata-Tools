...
- `tvbuild`: the recommended front door. Turns a person-level cohort plus one or more longitudinal sources into a committed interval frame in one validated, transactional call: a specification frame or a one-source inline shortcut, an exact read-only `dryrun`, strict or permissive coverage, an optional event stage, an optional provenance manifest, and a verified data signature. It coordinates the shared `tvexpose`, `tvmerge`, and `tvevent` engines rather than reimplementing interval semantics, and it makes no scientific decision for you.
