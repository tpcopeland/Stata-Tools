    ...
    * =========================================================================
    ...
    quietly {
        if "`model'" == "logit" {
            ...
            * Predict probability for each level using counter-based tempvar names
            * (avoids invalid local names when levels contain special characters)
            local k = 0
            foreach lev of local levels {
                local k = `k' + 1
                tempvar _ps_k`k'
                predict double `_ps_k`k'' if `touse', pr outcome(`lev')
                replace `generate' = 1 / `_ps_k`k'' if `exposure' == `lev' & `touse'
            }
            ...
            * Save denominator if requested (probability of observed treatment)
            if "`denominator'" != "" {
                ...
                foreach lev of local levels {
                    local k = `k' + 1
                    replace `denominator' = `_ps_k`k'' if `exposure' == `lev' & `touse'
                }
