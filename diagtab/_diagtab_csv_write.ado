*! _diagtab_csv_write Version 2.0.0  2026/08/19
*! Write visible table columns as CSV without Stata variable names
*! Author: Timothy P Copeland, Karolinska Institutet
*! Program class: nclass

program define _diagtab_csv_write, nclass
    version 17.0
    ...
    capture noisily {
        ...
        capture program list _diagtab_validate_path
        if _rc {
        ...
        _diagtab_validate_path `"`using'"' "csv()"
        ...
        * from the contents of row 1.
        if "`reservedrow'" != "" {
            tempvar _dt_filled
            quietly generate byte `_dt_filled' = 0
            foreach _v of local _vars {
                capture confirm string variable `_v'
                if !_rc quietly replace `_dt_filled' = 1 if strtrim(`_v') != ""
                else    quietly replace `_dt_filled' = 1 if !missing(`_v')
            }
            quietly count if `_dt_filled'
            if r(N) > 0 {
                tempvar _dt_rowid
                quietly generate long `_dt_rowid' = _n
                quietly summarize `_dt_rowid' if `_dt_filled', meanonly
                local _dt_first_filled = r(min)
                if `_dt_first_filled' > 1 quietly drop in 1/`=`_dt_first_filled' - 1'
            }
        }
