...
/*
DESCRIPTION:
    Common utility programs shared across the diagtab suite of table export
...
PROGRAMS INCLUDED:
    _diagtab_col_letter        - Convert column number to Excel letter (A, B, ..., Z, AA, AB, ...)
    _diagtab_validate_path     - Validate file path for dangerous characters
    _diagtab_validate_color    - Validate named/RGB color tokens for Excel formatting
    _diagtab_build_col_letters - Build list of Excel column letters for N columns
    _diagtab_open_file         - Open an xlsx file in the OS default application
    _diagtab_detect_vartype    - Auto-classify a variable as contn/conts/cat/bin
    _diagtab_validate_sheet    - Validate Excel sheet name (length, forbidden chars)
    _diagtab_apply_theme       - Apply journal-style formatting presets
    _diagtab_resolve_format    - Resolve font/fontsize/borderstyle from options, globals, and themes
    _diagtab_resolve_colors    - Resolve header/zebra colors from options and globals
    _diagtab_classify_stat      - Classify collect table statistics for formatting
    _diagtab_resolve_stat_format - Resolve per-statistic display formats
    _diagtab_collect_ci_level  - Read the CI level stored in an active collection
    _diagtab_strip_outer_quotes - Remove one balanced outer quote layer
    _diagtab_format_p          - Apply the package p-value display policy
    _diagtab_frame_put         - Store output in a named frame with optional replace
    _diagtab_frame_preflight   - Validate a frame spec without mutating anything
    _diagtab_resolve_ci_level  - Resolve the CI level to label intervals with
    _diagtab_helpers_ready     - Verify the helper bundle is fully loaded
    _diagtab_require_helpers   - Exit with package reinstall message if helpers are incomplete
USAGE:
    These programs are called internally by diagtab commands (table1_tc, regtab,
...
* =============================================================================
* _diagtab_col_letter: Convert column number to Excel letter reference
* =============================================================================
* Converts 1 -> A, 2 -> B, ..., 26 -> Z, 27 -> AA, 28 -> AB, etc.
* Returns result in c_local variable 'result'
*
* Usage: _diagtab_col_letter 3
*        local my_letter = "`result'"   // my_letter = "C"

capture program drop _diagtab_col_letter
program _diagtab_col_letter, nclass
    version 17.0
...
* =============================================================================
* _diagtab_validate_path: Validate file path for security
* =============================================================================
* Checks for dangerous characters that could enable command injection.
* Returns error code 198 if invalid characters found.
*
* Usage: _diagtab_validate_path "`filepath'" "xlsx()"
*        (exits with error if invalid)

capture program drop _diagtab_validate_path
program _diagtab_validate_path, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
* =============================================================================
* _diagtab_validate_color: Validate Excel color tokens
* =============================================================================
* Accepts supported Stata color names (e.g. navy) or RGB triplets (e.g. 200 220 240).
*
* Usage: _diagtab_validate_color "`color'" "headercolor()"

capture program drop _diagtab_validate_color
program _diagtab_validate_color, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
* Returns result in c_local variable 'result'
*
* Usage: _diagtab_build_col_letters 30
*        local letters = "`result'"   // letters = "A B C ... AA AB AC AD"

capture program drop _diagtab_build_col_letters
program _diagtab_build_col_letters, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
*
* Usage: _diagtab_open_file "`filepath'"

capture program drop _diagtab_open_file
program _diagtab_open_file, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
* =============================================================================
* _diagtab_detect_vartype: Auto-classify a variable for table1_tc
* =============================================================================
...
*      - p < 0.05  -> conts (skewed)
*
* Usage: _diagtab_detect_vartype myvar [if] [in]
*        local type "`result'"

capture program drop _diagtab_detect_vartype
program _diagtab_detect_vartype, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
...
* =============================================================================
* _diagtab_validate_sheet: Validate Excel sheet name
* =============================================================================
...
*
* Usage: _diagtab_validate_sheet "`sheet'" "sheet()"

capture program drop _diagtab_validate_sheet
program _diagtab_validate_sheet, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
program _diagtab_apply_theme, nclass
    ...
    if "`theme'" == "lancet" {
        ...
        * Custom theme reads from globals set by diagtab set theme custom
        c_local _theme_font = cond("$DIAGTAB_FONT" != "", "$DIAGTAB_FONT", "Arial")
        c_local _theme_fontsize = cond("$DIAGTAB_FONTSIZE" != "", "$DIAGTAB_FONTSIZE", "10")
        c_local _theme_border = cond("$DIAGTAB_BORDER" != "", "$DIAGTAB_BORDER", "thin")
        c_local _theme_headershade = cond("$DIAGTAB_HEADERCOLOR" != "", "1", "0")
        c_local _theme_headercolor "$DIAGTAB_HEADERCOLOR"
        c_local _theme_zebra = cond("$DIAGTAB_ZEBRACOLOR" != "", "1", "0")
    }
    else {
...
* =============================================================================
* _diagtab_resolve_format: Resolve font/fontsize/borderstyle from options,
*   globals, and themes
* =============================================================================
* Centralizes the format resolution logic shared across all diagtab commands.
* Resolves: user option -> theme -> $DIAGTAB_* global -> default.
*
* Sets c_local variables in the caller's scope:
*   _font, _fontsize, borderstyle, _hborder, headershade, zebra
*
* Usage: _diagtab_resolve_format, [theme(string) borderstyle(string)
*            headershade(string) zebra(string)]

capture program drop _diagtab_resolve_format
program _diagtab_resolve_format, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
    ...
    * Font defaults: global -> default
    local _font "Arial"
    local _fontsize 10
    if "$DIAGTAB_FONT" != "" local _font "$DIAGTAB_FONT"
    if "$DIAGTAB_FONTSIZE" != "" local _fontsize $DIAGTAB_FONTSIZE

    * Apply theme: explicit option overrides global
    if "`theme'" == "" & "$DIAGTAB_THEME" != "" local theme "$DIAGTAB_THEME"
    if "`theme'" != "" {
        _diagtab_apply_theme "`theme'"
    ...
    * Resolve borderstyle: global -> default
    if "`borderstyle'" == "" & "$DIAGTAB_BORDER" != "" local borderstyle "$DIAGTAB_BORDER"
...
*
* Usage: _diagtab_resolve_colors, [headercolor(string) zebracolor(string)]

capture program drop _diagtab_resolve_colors
program _diagtab_resolve_colors, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
    ...
    if "$DIAGTAB_HEADERCOLOR" != "" local _headercolor "$DIAGTAB_HEADERCOLOR"
    if "$DIAGTAB_ZEBRACOLOR" != "" local _zebracolor "$DIAGTAB_ZEBRACOLOR"
    if `"`headercolor'"' != "" local _headercolor `"`headercolor'"'
    if `"`zebracolor'"' != "" local _zebracolor `"`zebracolor'"'

    _diagtab_validate_color "`_headercolor'" "headercolor()"
    _diagtab_validate_color "`_zebracolor'" "zebracolor()"
...
capture program drop _diagtab_classify_stat
program _diagtab_classify_stat, rclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
capture program drop _diagtab_resolve_stat_format
program _diagtab_resolve_stat_format, rclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
* =============================================================================
* _diagtab_collect_ci_level: Read CI provenance from collect's saved JSON
*
...
capture program drop _diagtab_collect_ci_level
program _diagtab_collect_ci_level, rclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
* =============================================================================
* _diagtab_resolve_ci_level: decide the confidence level to LABEL intervals with
* =============================================================================
...
*
* Usage: _diagtab_resolve_ci_level `level'      // -1 when level() not given
*        local _ci_level = r(level)

capture program drop _diagtab_resolve_ci_level
program define _diagtab_resolve_ci_level, rclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
* =============================================================================
* _diagtab_strip_outer_quotes: Remove one balanced outer quoting layer only
* =============================================================================

capture program drop _diagtab_strip_outer_quotes
program _diagtab_strip_outer_quotes, rclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
    syntax , TEXT(string asis)
    mata: st_local("_out", _dt_strip_outer_quotes(st_local("text")))
...
* =============================================================================
* _diagtab_format_p: Shared p-value rendering without downward truncation
* =============================================================================

capture program drop _diagtab_format_p
program _diagtab_format_p, rclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
* =============================================================================
* _diagtab_console_display: Format and display a table dataset with proper
*   column widths
* =============================================================================
...
capture program drop _diagtab_console_display
program _diagtab_console_display, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
* =============================================================================
* _diagtab_helpers_ready: Verify helper bundle completeness
* =============================================================================
* Returns rc=0 when all requested helpers are loaded; rc=111 otherwise.
*
* Usage: capture _diagtab_helpers_ready

capture program drop _diagtab_helpers_ready
program _diagtab_helpers_ready, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
    ...
    if `"`required'"' == "" {
        local required "_diagtab_col_letter _diagtab_validate_path _diagtab_validate_color _diagtab_build_col_letters _diagtab_open_file _diagtab_detect_vartype _diagtab_validate_sheet _diagtab_apply_theme _diagtab_resolve_format _diagtab_resolve_colors _diagtab_classify_stat _diagtab_resolve_stat_format _diagtab_collect_ci_level _diagtab_resolve_ci_level _diagtab_strip_outer_quotes _diagtab_format_p _diagtab_console_display _diagtab_frame_put _diagtab_frame_preflight _diagtab_visible_vars _diagtab_require_helpers"
    }
...
* =============================================================================
* _diagtab_require_helpers: Require helper bundle completeness
* =============================================================================
* Displays the caller-compatible reinstall message and exits rc=111 when helpers
* are not loaded completely.
*
* Usage: _diagtab_require_helpers [, required(string) failmessage(string)]

capture program drop _diagtab_require_helpers
program _diagtab_require_helpers, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
    ...
    if `"`failmessage'"' == "" {
        local failmessage "_diagtab_common.ado failed to load fully; reinstall diagtab"
    }

    capture _diagtab_helpers_ready `"`required'"'
    if _rc {
...
*
* Usage: _diagtab_frame_put `"`frame'"'
*        local frame "`_frame_name'"

* =============================================================================
* _diagtab_frame_preflight: validate a frame specification WITHOUT mutating
* =============================================================================
...
*
* Usage: _diagtab_frame_preflight `"`spec'"' "plotframe()"
*        local nm = r(name)
*        local rp = r(replace)
*
...
capture program drop _diagtab_frame_preflight
program _diagtab_frame_preflight, rclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
capture program drop _diagtab_frame_put
program _diagtab_frame_put, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
...
string scalar _dt_strip_outer_quotes(string scalar x)
{
    real scalar n
...
program define _diagtab_visible_vars, nclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
        ...
        c_local _diagtab_visible_vars "`_vars'"
    }
    local rc = _rc
    set varabbrev `_orig_varabbrev'
    if `rc' exit `rc'
