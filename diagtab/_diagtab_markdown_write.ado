*! _diagtab_markdown_write Version 2.0.0  2026/08/19
*! Write the current dataset as a GitHub-Flavored Markdown table
*! Author: Timothy P Copeland, Karolinska Institutet
*! Program class: rclass

program define _diagtab_markdown_write, rclass
    version 17.0
    ...
    capture noisily {
        ...
        if _rc {
            ...
            if _rc == 0 {
                ...
                noisily display as error "_diagtab_common.ado not found; reinstall diagtab"
                exit 111
            }
        }
        ...
        capture _diagtab_validate_path `"`using'"' "markdown()"
        if _rc exit _rc

        local _visible_opts ""
        if `"`labelvar'"' != "" local _visible_opts "labelvar(`labelvar')"
        _diagtab_visible_vars, `_visible_opts'
        local _vars `"`_diagtab_visible_vars'"'
        ...
        * become a heading.
        ...
        forvalues _j = 1/`_k' {
            ...
            if "`novarnames'" == "" & inrange(`headerstart', 1, _N) {
                mata: st_local("_h`_j'", _dt_md_cell("`_v'", `headerstart'))
            }
            ...
            mata: st_local("_h`_j'", _dt_md_escape(st_local("_h`_j'")))
        }
        ...
        if `_body_end' >= `datastart' {
            forvalues _i = `datastart'/`_body_end' {
                ...
                forvalues _j = 1/`_k' {
                    local _v : word `_j' of `_vars'
                    mata: st_local("_cell`_j'", _dt_md_cell("`_v'", `_i'))
                    if `"`_cell`_j''"' != "" local _row_has_text = 1
                    mata: st_local("_cell`_j'", _dt_md_escape(st_local("_cell`_j'")))
                }
                if `_row_has_text' {
                    file write `_fh' "|"
                    forvalues _j = 1/`_k' {
...
version 17.0
capture mata: mata drop _dt_md_escape()
capture mata: mata drop _dt_md_cell()

mata:
mata set matastrict on

string scalar _dt_md_escape(string scalar x)
{
    x = strtrim(x)
    x = subinstr(x, "\", "\\")
    x = subinstr(x, "|", "\|")
...
string scalar _dt_md_cell(string scalar v, real scalar i)
{
    real scalar j
    real scalar x
