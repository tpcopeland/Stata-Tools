...
Current observed evidence therefore covers all 627 checks in the table, assembled from the isolated all-suite attempt plus the post-fix reruns described above. It does not supersede `run_status_full.txt`, because only one uninterrupted `run_all.sh full` invocation is allowed to publish that receipt.
