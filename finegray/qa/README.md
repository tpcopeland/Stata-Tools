...
| `validation_cluster_recovery.do` | Zhou et al. (2012) positive-stable shared-frailty DGP with exact marginal truth `beta = alpha*tau`: validates the generator's Laplace transform, coefficient recovery under cluster-constant and matched-pair covariates, rejection of the conditional frailty coefficient as the estimand, the study's opposite clustered-SE directions across those designs, and a second dependence/competing-frailty setting. |
