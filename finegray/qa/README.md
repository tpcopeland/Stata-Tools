...
The resource-constrained final audit on 2026-07-27 observed **30 suites and 520/520 checks pass**, with no failures and no skips, plus **5/5** in the standalone `test_run_all_wrapper.sh` regression. The first 29 suites (514 checks) ran in one isolated `full` attempt; after the 100-dataset ZZF oracle generator showed an approximately hour-long trajectory at the machine's current contention, that attempt was stopped under the explicit no-intensive-work constraint and `crossval_nuisance.do` (6/6) was run separately. `crossval_finegray_zzf.do` (102 checks), its cache-dependent FG-02 shell gate, the three multi-hour Monte Carlo gates, and the standalone benchmark were deliberately not rerun. Accordingly, this audit does **not** claim a new completed `full`-lane PASS receipt.
...
Current observed evidence is 29 per-suite sentinels totaling 514/514 from the isolated 2026-07-27 attempt, plus `RESULT: crossval_nuisance tests=6 pass=6 fail=0` from the separate bounded follow-up. The 622-check table above is the complete lane inventory, not a claim that all 622 ran in this resource-constrained audit.
...
| `test_finegray_contracts.do` | Direct weight-stratum mapping oracle, factor post-estimation contracts, and high/singleton censoring-strata checks |
