...
|---|---|---|---|---|
| `finegray` | `test_finegray*`, optimizer/variance/bootstrap/ZZF/nuisance/`bstrata`/`tvc`/`tvc_bstrata` suites, the fence matrix, the `mi` lattice probe, and the two `test_finegray_adversarial_*` suites | Recovery, CIF, LT-SE, `bstrata` recovery, `tvc` recovery, and ZZF gate suites | `crossval_finegray*`, `crossval_bstrata`, `crossval_tvc`, `crossval_nuisance`, `crossval_predict_stcrreg` | Documentation examples, contracts, determinism, ties; adversarial cross-feature (`mi` x `bstrata()`, `mi` x `tvc()`, `tvc()` x `strata()` x `cluster()`), namespace, hostile-label, and degenerate-mass probes |
...
| Lane | Suites |
|---|---|
| `quick` | All curated functional/regression `test_*` suites except the two adversarial suites; includes the fence matrix, the `mi` lattice probe and the `tvc()` x `bstrata()` suite. |
