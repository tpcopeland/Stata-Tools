...
**As of 2026-08-05 the `full` lane is 32 suites / 638 checks** — the table above, which is the authoritative inventory. The growth since the 2026-07-28 audit is `test_finegray_v110` 53 -> 54, `test_finegray_postest` 22 -> 27, `test_finegray_fg07_options` 6 -> 8, plus `test_finegray_sthlp_render`'s 2 checks, whose Tests/Pass/Fail/Skip cells were missing from the table entirely and are now recorded. A single uninterrupted `./run_all.sh full --source-repo ~/Stata-Tools` passed all 32 suites with 0 failures and 0 skips, alongside the FG-02 fail-closed gate, the wrapper's own regression test, and the delayed-entry transfer gate (4 arms identical vs the pinned tree). That run published `run_status_full.txt`, and it is the receipt to read.
