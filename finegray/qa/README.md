...
The 2026-07-28 audit obtained a passing observed result for every suite in the then-current **31-suite / 627-check** `full` inventory, with no skipped checks: the final estimator/QA tree passed the 26-suite `core` lane; after a run-directory initialization defect was corrected, the three affected R-backed suites passed standalone (`crossval_cif.do` 2/2, `crossval_predict_phtest.do` 14/14, and `crossval_finegray.do` 55/55); and the behaviorally identical first `full` attempt had already passed `crossval_finegray_zzf.do` 102/102 and `crossval_nuisance.do` 6/6, followed by the FG-02 stale-oracle sabotage gate. That first wrapper attempt reported 28/31 because those three early R-backed files referenced the nonexistent Stata setting `c(processid)` while constructing their fresh output directories; they now use session-unique `tempfile` paths.

This is combined per-suite evidence, **not** a single post-fix `full` receipt. The 100-dataset ZZF oracle takes approximately 30 minutes on this machine and its estimator/QA path was unchanged by the run-directory correction, so it was not repeated merely to replace the wrapper summary. The bounded final-tree `quick` lane separately passed 19/19 suites. The three multi-hour Monte Carlo gates and the standalone scaling benchmark were not rerun in this audit.

The latest completed historical `full` receipt remains the 2026-07-23 **27-suite / 586-check** run in `run_all_status.txt` and `run_status_full.txt`. It predates the current 31-suite tree and cannot sign off the present source. It was also written by the former wrapper before the receipt-ordering repair, so the file does not itself record the FG-02 verdict. On the current wrapper, only `run_all.sh` can publish a receipt: it requires the Stata runner sentinel and, on `python`/`full`, the exact FG-02 PASS sentinel before writing the final verdict.
...
That audit's observed evidence covered all 627 checks in its 31-suite table, assembled from the isolated all-suite attempt plus the post-fix reruns described above. It did not supersede `run_status_full.txt`, because only one uninterrupted `run_all.sh full` invocation is allowed to publish that receipt.

**As of 2026-08-07 the `full` lane is 33 suites / 652 checks** — the table above, which is the authoritative inventory.

The last uninterrupted `full` receipt is the 2026-08-05 one, and it covers the **32-suite / 638-check** inventory that preceded `test_finegray_v120b.do`: a single `./run_all.sh full --source-repo ~/Stata-Tools` passed all 32 suites with 0 failures and 0 skips, alongside the FG-02 fail-closed gate, the wrapper's own regression test, and the delayed-entry transfer gate (4 arms identical vs the pinned tree). That run published `run_status_full.txt`. The growth to that inventory since the 2026-07-28 audit was `test_finegray_v110` 53 -> 54, `test_finegray_postest` 22 -> 27, `test_finegray_fg07_options` 6 -> 8, plus `test_finegray_sthlp_render`'s 2 checks, whose Tests/Pass/Fail/Skip cells were missing from the table entirely and are now recorded.

`test_finegray_v120b.do` (33rd suite, 14 checks) was added on 2026-08-07 for the 1.2.0 presentation pass. Twelve of its 14 checks were observed to FAIL against the pre-fix tree; the other two are no-regression guards that pass on both.

On 2026-08-07 the whole 33-suite inventory passed on the 1.2.0 tree, **as two lane invocations rather than one `full` run**: `core` 28/28 suites and `python` 5/5 suites (`crossval_cif` 2/2, `crossval_predict_phtest` 14/14, `crossval_finegray` 55/55, `crossval_finegray_zzf` 102/102, `crossval_nuisance` 6/6), 0 failures and 0 skips in both. Each lane ran from its own scratch copy of the repo layout, so neither could overwrite the other's logs. **There is no single `full` receipt for the presentation pass**, and the three multi-hour ZZF Monte Carlo gates (their own lane, never part of `full`) were not rerun.
