...
*
* THE DGPs AND THE PREREGISTRATION.  The arms below are the same ZZF sec. 4.1
* arms Gate Z2 uses, and the signed expectations for their controls come from
* qa/validation_finegray_zzf_prereg_r.R -- an independent R probe, run before
* the gated repetitions, whose output is quoted verbatim in the Z2-PREREG
* header of validation_finegray_zzf_recovery.do.  Re-derive it with
*     cd finegray/qa && Rscript validation_finegray_zzf_prereg_r.R
* It is not a lane member by design: a preregistration that is regenerated by
* the run it constrains is not a preregistration.
*
...
* THE GATE COVERS BOTH ARMS THE RECEIPT CLAIMS (fixed 2026-09-02).
*
* run_status_gates.txt records this file's verdict as: "fixed_weight_sandwich
* covers in every arm; nuisance_adjusted covers in every pooled-weight arm".
* The gate itself was `gate_pass = win_rob' -- the first half only.  A
* nuisance_adjusted coverage regression printed the error line above ("it must
* not become the default") and then passed the suite, exited 0, and left the
* receipt asserting a property the run had just observed to be false.
*
* Both halves are now in the verdict.  win_nui is not vacuous: the stratified
* truncstrata() arms are marked ok by contract (ZZF Appendix E gives no
* nuisance cell there), but every POOLED-weight arm is evaluated exactly as the
* other two candidates are, and a missing or unusable arm scores 0.
local gate_pass = (`win_rob' & `win_nui')
local gate_fail = !`gate_pass'
if !`win_nui' & `win_rob' {
    display as error _newline "RESULT: FAIL -- nuisance_adjusted does not cover in every pooled-weight arm."
    display as error "  The shipped fixed_weight_sandwich still covers, but the gates receipt claims"
    display as error "  BOTH; do not record this run as a passing Gate Z-inference."
}
