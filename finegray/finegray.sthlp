...
{pstd}
{bf:Cross-validation against other implementations:} {cmd:finegray} is systematically validated against three independent implementations: Stata's {cmd:stcrreg}, R's {cmd:cmprsk::crr}, and R's {cmd:fastcmprsk::fastCrr}. The cross-validation suite (68 tests) covers coefficients, standard errors, log-likelihoods, cumulative incidence functions, baseline hazards, stratified censoring, and post-estimation predictions (xb, CIF, and Schoenfeld residuals) across real and simulated datasets.
...
{pstd}
{bf:Post-estimation predictions vs {help stcrreg}:} The {helpb finegray_predict} outputs are cross-validated against {cmd:stcrreg}'s native predictions and agree to numerical precision: {opt xb} equals {cmd:stcrreg}'s {cmd:predict, xb}, the baseline CIF (covariates at 0) equals {cmd:predict, basecif}, and {cmd:e(basehaz)} equals {cmd:stcrreg}'s cumulative-subhazard analogue (H0(t) = -ln(1 - {cmd:basecif})) at each distinct event time. The per-observation {opt cif} is the covariate-adjusted CIF 1 - exp(-H0(t)*exp(z'beta)); {cmd:stcrreg} produces this quantity through {cmd:stcurve, cif at()} rather than {cmd:predict} (its {cmd:predict} offers only the baseline {cmd:basecif} and the relative subhazard), and {cmd:finegray_predict, cif} reproduces it to numerical precision. {opt schoenfeld} residuals match {cmd:stcrreg}'s {cmd:predict, schoenfeld} exactly at untied cause-event times. At a {bf:tied} cause-event time the two implementations partition the residual among the simultaneous events differently, so an individual residual at a tied time can differ; the {bf:sum} of the residuals within each event time — and hence the overall score — is identical. See {helpb finegray_predict} for the per-prediction detail.
