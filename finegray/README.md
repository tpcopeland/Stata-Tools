...
The comprehensive demo script (`finegray/demo/demo_finegray.do`) loads the local package, exercises the current 1.2.1 public workflows, verifies exported CIF data, and refreshes the graph below. Run it from the Stata-Tools repository root with `stata-mp -b do finegray/demo/demo_finegray.do`.
...
- **1.2.1** (2026-07-28; Pending SSC release): Correctness, performance, and QA-methodology maintenance.
...
- **1.2.0** (2026-07-27; Pending SSC release): Left-truncation estimator, robust-SE finite-sample adjustment, the opt-in `nuisance` estimated-weight variance correction, opt-in baseline matrix, retired omnibus proportionality test, a hardened QA gate, and the reproducibility and reporting fixes below. First release since 1.0.0; the 1.1.x line was never published to SSC.
  ...
  - **Known in 1.2.0; fixed in 1.2.1:** a many-strata fit was dominated by rebuilding the beta-independent weight design (`_finegray_joint_setup`, `_finegray_A_at_times`, `_finegray_G_minus`) on every likelihood and score evaluation rather than once per fit. Note that the 100-joint-stratum ceiling still applies to delayed-entry fits only, so nothing bounds the stratum count on the right-censoring path.
