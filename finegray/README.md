...
`e(N_fail)`, `e(N_compete)`, `e(N_cens)`, `e(sum_w)` (only with weights), `e(ll)`, `e(ll_0)`, `e(chi2)`, `e(p)`, `e(df_m)`, `e(rank)`, `e(N_clust)`, `e(converged)`, `e(N_delayed)`, `e(N_G_trunc)`, `e(k_bstrata)`, `e(n_intervals)`, `e(k_tvc)`, `e(level)`, `e(cause)`, `e(censvalue)`, `e(iterate)`, `e(tolerance)`, `e(N_weight_strata)`, `e(min_weight_prob)`, `e(max_lt_weight)`, `e(N_prob_warn)`, `e(N_weight_warn)`, and `e(wsig_n)` (only with weights).

Posted macros are `e(cmd)`, `e(cmdline)`, `e(refitcmd)`, `e(predict)`, `e(compete)`, `e(compete_values)`, `e(designvars)`, `e(entryvar)`, `e(idvar)`, `e(mi_data)`, `e(postest)`, `e(fvvarlist)`, `e(fvsemantic)`, `e(strata)`, `e(truncstrata)`, `e(bstrata)`, `e(bstrata_noevent)`, `e(bstrata_noevent_x)`, `e(tvc)`, `e(tsplit)`, `e(tvc_covariates)`, `e(tvc_pos)`, `e(tsplit_nfail)`, `e(clustvar)`, `e(wtype)` and `e(wexp)` (only with weights), `e(lt_weight)`, `e(lt_vce)`, `e(bh_seq)`, `e(bh_key)`, `e(weight_warn_strata)`, `e(wsig)` (only with weights), `e(vce)`, `e(vce_meat)`, `e(vce_adjust)`, `e(title)`, `e(marginsok)`, `e(datasignature)`, and `e(datasignaturevars)`.
...
- **1.3.0** (2026-09-04): Variance unification, composable option axes, and weighted estimation. New features: `mi estimate, cmdok:` compatibility; `bstrata(varname)` for the stratified baseline model of Zhou et al. (2011); `tvc(varlist)` with `tsplit(numlist)` for piecewise-constant time-varying effects; `finegray_cif, over(varname)` for grouped CIF curves; `[pweight=]` and `[fweight=]` for sampling and frequency weights (see the corrected pweight scope in 1.3.1); and native `margins`/`contrast`/`pwcompare` support for factor terms. The three right-censoring option pairs (`nuisance`, `bstrata()`, `tvc()`) compose freely, and `nuisance` is now accepted on delayed-entry fits with the pooled weight (Zhang, Zhang & Fine 2011 Appendix B). Methods, formulas, grounding and refusal rationales are split into the new shipped help topic `finegray_methods.sthlp`. Worked examples expanded with `webuse hiv_si` and `webuse pneumonia`. Without the new options, `e(b)`, `e(V)`, `e(ll)` and `e(basehaz)` are bit-identical to 1.2.0. Breaking change: `e(covariates)` renamed to `e(designvars)`; results saved by an earlier build are refused with `r(301)`. Pre-release review fixes: `bootstrap()` is refused (`r(198)`) after an `fweight` fit, because `bsample` resamples rows rather than the replicated subjects the frequency weights stand for (the analytic `ci` is exact there); data in which a subject's failure record is not its last record (a second failure, or follow-up continued past it, as `stset ..., exit(time .)` permits) are refused (`r(198)`) instead of being silently reduced to each subject's last record; `bootstrap()` now restores the caller's random-number state; duplicate values in `finegray_cif, attime()`/`timepoints()` are collapsed instead of printing duplicate rows; and `finegray_phtest` widens its variable column so long design names are no longer abbreviated into collisions. A second review pass added: after a weighted fit, `finegray_cif`'s default covariate profile is now the **weighted** estimation-sample mean (and a factor indicator's weighted sample proportion) taken under the fit's own `[pweight=]`/`[fweight=]` column, so an `fweight` fit reports the same default curve as the fit of the `expand`ed data — it previously used the unweighted means, a profile the fit had never seen, at `rc 0`; `finegray_cif, over()` naming a package-owned design column of a factor fit no longer prints that column on the shared `at:` line as though it had been held fixed; `finegray, coeflegend` is accepted on replay and prints each coefficient's `_b[]` name (it was refused `r(198)`); a negative or non-numeric `attime()`/`timepoints()` gets this command's own message at `r(198)` instead of Stata's raw numlist complaint at `r(125)`; and `finegray_predict, schoenfeld` labels each residual with the same term spelling `finegray_phtest` uses (`2.grp#c.z`, not `2.grp#z`). Documentation: the help-file bootstrap and `stcrreg` comparison examples now carry their own `webuse hypoxia` setup (they followed a `webuse pneumonia` block and failed `r(111)` as printed), the delayed-entry example is a runnable self-contained block rather than a copyable schematic, the composability of `bstrata()`, `tvc()` and `nuisance` is stated correctly in the README, and `r(se_method)` is documented among `finegray_cif`'s stored results. A third review pass added: the baseline cache carries an identity, `e(bh_key)`, so a later fit cannot answer from an earlier fit's cached baseline; a weighted fit records a per-observation weight digest in `e(wsig)`/`e(wsig_n)`, keyed by the fit's `e(idvar)`, and post-estimation that rebuilds a different weight column — a scalar the weight expression reads changed after the fit, a re-sort under a subscript, or an exchange of two subjects' weights that leaves `e(sum_w)` unmoved — is refused with `r(459)` instead of answering from weights the fit never saw, and a fit whose `id()` variable is no longer in the data is refused by name because the digest cannot be rebuilt without it; every `_fg_*` design column and `_fg_entry` carries a per-run ownership mark, so a variable of yours spelled like one of them is refused with `r(198)` rather than silently dropped and replaced, and the whole adjudication runs before any change to the data, so a refusal leaves the dataset exactly as it was (a dataset fitted by the previous build carries no mark, so its columns are refused rather than dropped — drop them by hand or re-fit); baseline strata and `over()` levels are transported exactly (`r(levels_mat)`, `e(bstrata_noevent_x)`, and `bstratum()` accepting exact noninteger values), so a degenerate no-event stratum is identified by value rather than by its printed rounding; `basecshazard` answers on new data carrying only the time variable (and the strata variable under `bstrata()`), because the baseline reads no covariate design; `e(vce_adjust)`/`r(vce_adjust)` disclose the finite-sample factor as a coefficient-variance convention; within-subject constancy is judged exactly while interval adjacency is judged relatively — two records are contiguous when |`_t0` − previous `_t`| is at most 1e-12 times the larger boundary — so both rules mean the same thing at every time scale; and the printed `Variance:` line names both pieces of the sandwich when `cluster()` and `nuisance` are combined. A fourth review pass applied the fail-open contracts: `finegray_predict`'s cumulative-incidence paths now count their usable outputs before committing a variable, so a prediction that can be computed for no observation is refused with `r(2000)` instead of creating a column of missing values -- `cif` when a covariate in the fit is missing on every prediction row, and `cif ci` when the complementary log-log limits are undefined everywhere (a cumulative incidence that is identically zero at the requested horizon). Both refusals drop the variables the call had already created, leaving the data as they were. The subject-level reduction in `finegray` counts the subjects it keeps before it narrows the estimation sample rather than after. The new internal helper `_finegray_assert_cardinality.ado` is where that count is taken and is shipped with the package. A fifth review pass (2026-09-04 error report, FG-A01/FG-A02): `finegray_cif, attime()`/`timepoints()` horizons are used exactly as typed -- they were rewritten through `numlist`'s nine-significant-digit text, so a horizon at a cause event at .1000000000000001 became .1, reported CIF 0 with a note that it preceded the first event, and two horizons that round alike collapsed to one row; a covariate profile at which exp(xb) exceeds double precision is evaluated to its limit (CIF 1, SE 0, no limits, with a note) in `finegray_cif` (analytic, bootstrap and `tvc()` paths) and in `finegray_predict, cif` (which also returns 0 rather than missing before the first cause event), where `finegray_cif` previously posted a missing CIF with a fabricated zero SE at `rc 0`; a missing influence contribution now gives a missing SE instead of being summed as zero; and in the piecewise CIF an interval whose term overflowed is no longer dropped from the cumulative subhazard.
