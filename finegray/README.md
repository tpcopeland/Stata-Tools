...
Posted macros are `e(cmd)`, `e(cmdline)`, `e(refitcmd)`, `e(predict)`, `e(compete)`, `e(compete_values)`, `e(designvars)`, `e(entryvar)`, `e(idvar)`, `e(mi_data)`, `e(postest)`, `e(fvvarlist)`, `e(fvsemantic)`, `e(strata)`, `e(truncstrata)`, `e(bstrata)`, `e(bstrata_noevent)`, `e(bstrata_noevent_x)`, `e(tvc)`, `e(tsplit)`, `e(tvc_covariates)`, `e(tvc_pos)`, `e(tsplit_nfail)`, `e(clustvar)`, `e(wtype)` and `e(wexp)` (only with weights), `e(lt_weight)`, `e(lt_norm)` (stratified delayed-entry fits only), `e(lt_vce)`, `e(bh_seq)`, `e(bh_key)`, `e(weight_warn_strata)`, `e(wsig)` (only with weights), `e(vce)`, `e(vce_meat)`, `e(vce_adjust)`, `e(title)`, `e(marginsok)`, `e(datasignature)`, and `e(datasignaturevars)`.
...
- **1.3.5** (2026-09-13): Two corrections to the delayed-entry weights, found by an independent audit against the published Zhang, Zhang and Fine (2011) equations and `survival::finegray`. (1) **Event/censoring ties.** The censoring product-limit on a delayed-entry fit kept subjects failing at time *t* in the risk set of a censoring at *t*; Geskus's (2011, pp. 40-41) events-then-censorings-then-entries ordering removes them first, and only under that ordering does the product `G(t-)H(t-)` the engine carries equal the published `b(t)/S(t-)` weight. On a 401-subject fixture with five such ties the converged coefficient was -0.0603 against -0.0615 from the published weight and from R. The delayed-entry censoring KM now follows Geskus's ordering; the engine reproduces the dense published-weight coefficient to 1e-13 on tied data (`qa/test_finegray_v135.do`, `qa/crossval_finegray_zzf_ties.do`). Right-censored fits keep the `stcrreg` tie convention bit for bit. (2) **Observation gaps.** When the first entrant of a censoring stratum (or of the pooled sample) was censored before the next entrant arrived, its censoring drove that product-limit to zero and every later weight onto the 1e-10 floor, erasing the censoring weighting at `rc 0` with `e(converged) = 1` (a 300-subject fixture moved b(z2) from -0.101 to -0.084 on the pooled path and b(z1) from -0.229 to +0.002 under `strata()==truncstrata()`). Each product-limit is now estimated on the identifiable region of its own sample -- subjects observed before that sample's last empty risk set enter no weight estimate (He and Yang 1998, Theorem 2.2) -- which reproduces the published `b_g/S_g` form wherever it is finite; `e(N_lt_prehole)` and the fit-time note are now posted on the pooled path too (the nuisance-adjusted variance across such a gap equals the one on the identifiable sample exactly, and is accepted). Fits with no delayed entry, and delayed-entry fits with neither event/censoring ties nor a gap, are bit-identical to 1.3.4. Also: `e(refitcmd)` carries the fit-time factor expansion (`1b.grp 2.grp`) rather than a dynamic base such as `ib(freq).grp`, so bootstrap refits after a resample -- or after a later `fvset base` -- keep the point estimate's design (previously 10 of 25 replicates were rejected and the bootstrap failed `r(498)`); a weight expression containing a string literal (`[pw = cond(g == "A", 2, 1)]`) no longer fails the post-fit display with `r(198)`; `finegray_cif, bootstrap()` after `estimates use` over data saved before the fit succeeds (rebuilt design columns are kept out of the refit dataset; previously 0 of 25 replicates); `finegray_predict, xb` computable for no observation is refused with `r(2000)` like `cif`, instead of creating an all-missing column.
- **1.3.4** (2026-09-13): The stratified delayed-entry normalizer is estimated on the identifiable region. When a truncation stratum's risk set is empty at some entry time -- every earlier entrant already exited when the next arrived -- the entry product-limit is zero for the subjects observed before that gap and the truncation probability is not identified across it (He and Yang 1998, Theorem 2.2). 1.3.3 zeroed the whole stratum's normalizer whenever any member had `H(X_i-) = 0` and refused the fit with `r(459)` even when no weight ever consulted that subject; the shipped estimator therefore rejected 1 in 1000 replications of `qa/validation_finegray_zzf_coverage.do` arm `ts_mod_n2000` on data the published `b_g/S_g` form handles. The normalizer now sums over the members with `H(X_i-) > 0` and divides by the full observed cell size, which reproduces the published form wherever it is finite; the excluded subjects stay in `e(sample)`, are counted in the new `e(N_lt_prehole)` and a fit-time note, and the fit is refused only if a weight consults one (a cause event inside the gap, or a retained competing exit before it closes), with an `r(459)` message that names the mechanism, the count and the strata. Fits with no such gap are bit-identical to 1.3.3. See `help finegray_methods##lt`. QA: `qa/validation_finegray_zzf_factorization.do` check 2a now asserts that the JOINT and MARGINAL analytic standard errors are each calibrated against their Monte Carlo standard deviations, instead of asserting `JOINT > MARGINAL`, a direction the 1.3.3 stratified variance correction showed to be a property of the old estimator's inefficiency rather than of the design.
- **1.3.3** (2026-09-12): Corrects the stratified delayed-entry weight (`strata()`/`truncstrata()` with delayed entry): the subject-side denominator now carries the per-stratum normalizer that Zhang, Zhang and Fine's (2011, eq. 6) b_g/S_g form implies and 1.3.0-1.3.2 omitted. Such fits change and should be refitted; right-censored and pooled delayed-entry fits are bit-identical. See `help finegray_methods##lt`. Corrects the `nuisance` psi term under `strata()`: the cause-event sum in Fine & Gray's eq. 8 now runs over every cause event, not only those from the retained subject's own censoring stratum, so a stratum with competing events but no cause events contributes its censoring-estimation uncertainty (the previous form matched `cmprsk::crr(cengroup=)`, whose `crrvv` makes the same omission; multi-stratum `nuisance` variances move by ~1e-3 relative, single-stratum and `strata()==bstrata()` fits are bit-identical; checked against a numerical derivative of the fitted score in `qa/validation_nuisance_strata_numeric.do`). `finegray_cif` no longer draws a flat tail to the end of follow-up for a `timepoints()` grid that ends before the last cause-event time — the plotted curve stops at the last requested time; `r(table)` and `saving()` were unaffected.
- **1.3.2** (2026-09-08): ***Current SSC Release***. `stset` without `id()` is accepted. Each record is then one subject -- the meaning `stset` itself and `stcrreg` give an `id()`-less declaration, and the only shape such data can have, since `stsplit` and (start,stop] records require `id()` -- and the fit is bit-identical to the same data declared with an `id()` (pinned in `qa/test_finegray_noid.do` for the plain, delayed-entry, `cluster()`, `pweight`, post-estimation and bootstrap paths). `e(idvar)` is empty on such a fit; the weight digest `e(wsig)` is then keyed by value alone, and a changed weight is still refused with `r(459)`. The former refusal "finegray requires stset with id() variable" fired on data where `id()` carried no information.
- **1.3.1** (2026-09-08): Corrected the pweight scope: the unweighted analysis-sample censoring KM does not support general outcome-dependent case-cohort inference, and the sandwich treats censoring weights as fixed. Restored estimates use their own entry-time metadata in postestimation; multiple-record fits predict on new data without an unused entry column when the baseline is available; piecewise predictions preserve exact numeric baseline-stratum keys. Clarified fixed-weight inference and the scope of the delayed-entry literature. QA uses frozen, checksum-checked R references without routine refitting.
- **1.3.0** (2026-09-04): Variance unification, composable option axes, and weighted estimation. New: `mi estimate, cmdok:` support; `bstrata(varname)` for a stratified baseline subhazard (Zhou et al. 2011); `tvc(varlist)` with `tsplit(numlist)` for piecewise-constant time-varying effects; `finegray_cif, over(varname)`; `[pweight=]` and `[fweight=]` (scope corrected in 1.3.1); native `margins`/`contrast`/`pwcompare` for factor terms. `nuisance`, `bstrata()` and `tvc()` compose freely, and `nuisance` is accepted on delayed-entry fits (Zhang, Zhang & Fine 2011, Appendix B). Methods, formulas and refusal rationales moved to the shipped help topic `finegray_methods`. Breaking: `e(covariates)` renamed `e(designvars)`; estimates saved by an earlier build are refused with `r(301)`. Without the new options, `e(b)`, `e(V)`, `e(ll)` and `e(basehaz)` are bit-identical to 1.2.0. Pre-release review passes added fail-closed refusals (`bootstrap()` after an `fweight` fit, a failure record that is not a subject's last, a prediction computable for no observation, a user variable spelled like a package-owned `_fg_*` column), a per-observation weight digest `e(wsig)` and baseline-cache identity `e(bh_key)` that post-estimation reconciles against, weighted default profiles in `finegray_cif` after a weighted fit, exact transport of strata levels and horizon values, and overflow-safe CIF evaluation.
...
- **1.0.0** (2026-04-06): ***First SSC Release***. Initial Stata-Tools release of `finegray`, `finegray_predict`, and `finegray_phtest`.
