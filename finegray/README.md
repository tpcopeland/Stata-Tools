...
- **1.2.0** (2026-07-25; Pending SSC release): Left-truncation estimator, robust-SE finite-sample adjustment, the opt-in `nuisance` estimated-weight variance correction, opt-in baseline matrix, retired omnibus proportionality test, a hardened QA gate, and the reproducibility and reporting fixes below. First release since 1.0.0; the 1.1.x line was never published to SSC.
  ...
  - **Documented that `finegray_phtest`'s diagonal scaling cannot change the reported correlation.** A correlation is invariant to multiplication by a positive constant, so the reported `rho` equals the unscaled Schoenfeld/time correlation. This means no reported quantity depends on the Grambsch–Therneau transformation, whose applicability to the subdistribution hazard has never been established here; the scaling affects only the residual values shown by `detail`.
  ...
  - **Known, not fixed:** a many-strata fit is still dominated by rebuilding the beta-independent weight design (`_finegray_joint_setup`, `_finegray_A_at_times`, `_finegray_G_minus`) on *every* likelihood and score evaluation rather than once per fit — about 2.2 s per Newton iteration at 200 censoring strata against 0.18 s at 2, so an n = 20,000 fit takes ~18 s at 200 strata against ~1 s at 2. Hoisting that computation out of the Newton loop is the fix and is not attempted here. Note also that the 100-joint-stratum ceiling applies to delayed-entry fits only, so nothing bounds the stratum count on the right-censoring path.
