...
The suite is driven by `qa/run_all.do` (`quick`, `core`, `python`, `full`, and `gates` lanes) and documented in `qa/README.md`; `qa/run_all.sh` converts its numeric result sentinel into a reliable shell/CI exit status. The `qa/` directory contains 26 executable suite files: 12 functional/regression files (including `test_documentation_examples.do`, which runs the README workflows and advertised baseline options), 8 validation files, 5 cross-validation files covering all four public commands, and a performance benchmark. The two Monte Carlo recovery/coverage gates run under the `gates` lane (hours, not minutes). A skipped or missing suite fails the runner — an external oracle that does not run is treated as an unrun check, not a pass.

The latest isolated `full` lane (2026-07-15) passed 23/23 suites and 533/533 checks with no failures or skips. The separate full-size delayed-entry recovery and coverage gates also passed with every planned replication retained; see `qa/README.md` for the arm-level results and exact commands.
...
- **1.2.2** (2026-07-15; Pending SSC release): Delayed-entry contract and messaging clarity; no change to any estimate.
  - `e(lt_weight)` now reports `zzf1_factorized` (instead of `zzf1_stratified`) when `strata()` and `truncstrata()` name *different* groupings — the factorized `A = G·H` package extension, valid only when entry and censoring are independent within each joint cell. `zzf1_stratified` is now reserved for the Zhang–Zhang–Fine construction proper (matching groupings). The fit also prints a one-line note when the factorized weight is used, so the extension is visible at fit time rather than only in the help file. Weights and estimates are unchanged.
  - The `G(t) truncated to 1e-10` note is no longer reprinted by `finegray_cif`/`finegray_predict`/`finegray_phtest` when they recompute the censoring survivor for the influence function; it now appears once, at the fit, where the data characteristic is the user's to act on.
  - The same note is also printed only **once per fit**, rather than once per censoring stratum and again on every Newton iteration and step halving by the pooled delayed-entry stabilizer — a stratified fit under `truncstrata()` previously opened with a dozen or more copies of it. The single note counts the truncated observations across all strata, so the reported total is unchanged (a 4-stratum fit that printed 13/13/17/14 now prints 57). The pooled recompute inside the likelihood is silent: its floor is separately reported by `e(min_weight_prob)`/`e(N_prob_warn)` and escalated to `r(459)` by the pre-fit positivity check. Coefficients, standard errors, and the log pseudo-likelihood are bit-identical.
  - **Fixed:** `finegray_predict ..., cif bootstrap()` no longer poisons the baseline cache. Each bootstrap refit overwrote the single-slot Mata baseline cache, so a subsequent `predict, cif` on **new** data (estimation sample dropped) failed with `r(459)`. The bootstrap now snapshots and restores the cache around its refits; the bootstrap SE is unchanged (it reads `e(basehaz)` per refit, never this cache). Guarded by `test_finegray_postest.do` (FG-B04b).
  - The demo installs `tc_schemes` softly and falls back to `s2color`, so `demo/demo_finegray.do` runs from a checkout that contains only `finegray/`.

- **1.2.1** (2026-07-15; Not released to SSC): Correctness and QA hardening for the delayed-entry release.
  - Replaced the symmetric per-stratum stabilizer used by 1.2.0 with the pooled time-side stabilizer and stratum-specific subject-denominator form in Zhang, Zhang & Fine (2011, eq. 7). Matching groupings reproduce the paper's stratified construction; different censoring and entry groupings use the documented factorized extension. The corrected contract now drives coefficients, robust SEs, baseline hazards, Schoenfeld diagnostics, CIFs, analytic CIF SEs, positivity checks, and weight diagnostics. Exact integer activity counts prevent an empty stratum's floating-point risk-sum residue from consulting an undefined denominator. One-stratum and no-delayed-entry paths retain their previous formulas; the latter remains bit-identical.
  - Fixed `finegray_cif, at()` so numerically equivalent factor levels such as `grp=1` and `grp=1.0` map to the same fitted level. Factor profiles now use the fit-time semantic expansion, so a long internal `_fg_*` name truncated before its level suffix cannot silently select the reference profile.
  - Hardened QA gates and the independent R oracle: smoke runs now exit nonzero, every runner pass requires a zero-failure evaluated sentinel, recovery and coverage gates require every planned fit, and oracle manifests require every arm and replication at the pinned full size. Clean checkouts regenerate their fixtures, and same-group plus genuinely cross-classified stratified fits are compared dataset by dataset.
  - Clarified that continuous covariate-dependent entry cannot be diagnosed automatically and remains outside the supported pooled-weight assumptions.

- **1.2.0** (2026-07-15; Not released to SSC): Left-truncation estimator, opt-in baseline, and a hardened QA gate. Supersedes the never-released 1.1.x line.
  - **Left truncation now uses the Geskus (2011) product-limit weight.** Under delayed entry the weight factor is `A(t−) = G(t−)·H(t−)`, reweighting the risk set for entry rather than applying the censoring weight alone. This is equivalent to Zhang–Zhang–Fine Weight 1 in the unstratified continuous-time setting. Version 1.2.0's multi-stratum stabilizer was subsequently corrected in 1.2.1 to match Zhang et al.'s published equation 7. Delayed-entry point estimates that were biased by tens to hundreds of Monte Carlo standard errors — see the retracted 1.1.1 note below — now recover the truth to within Monte Carlo error. With **no** delayed entry the estimator is bit-identical to before. `e(lt_weight)` and `e(lt_vce)` record the stored weight label and the variance actually computed; the default sandwich met the package's truncation coverage gate, while `norobust` undercovered and now warns.
...
- **1.1.4** (2026-07-10; Not released to SSC): Bootstrap and parsing robustness fixes.
...
- **1.1.0** (2026-06-21; Not released to SSC): Feature release.
  ...
  - Documentation clarification (from the unreleased 1.0.1): `finegray_predict, cif` evaluates the CIF at each observation's own analysis time `_t`; `timevar()` gives a common horizon, and the fitted baseline cumulative subhazard is the cumulative-hazard analogue of `stcrreg`'s `basecif`.
