...
`e(lt_weight)` reports which weight was actually used: `zzf1_geskus` for a one-stratum delayed-entry fit, `zzf1_stratified` for the equation-7 pooled-stabilizer form with multiple joint strata (including the factorized extension just described), and `right_censoring` when there is no delayed entry. `e(lt_vce)` records the variance, so consumers do not have to infer either contract from the option list. Weight diagnostics are stored in `e(N_weight_strata)`, `e(min_weight_prob)`, `e(max_lt_weight)`, `e(N_prob_warn)`, `e(N_weight_warn)` and `e(weight_warn_strata)`. Product-limit delayed-entry weights may legitimately exceed 1.
