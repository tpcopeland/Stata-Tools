...
| `graph`, `simlines()` | `finegray_gof` | Plot each tested process against `simlines()` null realizations (default 20); one graph per process, `fggof1`, `fggof2`, … Opt-in, and it cannot change a p-value |
| `saving()` | `finegray_gof` | Write the plotted paths as a dataset (`process`, `kind`, `x`, `observed`, `_fgsim1`…); `filename[, replace]` |
...
The suite is driven by `qa/run_all.do` (`quick`, `core`, `python`, `full`, and `gates` lanes) and documented in `qa/README.md`; `qa/run_all.sh` converts its numeric result sentinel into a reliable shell/CI exit status. The `qa/` directory contains 39 executable suite files: 16 functional/regression files (including `test_documentation_examples.do`, which runs the README workflows and advertised baseline options), 11 validation files, 7 cross-validation files covering all five public commands, and a performance benchmark. The five Monte Carlo gates — delayed-entry recovery, coverage, factorized cross-classification, and `finegray_gof` type-I calibration *and power* — run under the `gates` lane (hours, not minutes). The two `finegray_gof` gates are a pair: a test can be perfectly sized and still detect nothing, so calibration against Tables 1 and 4 is checked alongside power against Tables 2 and 3, the latter carrying both of the paper's alternative shapes because the omnibus test loses to a correctly specified time interaction under one and beats it under the other. A skipped or missing suite fails the runner — an external oracle that does not run is treated as an unrun check, not a pass.
...
- **1.2.0** (2026-07-20; Pending SSC release): Left-truncation estimator, robust-SE finite-sample adjustment, the opt-in `nuisance` estimated-weight variance correction, opt-in baseline matrix, retired omnibus proportionality test, and a hardened QA gate. First release since 1.0.0; the 1.1.x line was never published to SSC.
  ...
  - **`finegray_gof` can now plot the residual processes: `graph`, `simlines()`, `saving()` (2026-07-21).** The supremum statistic collapses a whole process to its largest excursion; the plot the paper describes (p.202) keeps the shape, and answers a question the p-value cannot — *where* and *how* the model fails. `graph` draws the observed process in black against `simlines()` realizations from its own null distribution (default 20), one graph per tested process, named `fggof1`, `fggof2`, …. Under the null the observed line is one grey line among many; a departure shows up as the observed line isolated outside the band over some stretch, which for a proportionality plot names *when* the effect stops being proportional and for a functional-form plot names *which range* of the covariate is fitted badly. `saving(filename[, replace])` writes the same paths as a dataset (`process`, `kind`, `x`, `observed`, `_fgsim1`…) for plotting elsewhere, with `finegray_cif`'s exact suboption and filename rules. Unlike `finegray_cif`, the graph is **opt-in**: here the table is the output, and a five-covariate fit would otherwise throw five uninvited graphs on screen. **Requesting a plot cannot change a p-value:** the display paths come from the same multiplier bootstrap but are drawn strictly *after* every replication the test consumes, so for a fixed `seed()` every stored result is bit-for-bit identical with and without `graph`. Interleaving them — the natural way to write it — would have silently re-seeded every subsequent replication and moved every p-value at `rc = 0`; regression test G21 pins this and is confirmed to fail against that mutation. For display only, a process is thinned to at most 2,000 grid points (the link process has one point per distinct linear predictor, which would otherwise exceed Stata's matrix limits); statistics are always computed on the full grid. `simlines()` without `graph` or `saving()` is an error rather than a silent no-op. New tests G21–G23; the dataset is built in a temporary frame, so data in memory is untouched.
  - **The displayed p-value floor now scales with `nsim()` (2026-07-21).** An observed p of exactly 0 means "below the 1/`nsim` resolution floor", not "zero", so `finegray_gof` prints `< 0.0010` at `nsim(1000)` rather than a bare `0.0000`. The floor itself was formatted `%6.4f`, which rounds to `0.0000` once `nsim` reaches 50,000 — so at large `nsim` the display read `< 0.0000`: a bare zero wearing a "<", asserting exactly the precision the branch exists to deny. Decimals now scale with `nsim` (`< 0.00002` at `nsim(50000)`), with a four-decimal minimum that leaves conventional output byte-identical. Found in review, not by a suite: test G8 pins this display *on the rendered log and with a negative control*, but asserts at `nsim(200)` where any format shows the floor — the right axis at the wrong magnitude. Regression test `test_finegray_gof.do` G20, confirmed to fail against the previous behavior while G8 continues to pass on it.
