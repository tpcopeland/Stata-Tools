*! covarclose Version 1.0.4  2026/02/28
...
program define covarclose, rclass
    ...
    * Save master with just ID and index date
    ...
    quietly {
        ...
        duplicates tag `idvar', gen(_covar_dup)
        count if _covar_dup > 0
        if r(N) > 0 {
            local n_dup_ids = r(N)
            noisily display as text "Note: `n_dup_ids' observations have duplicate IDs with potentially different index dates"
            noisily display as text "      Using first index date per ID"
        }
        drop _covar_dup
    ...
    * Verify all requested variables exist (outside quietly so errors display)
        ...
        if "`impute'" != "" {
            * Fill missing with values from adjacent observations
            foreach var of local vars {
                ...
                * Replace user-specified missing codes with system missing
                if "`missing'" != "" {
        ...
        * Apply preference for before/after/closest
        if "`prefer'" == "before" {
            * Prefer observations before or at index
            tempvar before_flag has_before
    ...
    * Merge back to original master
    ...
    quietly merge m:1 `idvar' using `covar_closest', keep(master match) nogen
