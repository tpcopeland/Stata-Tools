*! icdexpand Version 2.0.2  2026/02/24
...
program define icdexpand_match, rclass
    ...
    * Build clean pattern list (handle comma-separated and space-separated)
    local pattern = trim("`codes'")
    local pattern = subinstr("`pattern'", ",", " ", .)
    local pattern = trim(itrim("`pattern'"))
    local n_patterns = 0

    local remaining "`pattern'"
    while "`remaining'" != "" {
        gettoken thispattern remaining : remaining
        local thispattern = upper(trim("`thispattern'"))
        ...
        if "`thispattern'" != "" {
            ...
            * Determine pattern type and apply efficient matching
            ...
            if `has_range' & `has_wild' {
                ...
                * Extract range part (remove *) and parse with gettoken
                local range_part = subinstr("`thispattern'", "*", "", .)
                gettoken start range_rest : range_part, parse("-")
                gettoken hyphen end : range_rest, parse("-")
                local start = upper(trim("`start'"))
                local end = upper(trim("`end'"))
                ...
                * Range only (e.g., E10-E14)
                gettoken start range_rest : thispattern, parse("-")
                gettoken hyphen end : range_rest, parse("-")
                local start = upper(trim("`start'"))
                local end = upper(trim("`end'"))
                local prefixlen = length("`start'")
