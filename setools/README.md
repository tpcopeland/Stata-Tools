...
- **1.5.1** (2026-07-19): Bug fixes. `cdp` with `roving allevents` no longer aborts with `r(111)` when a confirmation pass finds no progression candidates — the modal case of a confirmed progressor with residual flat follow-up visits, a zero-progression cohort, and the shipped `cdp` roving example (`_data/relapses.dta`) now return cleanly. `migrations` long-format files no longer abort with `variable in_1 not found, r(111)` when every immigrated person's first observed event is an emigration (born-in-country, emigrated, returned), including the same-day in/out tie. Also: `migrations` gains a `quietly` option to suppress the processing summary (parity with the other commands); the wide-format date sweep and pre-merge keep are restricted to numeric-suffixed `in_#`/`out_#` slots so unrelated `income`/`outcome` columns are no longer force-validated as dates or carried into the merge; save-target path aliasing is now case-folded on macOS as well as Windows; and help files clarify the exit()-only stored results, exit-censored row retention, and same-day migration boundary semantics.
