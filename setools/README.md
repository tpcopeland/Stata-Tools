...
save _examples/cci.dta, replace
use _examples/cohort.dta, clear
merge 1:1 id using _examples/cci.dta, nogenerate keep(master match)
...
use _examples/cohort.dta, clear
dateparse window study_entry, lookback(365) generate(comorb_start comorb_end)

* Calculate 5-year follow-up window
dateparse window study_entry, followup(1826) generate(fu_start fu_end)

* Determine which outpatient files to load for 2006-2023 cohort
dateparse filerange, index_start("2006-01-01") index_end("2023-12-31") lookback(730)
...
use _examples/migrations.dta, clear
...
save _examples/migrations_wide.dta, replace

* Apply migration exclusions and get censoring dates
use _examples/cohort.dta, clear
rename study_entry study_start
migrations, migfile("_examples/migrations_wide.dta") verbose
...
use _examples/procedures.dta, clear
...
use _examples/relapses.dta, clear
...
save _examples/relapses_only.dta, replace

* Basic PIRA analysis
use _examples/relapses.dta, clear
pira id edss edss_date, dxdate(dx_date) relapses(_examples/relapses_only.dta)
...
* Compare PIRA vs RAW
use _examples/relapses.dta, clear
pira id edss edss_date, dxdate(dx_date) relapses(_examples/relapses_only.dta) keepall
...
```stata
* Extract income closest to study entry from LISA
use _examples/cohort.dta, clear
covarclose using _examples/lisa.dta, idvar(id) indexdate(study_entry) ///
    datevar(year) vars(disp_income employment education_level) ///
    yearformat impute prefer(closest) noisily
```
