...
{synopt:{opt all:events}}track all CDP events, not just first{p_end}
...
{phang2}4. Confirms progression using a {bf:sustained-throughout} definition: the
{it:minimum} EDSS across all measurements at or after {opt confirmdays()} must
still meet the progression threshold. This is more conservative than
