...
program define dateparse_window, rclass
    ...
    // Validate lookback/followup values
    if `lookback' < 0 {
        display as error "lookback() must be non-negative"
        exit 198
    }
    if `followup' < 0 {
        display as error "followup() must be non-negative"
        exit 198
    }
    if `lookback' == 0 & `followup' == 0 {
        display as error "Must specify either lookback() or followup() or both"
        exit 198
    }
    ...
    if "`generate'" != "" {
        ...
        if "`startvar'" == "" {
            display as error "generate() requires at least one variable name"
            exit 198
        }

        // Warn if both lookback and followup but only one name
        if `lookback' > 0 & `followup' > 0 & "`endvar'" == "" {
            display as error "generate() requires two variable names when both lookback() and followup() are specified"
            exit 198
        }
        ...
        // Generate window variables based on lookback/followup
        if `lookback' > 0 & `followup' > 0 {
            // Both lookback and followup: full window around index
            quietly generate long `startvar' = `indexvar' - `lookback'
            label variable `startvar' "Window start (lookback)"
            format `startvar' %td
            return local startvar "`startvar'"

            if "`endvar'" != "" {
                quietly generate long `endvar' = `indexvar' + `followup'
                label variable `endvar' "Window end (followup)"
                format `endvar' %td
            ...
            quietly generate long `startvar' = `indexvar' - `lookback'
            if "`endvar'" != "" {
                quietly generate long `endvar' = `indexvar' - 1
            ...
            // Followup only
            quietly generate long `startvar' = `indexvar' + 1
            if "`endvar'" != "" {
                quietly generate long `endvar' = `indexvar' + `followup'
...
program define dateparse_filerange, rclass
    ...
    // Lookback/followup in approximate years (for return values)
    local lookback_years = ceil(`lookback' / 365.25)
    local followup_years = ceil(`followup' / 365.25)

    // File year range from exact date math (not ceil approximation)
    local file_start_year = `earliest_year'
    local file_end_year = `latest_year'
