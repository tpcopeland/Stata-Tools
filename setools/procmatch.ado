*! procmatch Version 1.2.0  2026/04/24
...
program define procmatch, rclass
    ...
    * Strip any trailing comma from subcommand
    ...
    if "`subcmd'" == "match" {
        ...
        * Propagate return values
        return local varname "`r(varname)'"
        return local datevarname "`r(datevarname)'"
        return local codes "`r(codes)'"
        return scalar n_codes = r(n_codes)
        return scalar n_persons = r(n_persons)
        return scalar n_matches = r(n_matches)
    }
    else if "`subcmd'" == "" {
...
program define procmatch_first, rclass
    ...
    * Count unique persons with a match
    ...
    if "`noisily'" != "" {
        ...
        display as text "  Matching records:       " as result `n_matches'
    }
