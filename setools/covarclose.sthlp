...
{phang}
{opt vars(string)} specifies the covariate variable names to extract from the
using file. List each variable name separated by spaces. Varlist range notation
(e.g., {it:x-y}) is not supported because variables are validated against the
using file, not the data in memory.
