...
* Validation tests for setools package
*
* Tests:
...
* - cdp/pira: CDP algorithm cross-validation between cdp.ado and pira.ado
...
if _rc == 0 {
    * Running from <pkg>/qa/ directory
    ...
    local base_path "."
    local validation_path "_devkit/_validation"
}
...
capture {
    ...
    // Merge first result back
    merge m:1 id using `run1', nogen
...
/*******************************************************************************
* Section 11: CDP/PIRA Cross-Validation
* Verifies the duplicated CDP algorithm produces identical results in both
* cdp.ado and pira.ado (Issue #2 from code review)
*******************************************************************************/
di as text _n "=== Section 11: CDP/PIRA Cross-Validation ===" _n

* Create test data for CDP/PIRA
clear
...
label data "CDP/PIRA test data"
save "`validation_path'/data/val_cdp_pira.dta", replace

* Create empty relapse file (no relapses -> all CDP events should be PIRA)
clear
gen long id = .
gen double relapse_date = .
format relapse_date %td
label data "Empty relapse file for PIRA testing"
save "`validation_path'/data/val_relapses_empty.dta", replace emptyok

di as text "CDP/PIRA test datasets created successfully."

* Test 11.1: CDP basic execution
local ++test_num
di as text "Test `test_num': cdp basic execution"
capture {
    use "`validation_path'/data/val_cdp_pira.dta", clear
    cdp id edss edss_dt, dxdate(dx_date) keepall generate(cdp_dt)
}
if _rc == 0 {
    di as result "  PASS: cdp executed without error"
    local ++pass_count
}
else {
    di as error "  FAIL: cdp failed with rc = `=_rc'"
    local ++fail_count
}

* Test 11.2: CDP identifies correct number of events
local ++test_num
di as text "Test `test_num': cdp identifies 2 events (persons 1 and 3)"
capture {
    use "`validation_path'/data/val_cdp_pira.dta", clear
    cdp id edss edss_dt, dxdate(dx_date) keepall generate(cdp_dt)
}
if r(N_events) == 2 {
    di as result "  PASS: r(N_events) = 2 as expected"
    local ++pass_count
}
else {
    di as error "  FAIL: r(N_events) = `r(N_events)', expected 2"
    local ++fail_count
}

* Test 11.3: PIRA basic execution with empty relapse file
local ++test_num
di as text "Test `test_num': pira basic execution"
capture {
    use "`validation_path'/data/val_cdp_pira.dta", clear
    pira id edss edss_dt, dxdate(dx_date) ///
        relapses("`validation_path'/data/val_relapses_empty.dta") ///
        keepall generate(pira_dt) rawgenerate(raw_dt)
}
if _rc == 0 {
    di as result "  PASS: pira executed without error"
    local ++pass_count
}
else {
    di as error "  FAIL: pira failed with rc = `=_rc'"
    local ++fail_count
}

* Test 11.4: With no relapses, all CDP events should be PIRA (N_pira == N_cdp)
local ++test_num
di as text "Test `test_num': No relapses -> N_pira == N_cdp"
local pira_n = 0
local pira_cdp = 0
capture {
    use "`validation_path'/data/val_cdp_pira.dta", clear
    pira id edss edss_dt, dxdate(dx_date) ///
        relapses("`validation_path'/data/val_relapses_empty.dta") ///
        keepall generate(pira_dt2) rawgenerate(raw_dt2)
    local pira_n = r(N_pira)
    local pira_cdp = r(N_cdp)
}
if `pira_n' == `pira_cdp' & `pira_cdp' == 2 {
    di as result "  PASS: N_pira = N_cdp = 2 (all CDP classified as PIRA)"
    local ++pass_count
}
else {
    di as error "  FAIL: N_pira=`pira_n', N_cdp=`pira_cdp', expected both = 2"
    local ++fail_count
}

* Test 11.5: Cross-validate CDP dates match between cdp.ado and pira.ado
local ++test_num
di as text "Test `test_num': CDP dates match between cdp and pira"
local dates_match = 0
capture {
    * Run cdp
    use "`validation_path'/data/val_cdp_pira.dta", clear
    ...
    * Run pira (no relapses -> pira_dt should equal cdp_dt)
    use "`validation_path'/data/val_cdp_pira.dta", clear
    pira id edss edss_dt, dxdate(dx_date) ///
        relapses("`validation_path'/data/val_relapses_empty.dta") ///
        keepall generate(pira_dt) rawgenerate(raw_dt)
    ...
    * Merge and compare
    ...
    gen match = (cdp_dt == pira_dt) | (missing(cdp_dt) & missing(pira_dt))
    sum match
    local dates_match = (r(min) == 1)
}
if `dates_match' == 1 {
    di as result "  PASS: CDP dates from cdp.ado and pira.ado are identical"
    local ++pass_count
}
else {
    di as error "  FAIL: CDP dates differ between cdp.ado and pira.ado"
    local ++fail_count
}
...
capture erase "`validation_path'/data/val_cdp_pira.dta"
capture erase "`validation_path'/data/val_relapses_empty.dta"
