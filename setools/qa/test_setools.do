...
if _rc == 0 {
    local ++pass_count
    display as result "  PASS: default dispatcher metadata matches v1.2.3 surface"
}
else {
    local ++fail_count
    local failed_tests "`failed_tests' dispatcher_default"
    display as error "  FAIL: default dispatcher metadata matches v1.2.3 surface (error `=_rc')"
}
