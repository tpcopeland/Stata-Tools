...
{phang2}{stata `"dateparse parse, datestring("2020-01-15")"':. dateparse parse, datestring("2020-01-15")}{p_end}
{phang2}{stata "display r(date)":. display r(date)}{p_end}
{phang2}{stata "display %td r(date)":. display %td r(date)}{p_end}

{pstd}
{bf:Example 2: Parse compact ISO date}

{phang2}{stata `"dateparse parse, datestring("20200115")"':. dateparse parse, datestring("20200115")}{p_end}
{phang2}{stata "local mydate = r(date)":. local mydate = r(date)}{p_end}
{phang2}{stata `"display "Date: " %td `mydate'"':. display "Date: " %td `mydate'}{p_end}
...
{phang2}{stata "dateparse window study_entry, followup(1826) generate(fu_start fu_end)":. dateparse window study_entry, followup(1826) generate(fu_start fu_end)}{p_end}
{phang2}{cmd:. * 1826 days = 5 years * 365.25 days/year}{p_end}

{pstd}
{bf:Example 5: Validate study period}

{phang2}{stata `"dateparse validate, start("2006-01-01") end("2023-12-31")"':. dateparse validate, start("2006-01-01") end("2023-12-31")}{p_end}
{phang2}{stata `"display "Study span: " r(span_years) " years""':. display "Study span: " r(span_years) " years"}{p_end}
{phang2}{stata `"display "Study span: " r(span_days) " days""':. display "Study span: " r(span_days) " days"}{p_end}

{pstd}
{bf:Example 6: Create indicator for events in lookback window}

{phang2}{cmd:. * Find diagnoses in the year before study entry}{p_end}
{phang2}{stata "dateparse window study_entry, lookback(365) generate(lb_start lb_end)":. dateparse window study_entry, lookback(365) generate(lb_start lb_end)}{p_end}
{phang2}{cmd:. }{p_end}
{phang2}{cmd:. * Check if visit_date falls within lookback}{p_end}
{phang2}{stata "dateparse inwindow visit_date, start(lb_start) end(lb_end) generate(in_lookback)":. dateparse inwindow visit_date, start(lb_start) end(lb_end) generate(in_lookback)}{p_end}
{phang2}{stata "tab in_lookback":. tab in_lookback}{p_end}

{pstd}
{bf:Example 7: Find events within fixed calendar period}

{phang2}{stata `"dateparse inwindow birth_date, start("1980-01-01") end("1989-12-31") generate(born_1980s)"':. dateparse inwindow birth_date, start("1980-01-01") end("1989-12-31") ///}{p_end}
{phang3}{cmd:generate(born_1980s)}{p_end}
{phang2}{stata "tab born_1980s":. tab born_1980s}{p_end}

{pstd}
{bf:Example 8: Determine which outpatient files to load}

{phang2}{cmd:. * Cohort with index dates 2006-2023, 2-year lookback}{p_end}
{phang2}{stata `"dateparse filerange, index_start("2006-01-01") index_end("2023-12-31") lookback(730)"':. dateparse filerange, index_start("2006-01-01") index_end("2023-12-31") ///}{p_end}
{phang3}{cmd:lookback(730)}{p_end}
{phang2}{cmd:. }{p_end}
{phang2}{stata `"display "Load files from " r(file_start_year) " to " r(file_end_year)"':. display "Load files from " r(file_start_year) " to " r(file_end_year)}{p_end}
...
{phang2}{stata "summarize study_entry, format":. summarize study_entry, format}{p_end}
{phang2}{stata `"local earliest = string(r(min), "%tdCCYY-NN-DD")"':. local earliest = string(r(min), "%tdCCYY-NN-DD")}{p_end}
{phang2}{stata `"local latest = string(r(max), "%tdCCYY-NN-DD")"':. local latest = string(r(max), "%tdCCYY-NN-DD")}{p_end}
{phang2}{cmd:. }{p_end}
{phang2}{stata `"dateparse filerange, index_start("`earliest'") index_end("`latest'") lookback(365)"':. dateparse filerange, index_start("`earliest'") index_end("`latest'") lookback(365)}{p_end}
{phang2}{stata "local year_start = r(file_start_year)":. local year_start = r(file_start_year)}{p_end}
{phang2}{stata "local year_end = r(file_end_year)":. local year_end = r(file_end_year)}{p_end}
{phang2}{cmd:. }{p_end}
{phang2}{stata `"display "Need to load outpatient files from `year_start' to `year_end'""':. display "Need to load outpatient files from `year_start' to `year_end'"}{p_end}

{pstd}
{bf:Example 10: Create multiple time windows for analysis}

{phang2}{cmd:. * Create multiple windows from study entry}{p_end}
{phang2}{stata "dateparse window study_entry, lookback(365) generate(year1_before_start year1_before_end)":. dateparse window study_entry, lookback(365) generate(year1_before_start year1_before_end)}{p_end}
{phang2}{stata "dateparse window study_entry, followup(365) generate(year1_after_start year1_after_end)":. dateparse window study_entry, followup(365) generate(year1_after_start year1_after_end)}{p_end}
{phang2}{stata "dateparse window study_entry, followup(1826) generate(year5_after_start year5_after_end)":. dateparse window study_entry, followup(1826) generate(year5_after_start year5_after_end)}{p_end}
