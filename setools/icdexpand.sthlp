...
{pstd}
Each wildcard base code expands to approximately 121 code variants (1 base + 10 decimal
subcodes + 10 non-decimal subcodes + 100 two-digit detail codes). Broad patterns can
generate large code lists; use {opt maxcodes(#)} to adjust the safety limit if needed.
...
{phang2}{stata `"icdexpand expand, pattern("I63*") noisily"':. icdexpand expand, pattern("I63*") noisily}{p_end}
{phang2}{stata "return list":. return list}{p_end}
{phang2}{stata "display r(n_codes)":. display r(n_codes)}{p_end}

{pstd}
{bf:Example 2: Expand a range of diabetes codes}

{phang2}{stata `"icdexpand expand, pattern("E10-E14")"':. icdexpand expand, pattern("E10-E14")}{p_end}
{phang2}{stata `"local diabetes_codes "`r(codes)'""':. local diabetes_codes "`r(codes)'"}{p_end}

{pstd}
{bf:Example 3: Expand multiple patterns}

{phang2}{stata `"icdexpand expand, pattern("I63* I64* G45*")"':. icdexpand expand, pattern("I63* I64* G45*")}{p_end}
{phang2}{stata `"local stroke_codes "`r(codes)'""':. local stroke_codes "`r(codes)'"}{p_end}

{pstd}
{bf:Example 4: Validate ICD codes}

{phang2}{stata `"icdexpand validate, pattern("I63.4, E11.2, ZZZ99") noisily"':. icdexpand validate, pattern("I63.4, E11.2, ZZZ99") noisily}{p_end}
{phang2}{stata `"if r(valid) == 0 display "Invalid codes: `r(invalid_codes)'""':. if r(valid) == 0 display "Invalid codes: `r(invalid_codes)'"}{p_end}

{pstd}
{bf:Example 5: Create stroke indicator from diagnosis data}
...
{phang2}{stata `"icdexpand match, codes("I63* I64*") dxvars(icd) generate(stroke) noisily"':. icdexpand match, codes("I63* I64*") dxvars(icd) generate(stroke) noisily}{p_end}
{phang2}{stata "tab stroke":. tab stroke}{p_end}

{pstd}
{bf:Example 6: Find MS diagnoses}

{phang2}{stata `"use "https://raw.githubusercontent.com/tpcopeland/Stata-Dev/main/_examples/diagnoses.dta", clear"':. use _examples/diagnoses.dta, clear}{p_end}
{phang2}{stata `"icdexpand match, codes("G35") dxvars(icd) generate(ms_dx) replace"':. icdexpand match, codes("G35") dxvars(icd) generate(ms_dx) replace}{p_end}
{phang2}{stata "keep if ms_dx == 1":. keep if ms_dx == 1}{p_end}

{pstd}
{bf:Example 7: Create cancer indicator (excluding non-melanoma skin cancer)}

{phang2}{stata `"icdexpand match, codes("C00-C43* C45-C97*") dxvars(icd) generate(cancer)"':. icdexpand match, codes("C00-C43* C45-C97*") dxvars(icd) generate(cancer)}{p_end}
...
{phang2}{stata `"use "https://raw.githubusercontent.com/tpcopeland/Stata-Dev/main/_examples/diagnoses.dta", clear"':. use _examples/diagnoses.dta, clear}{p_end}
{phang2}{cmd:. }{p_end}
{phang2}{cmd:. * Identify stroke diagnoses}{p_end}
{phang2}{stata `"icdexpand match, codes("I63* I64*") dxvars(icd) generate(stroke)"':. icdexpand match, codes("I63* I64*") dxvars(icd) generate(stroke)}{p_end}
{phang2}{cmd:. }{p_end}
{phang2}{cmd:. * Keep first stroke per person}{p_end}
{phang2}{stata "keep if stroke == 1":. keep if stroke == 1}{p_end}
{phang2}{stata "bysort id (visit_date): keep if _n == 1":. bysort id (visit_date): keep if _n == 1}{p_end}
{phang2}{stata "rename visit_date stroke_date":. rename visit_date stroke_date}{p_end}
