...
{synopt:{opt rel:apses(filename)}}path to a dataset of relapse dates (one row per relapse){p_end}

{syntab:Relapse file}
{synopt:{opt relapsei:dvar(varname)}}ID variable in the relapse file; default is the same as {it:idvar}{p_end}
{synopt:{opt relapsed:atevar(varname)}}relapse date variable; default is {cmd:relapse_date}; must be a daily {cmd:%td} date{p_end}

{syntab:Relapse window}
{synopt:{opt windowb:efore(#)}}days before a relapse to exclude from PIRA; default is {cmd:90}{p_end}
{synopt:{opt windowa:fter(#)}}days after a relapse to exclude from PIRA; default is {cmd:30}{p_end}
...
neurodegeneration, occurring outside of relapse episodes.{p_end}
...
{phang2}1. {bf:Identify confirmed disability progression (CDP)} using the same
algorithm as {helpb cdp}: determine baseline EDSS, apply the 1.0/0.5-point
threshold rule, and require sustained-throughout confirmation at
...
{pstd}
Both dates are returned as separate variables, so you can analyze PIRA and RAW
outcomes independently (e.g., competing-risks or cause-specific Cox models).
...
{synopt:{cmd:r(N_cdp)}}total number of confirmed disability progression events{p_end}
{synopt:{cmd:r(N_pira)}}number of PIRA events (progression outside relapse windows){p_end}
{synopt:{cmd:r(N_raw)}}number of RAW events (progression inside a relapse window){p_end}
