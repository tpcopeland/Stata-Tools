...
local qa_dir "`c(pwd)'"
local pkg_dir "`qa_dir'/.."
capture noisily include "`qa_dir'/test_datadict_v14.do"
local rc = _rc
if `rc' {
    display as error "test_datadict_v14.do failed with rc `rc'"
    local ++failures
}

local qa_dir "`c(pwd)'"
local pkg_dir "`qa_dir'/.."
capture noisily include "`qa_dir'/test_datamap_v15.do"
local rc = _rc
if `rc' {
    display as error "test_datamap_v15.do failed with rc `rc'"
    local ++failures
}

local qa_dir "`c(pwd)'"
local pkg_dir "`qa_dir'/.."
capture noisily include "`qa_dir'/test_datamap_v152.do"
local rc = _rc
if `rc' {
    display as error "test_datamap_v152.do failed with rc `rc'"
    local ++failures
}

local qa_dir "`c(pwd)'"
local pkg_dir "`qa_dir'/.."
capture noisily include "`qa_dir'/test_datamap_v154.do"
local rc = _rc
if `rc' {
    display as error "test_datamap_v154.do failed with rc `rc'"
    local ++failures
}

local qa_dir "`c(pwd)'"
local pkg_dir "`qa_dir'/.."
capture noisily include "`qa_dir'/test_datamap_v160.do"
local rc = _rc
if `rc' {
    display as error "test_datamap_v160.do failed with rc `rc'"
    local ++failures
}
