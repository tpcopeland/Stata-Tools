...
```stata
net install datamap, from("https://raw.githubusercontent.com/tpcopeland/Stata-Dev/main/datamap")
```
...
Specify a space-separated list of dataset names:

```stata
datamap, filelist(baseline followup_6mo followup_12mo final)
```

The `.dta` extension is optional and will be added automatically.
...
### Example 11: Document synthetic cohort with privacy controls

```stata
datamap, single(_examples/cohort.dta) ///
    exclude(id birth_date death_date) ///
    datesafe ///
    output(datamap/demo/cohort_documentation.txt)
```

### Example 12: Document all example datasets

```stata
datamap, directory(_examples) ///
    exclude(id) ///
    datesafe ///
    output(datamap/demo/synthdata_documentation.txt)
```
...
### Example 8: Multiple datasets from list

```stata
datadict, filelist(baseline followup_6mo followup_12mo final) ///
...
### Example 14: Create Markdown data dictionary for the cohort

```stata
datadict, single(_examples/cohort.dta) ///
...
### Example 15: Custom thresholds for survey data
...
```stata
net from https://raw.githubusercontent.com/tpcopeland/Stata-Dev/main/datamap
net install datamap
```
