	...
	if "`showstats'" != "" {
		...
		if "`varclass'" == "categorical" {
			...
			if r(N) > 0 {
				...
				FormatStatNumber `raw_mean'
				local mean `r(formatted)'
				FormatStatNumber `raw_sd'
				local sd `r(formatted)'
				FormatStatNumber `raw_p50'
				local median `r(formatted)'
				FormatStatNumber `raw_p25'
				local p25 `r(formatted)'
				FormatStatNumber `raw_p75'
				local p75 `r(formatted)'
				FormatStatNumber `raw_min'
				local vmin `r(formatted)'
				FormatStatNumber `raw_max'
			...
			if r(N) > 0 {
				...
				if strpos("`vfmt'", "%td") > 0 {
					local datefmt "%tdCCYY/NN/DD"
				}
				else if strpos("`vfmt'", "%tc") > 0 {
					local datefmt "%tcCCYY/NN/DD"
				}
				else {
