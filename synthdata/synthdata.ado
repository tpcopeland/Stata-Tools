...
program define synthdata
    ...
    // Clean up any leaked globals from previous failed runs
    cap macro drop SYNTHDATA_derived_*
    cap macro drop SD_nlog_max_*
    ...
    syntax [varlist] [if] [in], ///
        ...
        [CORRelations CONSTraints(string asis) AUTOCONStraints] ///
        [AUTORELate CONDCat] ///
        ...
        [CONDCont RANDomeffects TRANSform MISSpattern TRENDs] ///
    ...
    if `has_rowstruct' == 1 {
        ...
        // Detect variables that are constant within each ID
        _synthdata_detect_constwithin `varlist', idvar(`idvar') origdata(`origdata')
        local const_within_id `r(const_vars)'
        local n_const: word count `const_within_id'
        if `n_const' > 0 {
            di as txt "  Constant within ID: " as res "`const_within_id'" ///
                as txt " (`n_const' variable" cond(`n_const' > 1, "s", "") ")"
        }
    }
    ...
    if "`transform'" != "" & "`contvars'" != "" {
        ...
        if "`transform_vars'" != "" {
            ...
            _synthdata_apply_transform `contvars', transforms(`transforms')
            // Re-save origdata with transformed values
    ...
    // Check if condcont is enabled and we have both continuous and categorical vars
    // Use synth_contvars which includes both continuous and integer variables
    local use_condcont = 0
    if "`condcont'" != "" & "`synth_contvars'" != "" & "`catvars'" != "" {
    ...
    if `use_condcont' {
        ...
        if `: word count `catvars'' > 1 {
            ...
            // Use subroutine wrapper to avoid nested preserve in main program
            _synthdata_condcont_remaincat, catvars(`catvars') ///
                origdata(`origdata') n(`n')
        }
        // Generate string variables (not handled by condcont)
        local nstr: word count `strvars'
        if `nstr' > 0 {
            di as txt "    Generating string variables (`nstr')..."
            mata: _synthdata_synthstr_multi("`strvars'", "`origdata'", `n')
        }
        // Generate date variables (not handled by condcont)
        local ndate: word count `datevars'
        if `ndate' > 0 {
            di as txt "    Generating date variables (`ndate')..."
            preserve
            qui use `origdata', clear
            local datenum = 1
            foreach v of local datevars {
                qui su `v', detail
                local cc_datemean_`datenum' = r(mean)
                local cc_datesd_`datenum' = cond(r(sd) == 0 | r(sd) == ., 1, r(sd))
                local cc_datemin_`datenum' = r(min)
                local cc_datemax_`datenum' = r(max)
                local cc_datefmt_`datenum': format `v'
                local ++datenum
            }
            restore
            local datenum = 1
            foreach v of local datevars {
                qui gen double `v' = round(rnormal(`cc_datemean_`datenum'', `cc_datesd_`datenum''))
                qui replace `v' = max(`v', `cc_datemin_`datenum'')
                qui replace `v' = min(`v', `cc_datemax_`datenum'')
                format `v' `cc_datefmt_`datenum''
                local ++datenum
            }
        }
    }
    ...
    if `has_rowstruct' == 1 & "`id'" != "" {
        ...
        // Re-synthesize string variables to restore proper distribution
        // The expand/reduce above distorts string frequencies by random sampling
        // Exclude constant-within-ID strings (they get enforced in next step)
        local resynth_strs: list strvars - const_within_id
        local nstr: word count `resynth_strs'
        if `nstr' > 0 {
            di as txt "  Re-synthesizing string variables for correct distribution..."
            _synthdata_resynth_strings, strvars(`resynth_strs') origdata(`origdata')
        }

        // Enforce constant-within-ID variables
        if "`const_within_id'" != "" {
            local first_id: word 1 of `id'
            _synthdata_enforce_constwithin, idvar(`first_id') const_vars(`const_within_id')
        }
    }
    else if "`id'" != "" {
    ...
    if `multiple' > 1 {
        ...
        forvalues m = 2/`multiple' {
            ...
            if `use_condcont' {
                // Categorical-continuous conditioning (mirrors single-dataset path)
                _synthdata_condcont, contvars(`synth_contvars') catvars(`catvars') ///
                    n(`n') origdata(`origdata')
                // Generate remaining categoricals (beyond the stratification variable)
                if `: word count `catvars'' > 1 {
                    preserve
                    qui use `origdata', clear
                    local m_catnum = 1
                    foreach v of local catvars {
                        if `m_catnum' > 1 {
                            qui levelsof `v', local(m_levels_`m_catnum')
                            local m_nlevels_`m_catnum': word count `m_levels_`m_catnum''
                            if `m_nlevels_`m_catnum'' > 0 {
                                tempname m_catfreq_`m_catnum'
                                matrix `m_catfreq_`m_catnum'' = J(`m_nlevels_`m_catnum'', 2, .)
                                local m_j = 1
                                foreach lev of local m_levels_`m_catnum' {
                                    qui count if `v' == `lev'
                                    matrix `m_catfreq_`m_catnum''[`m_j', 1] = `lev'
                                    matrix `m_catfreq_`m_catnum''[`m_j', 2] = r(N)
                                    local ++m_j
                                }
                                local m_vallbl_`m_catnum': value label `v'
                            }
                        }
                        local ++m_catnum
                    }
                    ...
                    local m_catnum = 1
                    foreach v of local catvars {
                        if `m_catnum' > 1 & `m_nlevels_`m_catnum'' > 0 {
                            cap drop `v'
                            qui gen double `v' = .
                            mata: _synthdata_drawcat("`v'", st_matrix("`m_catfreq_`m_catnum''"), `n')
                            if "`m_vallbl_`m_catnum''" != "" {
                                cap label values `v' `m_vallbl_`m_catnum''
                            }
                        }
                        local ++m_catnum
                    }
                ...
                local m_nstr: word count `strvars'
                if `m_nstr' > 0 {
                    mata: _synthdata_synthstr_multi("`strvars'", "`origdata'", `n')
                }
                // Generate date variables
                local m_ndate: word count `datevars'
                if `m_ndate' > 0 {
                    preserve
                    qui use `origdata', clear
                    local m_datenum = 1
                    foreach v of local datevars {
                        qui su `v', detail
                        local m_cc_datemean_`m_datenum' = r(mean)
                        local m_cc_datesd_`m_datenum' = cond(r(sd) == 0 | r(sd) == ., 1, r(sd))
                        local m_cc_datemin_`m_datenum' = r(min)
                        local m_cc_datemax_`m_datenum' = r(max)
                        local m_cc_datefmt_`m_datenum': format `v'
                        local ++m_datenum
                    }
                    restore
                    local m_datenum = 1
                    foreach v of local datevars {
                        qui gen double `v' = round(rnormal(`m_cc_datemean_`m_datenum'', `m_cc_datesd_`m_datenum''))
                        qui replace `v' = max(`v', `m_cc_datemin_`m_datenum'')
                        qui replace `v' = min(`v', `m_cc_datemax_`m_datenum'')
                        format `v' `m_cc_datefmt_`m_datenum''
                        local ++m_datenum
                    }
                }
            }
            ...
            // =============================================================
            // POST-SYNTHESIS PIPELINE (mirrors single-dataset path)
            // =============================================================
            ...
            // Back-transform skewed variables
            if "`transform'" != "" & "`transform_vars'" != "" {
                _synthdata_backtransform `contvars', transforms(`transforms')
            }
            ...
            // Handle ID variables with row-count distribution
            if `has_rowstruct' == 1 & "`id'" != "" {
                tempfile m_synthdata_temp
                qui save `m_synthdata_temp'
                _synthdata_rowcounts, target_n_ids(`target_n_ids') ///
                    rowcount_dist(`rowcount_dist') rowdist(`rowdist') ///
                    rc_mean(`rc_mean') rc_sd(`rc_sd') rc_min(`rc_min') rc_max(`rc_max')
                tempfile m_synth_rowcounts
                qui save `m_synth_rowcounts'
                qui su synth_rowcount, meanonly
                local m_total_rows = r(sum)
                qui expand synth_rowcount
                qui bysort synth_id: gen long _rownum = _n
                qui gen long _obs = _n
                tempfile m_id_structure
                qui keep synth_id _rownum _obs
                qui save `m_id_structure'
                qui use `m_synthdata_temp', clear
                local m_curr_n = _N
                if `m_total_rows' > `m_curr_n' {
                    qui expand ceil(`m_total_rows' / `m_curr_n') + 1
                    qui gen double _rand = runiform()
                    qui sort _rand
                    qui keep in 1/`m_total_rows'
                    drop _rand
                }
                else if `m_total_rows' < `m_curr_n' {
                    qui gen double _rand = runiform()
                    qui sort _rand
                    qui keep in 1/`m_total_rows'
                    drop _rand
                ...
                qui gen long _obs = _n
                qui merge 1:1 _obs using `m_id_structure', nogen keepusing(synth_id)
                foreach v of local id {
                    cap drop `v'
                    qui gen long `v' = synth_id
                    label var `v' "Synthetic ID"
                ...
                // Re-synthesize string variables after restructuring
                // Exclude constant-within-ID strings
                local m_resynth_strs: list strvars - const_within_id
                local m_nstr: word count `m_resynth_strs'
                if `m_nstr' > 0 {
                    _synthdata_resynth_strings, strvars(`m_resynth_strs') origdata(`origdata')
                }
            ...
            if "`indexdate'" != "" & "`id'" != "" {
                ...
                if !_rc {
                    tempvar m_idx_first
                    qui bysort `first_id': gen double `m_idx_first' = `indexdate'[1]
                    qui replace `indexdate' = `m_idx_first'
                    drop `m_idx_first'
                }
            }
            ...
            if "`trends'" != "" & "`id'" != "" & "`synth_contvars'" != "" {
                ...
                if "`panel'" != "" {
                    tokenize "`panel'"
                    if "`2'" != "" local m_timevar "`2'"
                }
                if "`m_timevar'" != "" {
                    local first_id: word 1 of `id'
                    cap confirm numeric variable `m_timevar'
                    if !_rc {
                        _synthdata_trends, idvar(`first_id') timevar(`m_timevar') ///
                            contvars(`synth_contvars') origdata(`origdata')
                    }
                }
            }

            // Handle skip variables
            if "`skip'" != "" {
                foreach v of local skip {
                    cap drop `v'
                    local sv = substr("`v'", 1, 30)
            ...
            // Apply missingness (pattern-based or rate-based)
            if "`misspattern'" != "" & "`misspattern_file'" != "" {
                local m_miss_varlist ""
                foreach v of local allvars_track {
                    cap confirm variable `v'
                    if _rc continue
                    local isid: list v in id
                    ...
                    local isskip: list v in skip
                    if `isskip' continue
                    local m_miss_varlist `m_miss_varlist' `v'
                }
                if "`m_miss_varlist'" != "" {
                    _synthdata_misspattern_apply `m_miss_varlist', patterns(`misspattern_file')
                }
            }
            else {
                foreach v of local allvars_track {
    ...
    if "`condcont'" != "" local ++realism_features
...
program define _synthdata_parametric
    ...
    if `ncont' > 0 {
        ...
        if "`empirical'" != "" {
            ...
            di as txt "    [4a/4] Generating continuous variables via empirical quantiles (`ncont')..."
            ...
            di as txt "    [4a/4] Generating continuous variables (`ncont')..."
    ...
    if `ncat' > 0 {
        di as txt "    [4b/4] Generating categorical variables (`ncat')..."
        local catnum = 1
        foreach v of local catvars {
            qui gen double `v' = .
    ...
    if `nstr' > 0 {
        di as txt "    [4c/4] Generating string variables (`nstr')..."
    ...
    if `ndate' > 0 {
        di as txt "    [4d/4] Generating date variables (`ndate')..."
        local datenum = 1
        foreach v of local datevars {
...
program define _synthdata_bootstrap
    ...
    // bsample requires n <= _N, so expand first if needed
    if `n' > `orig_n' {
...
program define _synthdata_permute
    ...
    // If n > orig_n, expand first so we have enough values to permute
    if `n' > `orig_n' {
        local expand_factor = ceil(`n' / `orig_n')
        qui expand `expand_factor'
    }

    // Permute each variable independently using Mata
    // For each variable, generate a random permutation index and reindex
    // the column. Each variable gets its OWN independent permutation.
    foreach v of local varlist {
        capture confirm string variable `v'
        if _rc == 0 {
            mata: st_sstore(., "`v'", st_sdata(., "`v'")[order(runiform(st_nobs(), 1), 1)])
        }
        else {
            mata: st_store(., "`v'", st_data(., "`v'")[order(runiform(st_nobs(), 1), 1)])
        }
    }
    ...
    while `iter' < `iterate' & !`satisfied' {
        ...
        while `"`remaining'"' != "" {
            ...
            if r(N) > 0 {
                ...
                // For var1 < var2 style constraints (check before var<val)
...
// ENFORCE CONSTANT-WITHIN-ID VARIABLES
// =========================================================================
// For each variable in const_vars, set all rows within each ID group
// to share the same value (first row's value).
program define _synthdata_enforce_constwithin
    version 16.0
    set varabbrev off
    set more off
    syntax, idvar(varname) const_vars(string)

    local n_const: word count `const_vars'
    if `n_const' == 0 exit

    di as txt "  Enforcing " as res `n_const' as txt " constant-within-ID variable" ///
        cond(`n_const' > 1, "s", "") "..."

    foreach v of local const_vars {
        cap confirm variable `v'
        if _rc continue
        cap confirm string variable `v'
        if !_rc {
            // String variable
            qui bysort `idvar': replace `v' = `v'[1]
        }
        else {
            // Numeric variable
            tempvar cw_first
            qui bysort `idvar': gen double `cw_first' = `v'[1]
            qui replace `v' = `cw_first'
            drop `cw_first'
        }
    }
end
...
// =========================================================================
// RE-SYNTHESIZE STRING VARIABLES (wrapper for preserve/restore scoping)
// =========================================================================
// The Mata function _synthdata_synthstr_fromdata uses preserve/restore
// internally to load original data. This wrapper provides a program scope
// so the internal preserve doesn't conflict with the main program's preserve.
program define _synthdata_resynth_strings
    version 16.0
    set varabbrev off
    set more off
    syntax, strvars(string) origdata(string)

    local final_n = _N
    foreach v of local strvars {
        cap drop `v'
    }
    mata: _synthdata_synthstr_multi("`strvars'", "`origdata'", `final_n')
end

// =========================================================================
// DETECT VARIABLES THAT ARE CONSTANT WITHIN EACH ID
// =========================================================================
// For each variable in varlist, checks whether it is constant within
// each group defined by idvar. Returns space-separated list in r(const_vars).
program define _synthdata_detect_constwithin, rclass
    version 16.0
    set varabbrev off
    set more off
    syntax varlist, idvar(varname) origdata(string)
    ...
    foreach v of local varlist {
        ...
        // Check if variable is constant within each ID group
        // Compare only non-missing pairs: numeric missing != missing
        // evaluates to 1 (true), which would cause false negatives
        tempvar not_const
        cap confirm string variable `v'
        if !_rc {
            // String: "" != "" is 0, so simple comparison is safe
            cap bysort `idvar': gen byte `not_const' = (`v' != `v'[1])
        }
        else {
            // Numeric: skip rows where either value is missing
            cap bysort `idvar': gen byte `not_const' = ///
                (`v' != `v'[1]) if !missing(`v') & !missing(`v'[1])
        }
        if _rc {
            cap drop `not_const'
            continue
        }
        qui count if `not_const' == 1
        if r(N) == 0 {
            local const_vars `const_vars' `v'
        }
        drop `not_const'
    }
    ...
    return local const_vars `const_vars'
end
...
program define _synthdata_rowcounts
    ...
    if "`rowdist'" == "exact" {
        ...
        // Use Mata to sample and store directly (avoids local macro length limits)
        local orig_n = _N
        tempname rc_data
        mata: `rc_data' = st_data(., "`rcvar'")
        ...
        // Sample with replacement and store directly via Mata
        mata: st_store(., "synth_rowcount", ///
            `rc_data'[ceil(runiform(`target_n_ids', 1) :* `orig_n')])
        mata: mata drop `rc_data'
    }
    ...
    // Adjust total row count to match target
    local target_total = round(`rc_mean' * `target_n_ids')
    qui su synth_rowcount, meanonly
    local current_total = r(sum)
    local diff = `target_total' - `current_total'

    if `diff' != 0 {
        local floor_val = max(`rc_min', 1)
        mata: _synthdata_adjust_rowcounts("synth_rowcount", `diff', `floor_val')
    }
...
program define _synthdata_panel
    ...
    // Save synthetic data before _synthdata_rowcounts clears it
    tempfile synth_before_panel
    qui save `synth_before_panel'
    ...
    qui keep synth_id _timevar synth_rowcount _obs
    qui save `panel_structure'

    // Reload synthetic data for restructuring
    qui use `synth_before_panel', clear
...
program define _synthdata_condcont, rclass
    ...
    foreach lev of local strat_levels {
        ...
        if `ncont' > 1 {
            cap qui correlate `contvars' if `stratvar' == `lev', cov
            if _rc == 0 & r(N) >= `ncont' {
...
// Generate remaining categorical variables after condcont synthesis
// Wrapped in a subroutine to provide its own preserve scope
program define _synthdata_condcont_remaincat
    version 16.0
    set varabbrev off
    set more off
    syntax, catvars(string) origdata(string) n(integer)
    ...
    foreach v of local contvars {
        if `icc_`v'' > 0.01 {
            ...
            if `total_sd' > 0 {
                ...
                // Generate one random effect per ID using bysort
                tempvar re_`v'
                qui gen double `re_`v'' = rnormal(0, `re_sd')
                qui bysort `id_num': replace `re_`v'' = `re_`v''[1]
...
program define _synthdata_apply_transform
    ...
    foreach v of local varlist {
        ...
        if "`trans'" == "log" {
            ...
            // Use truncated varname (prefix is 12 chars, leaving 19 for varname)
            global SD_nlog_max_`=substr("`v'", 1, 19)' `vmax'
        }
        local ++i
    }
end
...
program define _synthdata_backtransform
    ...
    foreach v of local varlist {
        ...
        if "`trans'" == "log" {
            ...
            // Need the original max value - stored in global by apply_transform
            local sv_nlog = substr("`v'", 1, 19)
            local vmax = ${SD_nlog_max_`sv_nlog'}
            if "`vmax'" != "" {
                qui replace `v' = `vmax' + 1 - exp(`v')
            }
            macro drop SD_nlog_max_`sv_nlog'
        }
        local ++i
    }
end
...
program define _synthdata_misspattern_capture
    ...
    qui su _freq, meanonly
    qui gen double _prop = _freq / r(sum)
...
program define _synthdata_privacycheck, rclass
    ...
    // For computational efficiency, use Mata (pass tempvar name for result storage)
    mata: _synthdata_compute_mindist("`varlist'", "`orig_temp'", `sample', `nvars', "`min_dist'")
...
program define _synthdata_trends
    ...
    foreach v of local contvars {
        if `slope_sd_`v'' > 0 {
            // Generate one slope per ID using bysort
            tempvar synth_slope
            qui gen double `synth_slope' = rnormal(`slope_mean_`v'', `slope_sd_`v'')
            qui bysort `id_num': replace `synth_slope' = `synth_slope'[1]
...
program define _synthdata_enforce_dateorder
    ...
    // Sort dates within each observation using Mata (vectorized, O(N) not O(N*ndates^2))
    mata: _synthdata_sortdates("`date_vars'")
end
...
program define _synthdata_freqcheck
    ...
    // Load original frequencies ONCE and store in locals (avoids per-level disk I/O)
    preserve
    qui use `origfreq', clear
    local nrows = _N
    forvalues i = 1/`nrows' {
        local ofq_vn = varname[`i']
        local ofq_val = value[`i']
        // Use truncated key to stay within macro name limits
        local ofq_key = substr("`ofq_vn'", 1, 20) + "_" + subinstr("`ofq_val'", "-", "n", .)
        local ofq_key = subinstr("`ofq_key'", ".", "d", .)
        local origpct_`ofq_key' = pct[`i']
    }
    restore
    ...
    foreach v of local varlist {
        ...
        foreach lev of local synth_levels {
            ...
            // Original proportion (from pre-loaded locals)
            local ofq_key = substr("`v'", 1, 20) + "_" + subinstr("`lev'", "-", "n", .)
            local ofq_key = subinstr("`ofq_key'", ".", "d", .)
            local orig_pct = 0
            if "`origpct_`ofq_key''" != "" {
                local orig_pct = `origpct_`ofq_key''
            }
...
program define _synthdata_indexdate_analyze
    ...
    // Return index date format to parent for restoration after synthesis
...
void _synthdata_drawcat(string scalar varname, real matrix valfreq, real scalar n)
{
    ...
    real scalar i, lo, hi, mid, nlevels, total
    ...
    for (i = 1; i <= n; i++) {
        // Binary search for first j where cumprob[j] >= u[i]
        lo = 1
        hi = nlevels
        while (lo < hi) {
            mid = floor((lo + hi) / 2)
                            ...
                            string scalar strnum, real scalar n)
{
    ...
    real scalar i, j, lo, hi, mid, total
...
// Adjust row counts so their sum matches target total
// Randomly increments/decrements individual ID row counts by 1
// until sum(rowcounts) == target_total, respecting floor_val as minimum
void _synthdata_adjust_rowcounts(string scalar varname, real scalar diff,
                                   real scalar floor_val)
{
    real colvector rc, elig_idx
    real scalar n, idx, remaining, attempts, i, n_elig
    ...
    if (diff > 0) {
        // Need more rows: increment random IDs by 1
        while (remaining > 0 & attempts < n * 10) {
            idx = ceil(runiform(1, 1) * n)
            rc[idx] = rc[idx] + 1
            remaining--
            attempts++
        }
        ...
        // Need fewer rows: decrement random IDs by 1 (respecting floor)
        while (remaining > 0 & attempts < n * 10) {
            idx = ceil(runiform(1, 1) * n)
            if (rc[idx] > floor_val) {
                rc[idx] = rc[idx] - 1
        ...
        // Deterministic fallback: sweep all eligible IDs if random failed
        if (remaining > 0) {
            elig_idx = selectindex(rc :> floor_val)
            n_elig = rows(elig_idx)
            i = 1
            while (remaining > 0 & n_elig > 0) {
                rc[elig_idx[i]] = rc[elig_idx[i]] - 1
                remaining--
                i++
                if (i > n_elig) {
                    // Refresh eligible list (some may have hit floor)
                    elig_idx = selectindex(rc :> floor_val)
                    n_elig = rows(elig_idx)
                    i = 1
                }
                                 ...
                                 string scalar resultvar)
{
    ...
    // Load original data
    stata("preserve")
    stata("qui use " + char(34) + origfile + char(34) + ", clear")
    ...
    // Store results back to Stata using the passed variable name
    st_store(., resultvar, min_dists)
}
