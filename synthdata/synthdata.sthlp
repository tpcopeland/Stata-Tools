...
{phang}
{opt empirical} uses empirical quantiles instead of normal distribution for
continuous variable marginals. Preserves skewness and other non-normal features.
...
{phang}
{opt categorical(varlist)} forces specified variables to be treated as
categorical, overriding automatic detection.
