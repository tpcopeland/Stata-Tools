...
use or quick reference. May not be combined with {opt detail}.
...
{phang2}{stata "tc_schemes":. tc_schemes}{p_end}

{pstd}Show detailed descriptions:{p_end}
{phang2}{stata "tc_schemes, detail":. tc_schemes, detail}{p_end}

{pstd}List only blindschemes:{p_end}
{phang2}{stata "tc_schemes, source(blindschemes) list":. tc_schemes, source(blindschemes) list}{p_end}

{pstd}Use a scheme for all subsequent graphs:{p_end}
{phang2}{stata "set scheme plotplain":. set scheme plotplain}{p_end}
{phang2}{stata "sysuse auto, clear":. sysuse auto, clear}{p_end}
{phang2}{stata "scatter mpg weight":. scatter mpg weight}{p_end}

{pstd}Use a scheme for a single graph:{p_end}
{phang2}{stata "sysuse auto, clear":. sysuse auto, clear}{p_end}
{phang2}{stata "scatter mpg weight, scheme(white_tableau)":. scatter mpg weight, scheme(white_tableau)}{p_end}

{pstd}Compare schemes visually:{p_end}
{phang2}{stata "sysuse auto, clear":. sysuse auto, clear}{p_end}
{phang2}{stata "scatter mpg weight, scheme(plotplain) name(g1, replace)":. scatter mpg weight, scheme(plotplain) name(g1, replace)}{p_end}
{phang2}{stata "scatter mpg weight, scheme(gg_viridis) name(g2, replace)":. scatter mpg weight, scheme(gg_viridis) name(g2, replace)}{p_end}
...
{phang2}{stata "capture which tc_schemes":. capture which tc_schemes}{p_end}
{phang2}{stata `"if _rc != 0 net install tc_schemes, from("...")"':. if _rc != 0 net install tc_schemes, from("...")}{p_end}
