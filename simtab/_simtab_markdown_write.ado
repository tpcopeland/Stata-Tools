*! _simtab_markdown_write Version 2.0.0  2026/08/19
*! Write the current dataset as a GitHub-Flavored Markdown table
*! Author: Timothy P Copeland, Karolinska Institutet
*! Program class: rclass

program define _simtab_markdown_write, rclass
    version 17.0
    ...
    capture noisily {
        ...
        if _rc {
            ...
            if _rc == 0 {
                ...
                noisily display as error "_simtab_common.ado not found; reinstall simtab"
                exit 111
            }
        }
        ...
        capture _simtab_validate_path `"`using'"' "markdown()"
        if _rc exit _rc

        local _visible_opts ""
        if `"`labelvar'"' != "" local _visible_opts "labelvar(`labelvar')"
        _simtab_visible_vars, `_visible_opts'
        local _vars `"`_simtab_visible_vars'"'
        ...
        * become a heading.
        ...
        forvalues _j = 1/`_k' {
            ...
            if "`novarnames'" == "" & inrange(`headerstart', 1, _N) {
                mata: st_local("_h`_j'", _st_md_cell("`_v'", `headerstart'))
            }
            ...
            mata: st_local("_h`_j'", _st_md_escape(st_local("_h`_j'")))
        }
        ...
        if `"`_title'"' != "" {
            mata: st_local("_title", _st_md_escape(st_local("_title")))
        ...
        if `_body_end' >= `datastart' {
            forvalues _i = `datastart'/`_body_end' {
                ...
                forvalues _j = 1/`_k' {
                    local _v : word `_j' of `_vars'
                    mata: st_local("_cell`_j'", _st_md_cell("`_v'", `_i'))
                    if `"`_cell`_j''"' != "" local _row_has_text = 1
                    mata: st_local("_cell`_j'", _st_md_escape(st_local("_cell`_j'")))
                }
                if `_row_has_text' {
                    file write `_fh' "|"
                    forvalues _j = 1/`_k' {
        ...
        if `"`footnote'"' != "" {
            mata: st_local("footnote", _st_md_escape(st_local("footnote")))
...
version 17.0
capture mata: mata drop _st_md_escape()
capture mata: mata drop _st_md_cell()

mata:
mata set matastrict on

string scalar _st_md_escape(string scalar x)
{
    x = strtrim(x)
    x = subinstr(x, "\", "\\")
    x = subinstr(x, "|", "\|")
...
string scalar _st_md_cell(string scalar v, real scalar i)
{
    real scalar j
    real scalar x
