...
program define _simtab_csv_write, nclass
    version 17.0
    ...
    capture noisily {
        ...
        capture program list _simtab_validate_path
        if _rc {
        ...
        _simtab_validate_path `"`using'"' "csv()"
        ...
        * from the contents of row 1.
        if "`reservedrow'" != "" {
            tempvar _st_filled
            quietly generate byte `_st_filled' = 0
            foreach _v of local _vars {
                capture confirm string variable `_v'
                if !_rc quietly replace `_st_filled' = 1 if strtrim(`_v') != ""
                else    quietly replace `_st_filled' = 1 if !missing(`_v')
            }
            quietly count if `_st_filled'
            if r(N) > 0 {
                tempvar _st_rowid
                quietly generate long `_st_rowid' = _n
                quietly summarize `_st_rowid' if `_st_filled', meanonly
                local _st_first_filled = r(min)
                if `_st_first_filled' > 1 quietly drop in 1/`=`_st_first_filled' - 1'
            }
        }
        ...
        * neither option anyway.
        local _st_first : word 1 of `_vars'
        capture confirm string variable `_st_first'
        if !_rc & _N > 0 {
            if `"`title'"' != "" {
                quietly insobs 1, before(1)
                quietly replace `_st_first' = `"`title'"' in 1
            }
            if `"`footnote'"' != "" {
                quietly insobs 1
                quietly replace `_st_first' = `"`footnote'"' in `=_N'
            }
        }
