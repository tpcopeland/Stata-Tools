...
capture program drop _simtab_xlsx_write
program define _simtab_xlsx_write, rclass
    version 17.0
    ...
    capture noisily {
        ...
        * cannot poison the next write.
        ...
        mata: `book' = _st_xlsx_write_mata(`"`using'"', `"`sheet'"', `"`_vars'"')
...
class xl scalar _st_xlsx_write_mata(
    ...
    string scalar varlist)
{
    ...
    b.set_mode("open")
    table = _st_cur_strmat(varlist)
    b.put_string(1, 1, table)

    return(b)
}

string matrix _st_cur_strmat(string scalar varlist)
{
...
program define _simtab_xlsx_build_styles, rclass
    ...
    capture noisily {
        ...
        mata: _st_xlsx_build_styles("`matrix'", `"`rules'"', `cols')
...
version 17.0
capture mata: mata drop _st_xlsx_build_styles()
capture mata: mata drop _st_xlsx_build_styles_rows()
    ...
    real scalar requested_cols)
{
    ...
    if (spec == "") {
        _st_xlsx_build_styles_error("rules() must contain at least one row")
    }
    if (requested_cols < 0 | requested_cols != floor(requested_cols)) {
        _st_xlsx_build_styles_error("cols() must be a nonnegative integer")
    }

    parts = _st_xlsx_build_styles_rows(spec)
    n_rules = 0
    n_cols = requested_cols
    for (i = 1; i <= cols(parts); i++) {
        if (strtrim(parts[i]) == "") continue
        values = _st_xlsx_build_styles_values(parts[i], n_rules + 1)
        n_rules++
    ...
    if (n_cols == 0) {
        _st_xlsx_build_styles_error("rules() rows must contain numeric values")
    }
    ...
    for (i = 1; i <= cols(parts); i++) {
        if (strtrim(parts[i]) == "") continue
        j++
        values = _st_xlsx_build_styles_values(parts[i], j)
...
string rowvector _st_xlsx_build_styles_rows(string scalar spec)
{
...
real rowvector _st_xlsx_build_styles_values(
    string scalar row,
    real scalar row_number)
{
    ...
    row = strtrim(row)
    if (row == "") {
        _st_xlsx_build_styles_error("empty row in rules()")
    }
    ...
    for (i = 1; i <= cols(values); i++) {
        if (values[i] >= .) {
            _st_xlsx_build_styles_error("non-numeric value in row " +
...
void _st_xlsx_build_styles_error(string scalar message)
{
    errprintf(message + "\n")
    _error(198)
}
...
program define _simtab_xlsx_apply_styles, rclass
    ...
    capture noisily {
        ...
        mata: _st_xlsx_apply_styles(`book', `"`sheet'"', st_matrix("`rules'"), ///
...
version 17.0
capture mata: mata drop _st_xlsx_apply_styles()
capture mata: mata drop _st_xlsx_style_font()
capture mata: mata drop _st_xlsx_style_onoff()
capture mata: mata drop _st_xlsx_style_halign()
capture mata: mata drop _st_xlsx_style_valign()
capture mata: mata drop _st_xlsx_style_border()
capture mata: mata drop _st_xlsx_style_rgb()
capture mata: mata drop _st_xlsx_style_validate()
capture mata: mata drop _st_xlsx_style_validate_range()
capture mata: mata drop _st_xlsx_style_validate_code()
capture mata: mata drop _st_xlsx_style_validate_positive()
    ...
    string scalar color4)
{
    ...
    for (i = 1; i <= rows(rules); i++) {
        _st_xlsx_style_validate(rules, i, colors)
        ...
        if (op == 1) {
            b.set_font((r1, r2), (c1, c2),
                _st_xlsx_style_font(rules[i, 7], font, altfont), rules[i, 6])
        }
        else if (op == 2) {
            b.set_font_bold((r1, r2), (c1, c2),
                _st_xlsx_style_onoff(rules[i, 7]))
        }
        else if (op == 3) {
            ...
            b.set_horizontal_align((r1, r2), (c1, c2),
                _st_xlsx_style_halign(rules[i, 7]))
        }
        else if (op == 6) {
            b.set_vertical_align((r1, r2), (c1, c2),
                _st_xlsx_style_valign(rules[i, 7]))
        }
        else if (op == 7) {
            b.set_fill_pattern((r1, r2), (c1, c2), "solid",
                _st_xlsx_style_rgb(rules[i, (7..9)], colors))
        }
        else if (op == 8) {
            b.set_top_border((r1, r2), (c1, c2),
                _st_xlsx_style_border(rules[i, 7]))
        }
        else if (op == 9) {
...
string scalar _st_xlsx_style_font(
    real scalar code,
    string scalar font,
    string scalar altfont)
{
...
string scalar _st_xlsx_style_onoff(real scalar code)
{
    return(code == 0 ? "off" : "on")
}

string scalar _st_xlsx_style_halign(real scalar code)
{
...
string scalar _st_xlsx_style_valign(real scalar code)
{
    if (code == 2) return("center")
...
string scalar _st_xlsx_style_border(real scalar code)
{
...
string scalar _st_xlsx_style_rgb(real rowvector rgb, string rowvector colors)
{
    ...
    string rowvector colors)
{
    ...
    op = rules[row, 1]
    _st_xlsx_style_validate_code(op, row, "operation", 1, 15)

    if (op == 12) {
        _st_xlsx_style_validate_range(rules[row, 2], rules[row, 3],
            row, "row")
        _st_xlsx_style_validate_positive(rules[row, 6], row, "height")
    }
    else if (op == 13) {
        _st_xlsx_style_validate_range(rules[row, 4], rules[row, 5],
            row, "column")
        _st_xlsx_style_validate_positive(rules[row, 6], row, "width")
    }
    else {
        _st_xlsx_style_validate_range(rules[row, 2], rules[row, 3],
            row, "row")
        ...
        if (op == 1) {
            _st_xlsx_style_validate_positive(rules[row, 6], row, "font size")
            _st_xlsx_style_validate_code(rules[row, 7], row, "font code", -1, 5)
        }
        else if (op >= 2 & op <= 4) {
            _st_xlsx_style_validate_code(rules[row, 7], row, "on/off code", 0, 1)
        }
        else if (op == 5) {
            _st_xlsx_style_validate_code(rules[row, 7], row,
            ...
            _st_xlsx_style_validate_rgb(rules[row, (7..9)], row, colors)
        }
        else if (op >= 8 & op <= 11) {
            _st_xlsx_style_validate_code(rules[row, 7], row, "border code", 1, 4)
        }
        else if (op == 15) {
            _st_xlsx_style_validate_positive(rules[row, 6], row, "font size")
            _st_xlsx_style_validate_rgb(rules[row, (7..9)], row, colors)
...
void _st_xlsx_style_validate_range(
    real scalar first,
    real scalar last,
    real scalar row,
    string scalar name)
{
    if (first >= . | first != floor(first) | first < 1) {
        _st_xlsx_style_error(row, name + " start must be a positive integer")
    }
    if (last >= . | last != floor(last) | last < 1) {
        _st_xlsx_style_error(row, name + " end must be a positive integer")
    }
    if (last < first) {
        _st_xlsx_style_error(row, name + " end must be >= start")
    }
}

void _st_xlsx_style_validate_code(
    real scalar code,
    real scalar row,
    string scalar name,
    real scalar minval,
    real scalar maxval)
{
    if (code >= . | code != floor(code) | code < minval | code > maxval) {
        _st_xlsx_style_error(row, name + " out of range")
    }
}

void _st_xlsx_style_validate_positive(
    real scalar value,
    real scalar row,
    string scalar name)
{
    if (value >= . | value <= 0) {
        _st_xlsx_style_error(row, name + " must be positive")
    }
}

void _st_xlsx_style_validate_rgb(
    real rowvector rgb,
    real scalar row,
    string rowvector colors)
{
    ...
    if (rgb[1] < 0) {
        ...
        if (color_code != floor(color_code) | color_code < 1 | color_code > 4) {
            _st_xlsx_style_error(row, "color alias code out of range")
        }
        if (colors[color_code] == "") {
            _st_xlsx_style_error(row, "color alias option is empty")
        }
        return
    }
    ...
    for (i = 1; i <= 3; i++) {
        if (rgb[i] >= . | rgb[i] != floor(rgb[i]) | rgb[i] < 0 | rgb[i] > 255) {
            _st_xlsx_style_error(row, "RGB values must be integers in [0,255]")
        }
    }
}

void _st_xlsx_style_error(real scalar row, string scalar message)
{
...
* handled, because nothing in simtab calls set_number_format() and that pool
...
capture program drop _simtab_xlsx_compact_styles
program define _simtab_xlsx_compact_styles, rclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
        ...
        * middle of a table export.
        capture _simtab_xlsx_compact_engine using `"`using'"'
...
capture program drop _simtab_xlsx_compact_engine
program define _simtab_xlsx_compact_engine, rclass
    version 17.0
    local _orig_varabbrev = c(varabbrev)
    set varabbrev off
    capture noisily {
    ...
    * Copy first, while the caller's relative path is still resolvable.
    quietly copy `"`using'"' "`work'/_st_src.xlsx"

    foreach _p in fonts fills borders formats {
        local _st_`_p'_before 0
        local _st_`_p'_after 0
    }
    ...
    * still runs rather than being skipped on an unknown state.
    ...
    capture noisily {
        quietly cd "`work'"
        quietly unzipfile "_st_src.xlsx"
        erase "_st_src.xlsx"
        ...
        * what keeps a per-sheet compaction affordable across a long workbook.
        if `_st_changed' {
            mata: st_local("_flist", _st_xlsx_quoted_files("."))
            if `"`_flist'"' == "" exit 601
            quietly zipfile `_flist', saving("_st_out.xlsx")
            confirm file "_st_out.xlsx"
            ...
            * count.
            capture mkdir "_st_verify"
            if _rc exit 693
            quietly cd "_st_verify"
            quietly unzipfile "../_st_out.xlsx"
            quietly cd ".."
            mata: _st_xlsx_check_manifest(".", "_st_verify", "_st_out.xlsx")
        }
    }
    local _rc_inner = _rc
    quietly cd "`_home'"
    ...
    * exists to shrink, and it grows with every sheet added.
    if `_rc_inner' == 0 & `_st_changed' {
        capture mata: _st_xlsx_verify("`work'/_st_out.xlsx", "`work'")
        local _rc_inner = _rc
    }

    if `_rc_inner' == 0 {
        if `_st_changed' quietly copy "`work'/_st_out.xlsx" `"`using'"', replace
        return scalar fonts_before = `_st_fonts_before'
        return scalar fonts_after = `_st_fonts_after'
        return scalar fills_before = `_st_fills_before'
        return scalar fills_after = `_st_fills_after'
        return scalar borders_before = `_st_borders_before'
        return scalar borders_after = `_st_borders_after'
        return scalar formats_before = `_st_formats_before'
        return scalar formats_after = `_st_formats_after'
    }

    capture mata: _st_xlsx_rmtree("`work'")
...
version 17.0
capture mata: mata drop _st_xlsx_compact_dir()
capture mata: mata drop _st_xlsx_compact_styles_xml()
capture mata: mata drop _st_xlsx_slurp()
capture mata: mata drop _st_xlsx_spit()
capture mata: mata drop _st_xlsx_pool_items()
capture mata: mata drop _st_xlsx_dedupe()
capture mata: mata drop _st_xlsx_block()
capture mata: mata drop _st_xlsx_remap_attr()
capture mata: mata drop _st_xlsx_remap_sheet()
capture mata: mata drop _st_xlsx_files()
capture mata: mata drop _st_xlsx_slash()
capture mata: mata drop _st_xlsx_quoted_files()
capture mata: mata drop _st_xlsx_rmtree()
capture mata: mata drop _st_xlsx_verify()
capture mata: mata drop _st_xlsx_sheetnames_xml()
capture mata: mata drop _st_xlsx_unescape()
...
void _st_xlsx_compact_dir(string scalar root)
{
    ...
    xfmap = J(0, 1, .)
    styles = _st_xlsx_slurp(root + "/xl/styles.xml")
    styles = _st_xlsx_compact_styles_xml(styles, xfmap)

    // _st_xlsx_compact_styles_xml reports whether any pool actually lost a
    ...
    if (st_local("_st_changed") == "0") return

    _st_xlsx_spit(root + "/xl/styles.xml", styles)
    ...
    sheets = _st_xlsx_files(root + "/xl/worksheets")
    for (i = 1; i <= rows(sheets); i++) {
        path = sheets[i]
        if (substr(path, strlen(path) - 3, 4) != ".xml") continue
        _st_xlsx_spit(path, _st_xlsx_remap_sheet(_st_xlsx_slurp(path), xfmap))
    }
}

string scalar _st_xlsx_compact_styles_xml(string scalar styles,
    real colvector xfmap)
{
    ...
    changed = 0
    st_local("_st_changed", "1")

    pools = ("fonts", "<font", "fonts") \ ("fills", "<fill", "fills") \
        ("borders", "<border", "borders")

    for (i = 1; i <= rows(pools); i++) {
        span = _st_xlsx_block(out, pools[i, 1])
        if (span[1] == 0) continue
        body = substr(out, span[2], span[3] - span[2])
        items = _st_xlsx_pool_items(body, pools[i, 2])
        if (rows(items) == 0) continue

        thismap = J(0, 1, .)
        uniq = _st_xlsx_dedupe(items, thismap)
        if (i == 1) fontmap = thismap
        else if (i == 2) fillmap = thismap
        else bordermap = thismap

        st_local("_st_" + pools[i, 3] + "_before", strofreal(rows(items), "%18.0f"))
        st_local("_st_" + pools[i, 3] + "_after", strofreal(rows(uniq), "%18.0f"))
    ...
    span = _st_xlsx_block(out, "cellStyleXfs")
    if (span[1] != 0) {
        body = substr(out, span[2], span[3] - span[2])
        items = _st_xlsx_pool_items(body, "<xf")
        if (rows(items) > 0) {
            for (i = 1; i <= rows(items); i++) {
                items[i] = _st_xlsx_remap_attr(items[i], "fontId", fontmap)
                items[i] = _st_xlsx_remap_attr(items[i], "fillId", fillmap)
                items[i] = _st_xlsx_remap_attr(items[i], "borderId", bordermap)
            }
            out = substr(out, 1, span[1] - 1) +
    ...
    span = _st_xlsx_block(out, "cellXfs")
    if (span[1] == 0) {
        st_local("_st_changed", changed > 0 ? "1" : "0")
        return(out)
    }
    body = substr(out, span[2], span[3] - span[2])
    xfs = _st_xlsx_pool_items(body, "<xf")
    if (rows(xfs) == 0) {
        st_local("_st_changed", changed > 0 ? "1" : "0")
        return(out)
    }
    for (i = 1; i <= rows(xfs); i++) {
        xfs[i] = _st_xlsx_remap_attr(xfs[i], "fontId", fontmap)
        xfs[i] = _st_xlsx_remap_attr(xfs[i], "fillId", fillmap)
        xfs[i] = _st_xlsx_remap_attr(xfs[i], "borderId", bordermap)
    }
    xfuniq = _st_xlsx_dedupe(xfs, xfmap)
    st_local("_st_formats_before", strofreal(rows(xfs), "%18.0f"))
    st_local("_st_formats_after", strofreal(rows(xfuniq), "%18.0f"))
...
real rowvector _st_xlsx_block(string scalar s, string scalar tag)
{
...
string colvector _st_xlsx_pool_items(string scalar body, string scalar opentag)
{
    string rowvector parts
...
string colvector _st_xlsx_dedupe(string colvector items, real colvector map)
{
...
string scalar _st_xlsx_remap_attr(string scalar item, string scalar attr,
    real colvector map)
{
    real scalar p, q, v
    string scalar key
...
string scalar _st_xlsx_remap_sheet(string scalar sheet, real colvector xfmap)
{
    string rowvector parts
    real scalar i
    ...
    for (i = 1; i <= cols(parts); i++) {
        if (substr(parts[i], 1, 2) == "c ") {
            parts[i] = _st_xlsx_remap_attr(parts[i], "s", xfmap)
        }
        else if (substr(parts[i], 1, 4) == "row ") {
            parts[i] = _st_xlsx_remap_attr(parts[i], "s", xfmap)
        }
        else if (substr(parts[i], 1, 4) == "col ") {
            parts[i] = _st_xlsx_remap_attr(parts[i], "style", xfmap)
        }
    }
    return(invtokens(parts, "<"))
}

string scalar _st_xlsx_slurp(string scalar path)
{
    real scalar fh, n
    string scalar s
...
void _st_xlsx_spit(string scalar path, string scalar s)
{
    real scalar fh
...
// comparison strings with "/".  Left alone, _st_xlsx_check_manifest() then
...
string colvector _st_xlsx_files(string scalar base)
{
    string colvector out, subdirs
    real scalar i

    out = _st_xlsx_slash(dir(base, "files", "*", 1))
    subdirs = _st_xlsx_slash(dir(base, "dirs", "*", 1))
    for (i = 1; i <= rows(subdirs); i++) {
        out = out \ _st_xlsx_files(subdirs[i])
    }
    return(out)
}
...
string colvector _st_xlsx_slash(string colvector paths)
{
    if (rows(paths) == 0) return(paths)
    return(subinstr(paths, "\", "/", .))
}

string scalar _st_xlsx_quoted_files(string scalar base)
{
    string colvector files

    files = _st_xlsx_files(base)
    if (rows(files) == 0) return("")
    return(char(34) + invtokens(files', char(34) + " " + char(34)) + char(34))
}

void _st_xlsx_rmtree(string scalar base)
{
    ...
    for (i = 1; i <= rows(subdirs); i++) _st_xlsx_rmtree(subdirs[i])
    rmdir(base)
}

real scalar _st_xlsx_filesize(string scalar path)
{
    real scalar fh, n
...
void _st_xlsx_check_manifest(string scalar root, string scalar sub,
    string scalar archive)
{
    ...
    for (i = 1; i <= rows(oname); i++) {
        ...
        k = _st_xlsx_filesize(root + "/" + oname[i])
        if (k != _st_xlsx_filesize(prefix + cname[i])) {
...
string colvector _st_xlsx_sheetnames_xml(string scalar root)
{
    string scalar wb
    string rowvector parts
    string colvector out
    ...
    wb = _st_xlsx_slurp(root + "/xl/workbook.xml")
    ...
    for (i = 2; i <= cols(parts); i++) {
        ...
        if (q == 0) continue
        out = out \ _st_xlsx_unescape(substr(parts[i], p, q - 1))
    }
    return(out)
}
...
string scalar _st_xlsx_unescape(string scalar s)
{
    string scalar out
...
void _st_xlsx_verify(string scalar rebuilt, string scalar root)
{
    class xl scalar b
    string colvector sa, sb

    sa = _st_xlsx_sheetnames_xml(root)
