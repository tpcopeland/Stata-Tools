...
```stata
datefix varlist [, newvar(name) drop df(date_format) order(string) topyear(integer)]
```
...
|--------|---------|-------------|
| `newvar(name)` | (none) | Creates a new numeric date variable with the given name. When this option is used, only one variable can be specified in `varlist`. The original string variable is preserved unless `drop` is specified. |
...
| `df(date_format)` | `%tdCCYY/NN/DD` | Specifies the display format for the date. Accepts any valid Stata daily date format starting with `%td` (see examples below). |
...
Convert the string date variables `dob` and `dod` using the default date format (`%tdCCYY/NN/DD`) and whichever ordering of day, month, and year produces the fewest missing values, replacing the original string variables:
...
```stata
datefix admission_date, newvar(admit_dt) drop df(%tdDD/NN/CCYY)
```
