...
{synopt:{opt graphopt:ions(twoway_options)}}additional options passed to graph{p_end}
...
Must be coded as 0 (control) and 1 (treated). Both groups must be present.
...
used values are 95, 99, or 99.5. Cannot be combined with {opt truncate()} or
{opt stabilize}.

{phang}
{opt truncate(#)} truncates weights at the specified maximum value. Weights
above this value are set to this value. Common choices are 10 or 20. Cannot be
combined with {opt trim()} or {opt stabilize}.
...
have better properties. Cannot be combined with {opt trim()} or {opt truncate()}.
...
{phang}
{opt scheme(schemename)} specifies the graph scheme. For example,
{cmd:scheme(plotplainblind)} provides a clean, colorblind-friendly appearance.

{phang}
{opt graphoptions(twoway_options)} specifies additional options passed directly
to the {helpb twoway} command that draws the histogram. Any valid twoway option
may be specified, including {opt note()}, {opt xsize()}, {opt ysize()}, and
...
{phang2}{stata "iptw_diag ipw, treatment(treated)":. iptw_diag ipw, treatment(treated)}{p_end}

{pstd}
{bf:Example 2: With detailed percentiles}

{phang2}{stata "iptw_diag ipw, treatment(treated) detail":. iptw_diag ipw, treatment(treated) detail}{p_end}

{pstd}
{bf:Example 3: Trim at 99th percentile}

{phang2}{stata "iptw_diag ipw, treatment(treated) trim(99) generate(ipw_trim)":. iptw_diag ipw, treatment(treated) trim(99) generate(ipw_trim)}{p_end}

{pstd}
{bf:Example 4: Stabilized weights}

{phang2}{stata "iptw_diag ipw, treatment(treated) stabilize generate(ipw_stab)":. iptw_diag ipw, treatment(treated) stabilize generate(ipw_stab)}{p_end}

{pstd}
{bf:Example 5: With histogram}

{phang2}{stata "iptw_diag ipw, treatment(treated) graph scheme(plotplainblind)":. iptw_diag ipw, treatment(treated) graph scheme(plotplainblind)}{p_end}
