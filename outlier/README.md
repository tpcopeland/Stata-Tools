...
### Outlier Report
![Outlier Report](demo/outlier_report.png)
...
### Detect income outliers in LISA data

```stata
use _examples/lisa.dta, clear
outlier disp_income, report
```

### Detect age outliers within treatment groups

```stata
* After deriving treatment (Prep 1A)
use _examples/cohort.dta, clear
merge 1:1 id using _examples/treatment.dta, nogen keep(match)
outlier index_age, by(treated) report
```

### Winsorize income in LISA data

```stata
use _examples/lisa.dta, clear
outlier disp_income, action(winsorize) generate(w_)
```
...
- **Version 1.0.1** (26 February 2026): Fix xlsx export bug (return matrix moved before export read it); add `set more off`
