...
tvevent <- function(
  ...
  if (n_splits > 0) {
    ...
    # Expand rows that need splitting
    split_rows <- intervals_data %>%
      dplyr::filter(needs_split)
    ...
    if (nrow(split_rows) > 0) {
      ...
      intervals_data <- dplyr::bind_rows(
        intervals_data %>% dplyr::filter(!needs_split) %>% dplyr::select(-needs_split, -dplyr::all_of(date)),
        split_rows_pre %>% dplyr::select(-needs_split, -dplyr::all_of(date)),
        split_rows_post %>% dplyr::select(-needs_split, -dplyr::all_of(date))
      ) %>%
      ...
      # No splits actually needed
      intervals_data <- intervals_data %>%
        dplyr::select(-needs_split, -dplyr::all_of(date))
    }
  }
  ...
  if (type == "single") {
    ...
    intervals_data <- intervals_data %>%
      dplyr::mutate(
        censor_time = ifelse(as.integer(get(generate)) > 0 & event_rank == 1,
