...
# Visit times are generated by THINNING (accept/reject against a homogeneous
# Poisson envelope), which is what the loop below actually does. An earlier
# comment here claimed a fixed visit grid was used instead of thinning; the
# only grid in this file is t_grid, and it exists solely to bound the intensity
# so lambda_max can be chosen. (audit IIVW-11)
all_data <- list()
obs_data <- list()
...
for (i in 1:n) {
    ...
    # Generate visit times via the thinning algorithm.
