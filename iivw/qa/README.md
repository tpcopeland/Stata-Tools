...
- `validation_iivw_inference.do` — release-only nested coverage simulation. It is intentionally excluded from `run_all.do` because release mode requires at least 1,000 outer simulations with 999 inner bootstrap draws; run it explicitly. `_skip.txt` records the exclusion so it cannot masquerade as ordinary lane coverage. The `n=600`/`n=1200` FIPTIW diagnostic record is `coverage_results/FIPTIW_NSCALE_2026-07-23.md`.
