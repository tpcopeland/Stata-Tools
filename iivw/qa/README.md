...
- `test_iivw_v341_regressions.do` — inference-evidence boundary fixed in 3.4.1. Bare FIPTIW stays point-only even at 600 clusters because the R=200 stacked study explicitly classified itself as a diagnostic screen rather than a release gate. Covers Gaussian/identity, binomial/logit, and Poisson/log, preserves explicit `vce(stacked)` with its `uncleared-stacked-analytic` stamp, checks the visible point-only message, and requires the README syntax table to list the explicit stacked method. T1/T2/T4/T5 fail on 3.4.0; T3 is the positive control.
