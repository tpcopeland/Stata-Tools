...
| 11 | **Corrected variance** | Coverage simulation ≥1,000 reps; **and** a fixed-weight arm that demonstrably **fails** a scenario the corrected method passes | 2 | ⚠️ **HARNESS BUILT, RELEASE RUN PENDING** — `validation_iivw_inference.do` implements the nested driver (three DGP families, refit vs fixed vs fixed-weight bootstrap, Wilson intervals, `smoke`/`inference`/`release` modes with a non-gate sentinel). The ≥1000×999 release run has **not** executed (it takes days); only a reduced smoke pilot has. **Still the release blocker for the inference CLAIM**, though the refit bootstrap is now the honest *default* (stamped `candidate`, never `cleared`) |
...
remaining tier-1 asserts to grow. **The one still-open release blocker is #11 (corrected-variance
coverage): the harness exists but the ≥1000×999 run has not been executed.** The refit bootstrap is the
honest default in the meantime, stamped `candidate` (never `cleared`) so no run misrepresents itself.
