...
* sim_scenarios_abc.do — Scenarios A, B, and C from the IIVW manuscript
*
* Three simulation scenarios demonstrating when IIVW does and does not work:
...
*
* DGP:
*   N subjects with binary treatment confounded by conf_ti (observed, in
*   the outcome model) and u_i (latent, NOT in the outcome model).
*   u_i drives visit frequency (Scenarios A, C) and enters the outcome,
*   creating confounding bias that standard regression cannot remove.
*   IIW addresses visit-frequency overrepresentation (u_i enters the
*   visit model through conf_tv proxy). FIPTIW adds IPTW for treatment
*   confounding (treat_cov includes conf_ti and u_i).
*
*   Scenario B: protocol-driven visits (no u_i dependence). Artifact
*   adds 1.5*log(test_n + 1) to each measurement.
*
* Usage:
...
*
* See: iivw_manuscript (Section 4), Table 2
...
program define _sim_generate
    ...
    * Treatment confounded by conf_ti (observed) AND u_i (latent)
    ...
    * Visit process
    if "`scenario'" == "A" | "`scenario'" == "C" {
        * Informative visits: u_i drives visit frequency
        ...
        * Scenario B: protocol-driven visits
...
foreach scenario in A B C {
    ...
    forvalues s = 1/`n_sims' {
        ...
        quietly {
            ...
            * Estimator 4: FIPTIW + cumulative test count
            if `has_artifact' {
                capture iivw_weight, id(id) time(months) ///
                    visit_cov(u_i conf_tv) ///
                    treat(treatment) treat_cov(conf_ti u_i) ///
    ...
    * === Summarize ===
    ...
    display _n "{hline 72}"
    display "Scenario `scenario': Results"
