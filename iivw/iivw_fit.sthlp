...
With {cmd:model(gee)} and {cmd:bootstrap(0)}, this is passed to
{cmd:vce(cluster ...)} in {cmd:glm}.  With {cmd:model(mixed)} and
{cmd:bootstrap(0)}, it is passed to {cmd:vce(cluster ...)} in {cmd:mixed}.
...
subject level.  Note: the bootstrap treats the IIW/IPTW weights as fixed
...
{phang}
{opt gee:opts(string)} passes additional options to {cmd:glm}.

{phang}
{opt mixed:opts(string)} passes additional options to {cmd:mixed}.
...
then export with {helpb regtab} (from the {cmd:tabtools} package; install
