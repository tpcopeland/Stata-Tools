...
| **IIVW-B02** | Default variance treats estimated weights as known | B&L p.10–11; Coulombe PDF p.86 | #1 — **open, blocking.** Phase 3 made it *visible*, not correct: `e(iivw_vce)` records which of the three variances was computed, the fixed-weight path says so where the SE is read, and an incomplete bootstrap is now `r(430)` rather than a quietly-shrunk draw set. The default is still the fixed-weight sandwich, and **no coverage simulation has been run against a preregistered gate**. Test: `test_iivw_inference_contract` |
