/*  demo_iivw.do - Demo output for iivw
    ...
    Produces:
      1. Console workflow output -> batch log
...
local demo_plus "`c(sysdir_plus)'"
local demo_personal "`c(sysdir_personal)'"
local orig_scheme "`c(scheme)'"
local demo_data_touched = 0
...
capture noisily {
    ...
    * # Step 6: exogeneity check and diagnostic decomposition
    ...
    local exo "exogenous"
    if r(history_association_flag) local exo "endogenous"
...
**# Cleanup
* Cleanup is outside the captured work so every failure restores caller state.
capture restore
capture graph close _all
capture log close _all
capture sysdir set PLUS "`demo_plus'"
local cleanup_rc = _rc
if `demo_rc' == 0 & `cleanup_rc' != 0 local demo_rc = `cleanup_rc'
capture sysdir set PERSONAL "`demo_personal'"
local cleanup_rc = _rc
if `demo_rc' == 0 & `cleanup_rc' != 0 local demo_rc = `cleanup_rc'
capture set scheme `orig_scheme'
local cleanup_rc = _rc
if `demo_rc' == 0 & `cleanup_rc' != 0 local demo_rc = `cleanup_rc'
