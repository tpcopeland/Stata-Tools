...
program define _spaghetti_mean
    ...
    * Apply smoothing if requested (case-insensitive)
    local smooth = lower("`smooth'")
        ...
        if "`by'" != "" {
            * Use numeric group for iteration
            ...
            forvalues g = 1/`n_grps' {
                ...
                if "`smooth'" == "lowess" {
                    tempvar _lw
                    quietly lowess _spag_mean_y `time' if `_grp' == `g', ///
                        gen(`_lw') nograph
            ...
            if r(N) >= 3 {
                if "`smooth'" == "lowess" {
                    tempvar _lw
                    quietly lowess _spag_mean_y `time', gen(`_lw') nograph
