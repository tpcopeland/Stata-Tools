...
quietly net install spaghetti, from("`pkg_dir'")
...
* =============================================================================
* v1.0.1 FIX COVERAGE TESTS
* =============================================================================

* TEST 51: highlight() invalid token error
local ++test_count
local test_desc "v1.0.1: highlight() invalid token produces error"
_run_test `test_count' "`test_desc'"
...
if `run_only' == 0 | `run_only' == `test_count' {
    ...
    capture {
        capture noisily spaghetti sdmt, id(patid) time(months) ///
            highlight(nonexistent_variable_xyz)
        assert _rc == 198
    }
    if _rc == 0 {
        local ++pass_count
...
* TEST 52: colorby() missing values excluded
local ++test_count
local test_desc "v1.0.1: colorby() excludes missing values"
_run_test `test_count' "`test_desc'"

if `run_only' == 0 | `run_only' == `test_count' {
    _make_panel_data 50 10 12345
    * Introduce missings in bl_sdmt for patid 1-5
    quietly replace bl_sdmt = . if patid <= 5
    local N_with_miss = _N
    capture {
        spaghetti sdmt, id(patid) time(months) colorby(bl_sdmt)
        * r(N) should exclude rows with missing bl_sdmt
        assert r(N) < `N_with_miss'
        * Only 45 individuals should remain (50 minus 5 with missing)
        assert r(n_ids) == 45
    }
    if _rc == 0 {
        local ++pass_count
...
* TEST 53: colorby() + mean() combined
local ++test_count
local test_desc "v1.0.1: colorby() + mean() combined"
_run_test `test_count' "`test_desc'"
...
* TEST 54: colorby() + individual(opacity()) respected
local ++test_count
local test_desc "v1.0.1: colorby() respects individual(opacity())"
_run_test `test_count' "`test_desc'"
...
if `run_only' == 0 | `run_only' == `test_count' {
    ...
    capture {
        spaghetti sdmt, id(patid) time(months) colorby(bl_sdmt) ///
            individual(opacity(50))
        * Check the graph command contains %50 (not hardcoded %40)
        assert strpos(`"`r(cmd)'"', "%50") > 0
    }
    if _rc == 0 {
        local ++pass_count
        if `machine' {
...
* TEST 55: lowess smoothing runs without extra output
local ++test_count
local test_desc "v1.0.1: mean(smooth(lowess)) runs cleanly"
_run_test `test_count' "`test_desc'"
...
* TEST 56: lowess smoothing with by()
local ++test_count
local test_desc "v1.0.1: lowess + by() combined"
_run_test `test_count' "`test_desc'"
...
* TEST 57: Data preservation after colorby with missing
local ++test_count
local test_desc "v1.0.1: Data preserved after colorby with missing"
_run_test `test_count' "`test_desc'"

if `run_only' == 0 | `run_only' == `test_count' {
    _make_panel_data 50 10 12345
    quietly replace bl_sdmt = . if patid <= 3
    local N_before = _N
    local k_before = .
    describe, short
    local k_before = r(k)
...
* TEST 58: varabbrev restored after colorby missing error
local ++test_count
local test_desc "v1.0.1: varabbrev restored after all-missing colorby"
_run_test `test_count' "`test_desc'"

if `run_only' == 0 | `run_only' == `test_count' {
    _make_panel_data 50 10 12345
    quietly replace bl_sdmt = .
    local va_before = c(varabbrev)
    capture noisily spaghetti sdmt, id(patid) time(months) colorby(bl_sdmt)
    capture {
        assert "`va_before'" == c(varabbrev)
    }
    if _rc == 0 {
