/*  demo_spaghetti.do - Generate screenshots for spaghetti v1.0.1

    Produces 8 output types:
      1. Console output (return values)             -> .smcl
      2. Graph: by-group with mean + CI             -> basic_by_mean.png
      ...
      6. Graph: reference line                       -> refline.png
      7. Graph: custom styling                       -> custom_style.png
      8. Graph: lowess smoothing with by-group       -> smooth_lowess.png
*/
...
spaghetti sdmt, id(patid) time(months) by(treatment) ///
    highlight(patid==5 | patid==55 bgopacity(10)) ///
    mean(bold) ///
...
spaghetti sdmt, id(patid) time(months) by(treatment) ///
    mean(bold ci) sample(40) seed(42) ///
    refline(6, label("Mid-study") style(dash)) ///
    title("Treatment Trajectories with Mid-Study Marker") ///
    name(demo_refline)
graph export "`pkg_dir'/refline.png", replace width(1200)
capture graph close _all

* --- 7. Custom styling ---
spaghetti sdmt, id(patid) time(months) by(treatment) ///
    individual(color(gs14) opacity(15) lwidth(vthin)) ///
    mean(bold ci) colors(dknavy cranberry) ///
    title("Custom Styled Trajectories") ///
    name(demo_custom)
graph export "`pkg_dir'/custom_style.png", replace width(1200)
capture graph close _all

* --- 8. Lowess-smoothed mean with by-group ---
spaghetti sdmt, id(patid) time(months) by(treatment) ///
    mean(bold smooth(lowess) ci) sample(40) seed(42) ///
    title("Smoothed Mean Trajectories (Lowess)") ///
    name(demo_lowess)
graph export "`pkg_dir'/smooth_lowess.png", replace width(1200)
capture graph close _all
