...
program define cstat_surv, eclass
    ...
    capture noisily {
        ...
        * Set confidence level (default: c(level))
        ...
        * Generate predictions
        quietly {
            ...
            if _rc {
                noisily display as error "Failed to predict from Cox model"
                exit 322
            }
            * Exclude observations with missing predictions
            markout `touse' `hrs'
        ...
        * Warn if original model used weights
        if "`wtype'" != "" {
            display as text ///
                "note: weights from stcox model are not used in" ///
        ...
        * Check for comparable pairs
        if r(N_comparable) == 0 {
            display as error ///
                "no comparable pairs found; check that events" ///
                " exist in the data"
            exit 2001
        }
        ...
        * Post results
        ...
        ereturn scalar somers_d = `somers_d'
        ...
        * Display results
        ...
        display as text "Number of comparable pairs" _col(35) "= " as result %10.1fc `N_comp'
        display as text "  Concordant pairs" _col(35) "= " as result %10.1fc `N_conc'
        display as text "  Discordant pairs" _col(35) "= " as result %10.1fc `N_disc'
        display as text "  Tied pairs" _col(35) "= " as result %10.1fc `N_tied'
        display
        display as text "{hline 80}"
        display as text _col(35) "Coef." _col(46) "Std. Err." _col(58) "[`level'% Conf. Interval]"
        display as text "{hline 80}"
