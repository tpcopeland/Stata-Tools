...
{phang2}{stata "validate mpg, range(10 50) nomiss":. validate mpg, range(10 50) nomiss}{p_end}

{pstd}
{bf:Example 2: Check categorical values}

{phang2}{stata "validate foreign, values(0 1)":. validate foreign, values(0 1)}{p_end}

{pstd}
{bf:Example 3: Check multiple variables}

{phang2}{stata "validate price mpg weight, nomiss":. validate price mpg weight, nomiss}{p_end}

{pstd}
{bf:Example 4: Check uniqueness}

{phang2}{stata "webuse nlswork, clear":. webuse nlswork, clear}{p_end}
{phang2}{stata "validate idcode, unique":. validate idcode, unique}{p_end}

{pstd}
{bf:Example 5: Cross-variable validation}

{phang2}{stata "validate price mpg, cross(price > 0 & mpg > 0)":. validate price mpg, cross(price > 0 & mpg > 0)}{p_end}

{pstd}
{bf:Example 6: Generate validation indicator}

{phang2}{stata "validate price mpg, range(0 50000) generate(valid)":. validate price mpg, range(0 50000) generate(valid)}{p_end}
{phang2}{stata "tab valid":. tab valid}{p_end}

{pstd}
{bf:Example 7: Assert on failure (stop execution)}

{phang2}{stata "validate foreign, values(0 1) assert":. validate foreign, values(0 1) assert}{p_end}

{pstd}
{bf:Example 8: Export to Excel}

{phang2}{stata "validate price mpg weight, nomiss xlsx(validation.xlsx)":. validate price mpg weight, nomiss xlsx(validation.xlsx)}{p_end}
...
{phang2}{stata "validate study_entry study_exit, cross(study_entry <= study_exit)":. validate study_entry study_exit, cross(study_entry <= study_exit)}{p_end}
{phang2}{stata "validate study_entry death_date, cross(death_date >= study_entry | missing(death_date))":. validate study_entry death_date, ///}{p_end}
{phang2}{cmd:.     cross(death_date >= study_entry | missing(death_date))}{p_end}
