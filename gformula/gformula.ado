...
*!   - Merged gformula_.ado into single file (_gformula_bootstrap)
...
*!   - Eliminated global macro pollution (`_gf_maxid', $check_*, `_gf_almost')
...
program define gformula, eclass
version 16.0
set varabbrev off
set more off
...
if "`mediation'"=="" {
	...
	if "`death'"!="" {
		...
		_gformula_detangle "`commands'" command "`varlist2'"
		forvalues i=1/`nvar' {
			if "${S_`i'}"!="" {
...
if "`impute'"!="" {
	...
	_gformula_detangle "`imp_eq'" imp_eq "`impute'"
...
* _gformula_detangle equations
_gformula_detangle "`equations'" equation "`varlist2'"
forvalues i=1/`nvar' {
	if "${S_`i'}"!="" {
...
forvalues i=1/`nvar' {
	...
	_gformula_formatline, n(`eq') maxlen(`longstring')
...
if "`impute'"!="" {
	* Display in a table the parametric models that have been specified for imputation
	local imp_nvar: word count `impute'
	* _gformula_detangle imputation commands
	_gformula_detangle "`imp_cmd'" imp_cmd "`impute'"
	forvalues i=1/`imp_nvar' {
		if "${S_`i'}"!="" {
			local imp_cmd`i' ${S_`i'}
		}
	}
	* _gformula_detangle imputation equations
	_gformula_detangle "`imp_eq'" imp_eq "`impute'"
	forvalues i=1/`imp_nvar' {
		if "${S_`i'}"!="" {
			local imp_eq`i' ${S_`i'}
	...
	forvalues i=1/`imp_nvar' {
		...
		_gformula_formatline, n(`imp_eq_disp') maxlen(`longstring')
		local nlines=r(lines)
		forvalues j=1/`nlines' {
...
local _gf_check_delete = 0
local _gf_check_print = 0
local _gf_check_save = 0
...
if "`mediation'"=="" {
	...
	local i=`i'-1
	local _gf_almost_varlist ""
	forvalues j=1(1)`i' {
		local _gf_almost_varlist "`_gf_almost_varlist' `bit_`j''"
	}
	qui reshape wide `_gf_almost_varlist', i(`idvar') j(`tvar')
	qui gen `tvar'=.
	foreach pastvar of local _gf_almost_varlist {
		qui gen `pastvar'=.
	}
}
_gformula_bootstrap `varlist' `if' `in', out(`outcome') com(`commands') eq(`equations') i(`idvar') t(`tvar') ///
	...
	_gf_maxid(`maxid') _gf_chk_del(`_gf_check_delete') _gf_chk_prt(`_gf_check_print') _gf_chk_sav(`_gf_check_save') _gf_almost(`_gf_almost_varlist')

local _gf_check_delete `_gf_check_delete'
local _gf_check_print `_gf_check_print'
local _gf_check_save `_gf_check_save'
...
bootstrap `_b' `_po' `_cinc', reps(`samples') `bca' noheader nolegend notable nowarn: _gformula_bootstrap `varlist' `if' `in', ///
...
if "`msm'"!="" {
	...
	if "`all'"=="" {
		...
		forvalues i=1/`r1' {
			...
			_gformula_display_stats, est(`=b[1,`i']') se(`=se[1,`i']') ci_lo(`=ci_normal[1,`i']') ci_hi(`=ci_normal[2,`i']')
		}
		...
		forvalues i=1/`r1' {
			local w=14-length(abbrev("`colname`i''",12))
			noi di as text _col(`w') abbrev("`colname`i''",12) _col(15) "{c |}" _cont
			_gformula_display_stats, est(`=b[1,`i']') se(`=se[1,`i']') ci_lo(`=ci_normal[1,`i']') ci_hi(`=ci_normal[2,`i']') continue
			noi di as text "   (N)"
...
if "`mediation'"=="" {
	...
	* tokenize interventions
	...
	if "`obe'"=="" & "`linexp'"=="" {
		_gformula_detangle "`baseline'" baseline "`exposure'"
		forvalues i=1/`nbase' {
			if "${S_`i'}"!="" {
    ...
    if "`control'"!="" {
        _gformula_detangle "`control'" control "`mediator'"
        forvalues i=1/`nmed' {
        	if "${S_`i'}"!="" {
	...
	if "`specific'"!="" {
		...
		_gformula_detangle "`alternative'" alternative "`exposure'"
		forvalues i=1/`nbase' {
			if "${S_`i'}"!="" {
	...
	if "`all'"=="" {
        ...
        forvalues i=1/`maxrowtab' {
			forvalues j=1/`nexplev' {
				...
				if "`oce'"=="" {
					_gformula_display_stats, est(`=b[1,`i']') se(`=se[1,`i']') ci_lo(`=ci_normal[1,`i']') ci_hi(`=ci_normal[2,`i']') se_col(37) p_col(57)
				}
				else {
					qui tab `exposure', matrow(_matrow)
					local nexplev=r(r)-1
					local ii=(`i'-1)*`nexplev'+`j'
					_gformula_display_stats, est(`=b[1,`ii']') se(`=se[1,`ii']') ci_lo(`=ci_normal[1,`ii']') ci_hi(`=ci_normal[2,`ii']') se_col(36) p_col(57)
        ...
        forvalues i=1/`maxrowtab' {
			forvalues j=1/`nexplev' {
				...
				if "`oce'"=="" {
					_gformula_display_stats, est(`=b[1,`i']') se(`=se[1,`i']') ci_lo(`=ci_normal[1,`i']') ci_hi(`=ci_normal[2,`i']') se_col(36) p_col(57) continue
					noi di as text "   (N)"
					...
					local nexplev=r(r)-1
					local ii=(`i'-1)*`nexplev'+`j'
					_gformula_display_stats, est(`=b[1,`ii']') se(`=se[1,`ii']') ci_lo(`=ci_normal[1,`ii']') ci_hi(`=ci_normal[2,`ii']') se_col(36) p_col(57) continue
					noi di as text "   (N)"
					noi di as text _col(15) "{c |}" _cont
...
* Clean up _gformula_detangle globals
forvalues _gf_i = 1/50 {
	global S_`_gf_i'
}
...
program define _gformula_bootstrap, rclass
version 16.0
set varabbrev off
set more off
syntax varlist(min=2 numeric) [if] [in] , OUTcome(varname) COMmands(string) EQuations(string) [Idvar(varname) ///
	...
	_gf_maxid(integer 0) _gf_chk_del(integer 0) _gf_chk_prt(integer 0) _gf_chk_sav(integer 0) _gf_almost(string)]
preserve
...
if "`mediation'"=="" {
	...
	drop `_gf_almost'
	drop `tvar'
	qui reshape long `_gf_almost', i(`idvar') j(`tvar')
	qui tab `tvar', matrow(matvis)
	local maxv=rowsof(matvis)
	local maxvlab=matvis[`maxv',1]
	local firstv=matvis[1,1]
...
if "`impute'"!="" {
	...
	* _gformula_detangle imputation equations
	...
	if "`mediation'"=="" {
		*lagged variables
		* _gformula_detangle lag rules
		local nlag: word count `laggedvars'	
    	_gformula_detangle "`lagrules'" lagrule "`laggedvars'"
    	forvalues i=1/`nlag' {
	...
	*derived variables
	* _gformula_detangle derivation rules
	local nder: word count `derived'	
	if `nder'>0 {
		_gformula_detangle "`derrules'" derrule "`derived'"
		forvalues i=1/`nder' {
					...
					if `visitcalc`i'_`j''==1 {
						...
						if "`imp_cmd`i''"=="mlogit" | "`imp_cmd`i''"=="ologit" {
							...
							forvalues l=1/`maxl' {
								...
								capture drop _gf_pred_imp`i'_`l'
							}
							qui predict _gf_pred_imp`i'_1'-_gf_pred_imp`i'_`maxl'
						}
						else {
						...
						if "`imp_cmd`i''"=="mlogit" | "`imp_cmd`i''"=="ologit" {
							...
							forvalues l=1/`maxl' {
								qui replace `imp_var`i''=`out_mlogit_`l'' if `u_for_mlogit'>=`cumulative_pred' & `u_for_mlogit'<(`cumulative_pred'+_gf_pred_imp`i'_`l') & `imp_imp_var`i''==. & `tvar'==`k' 
								qui replace `cumulative_pred'=`cumulative_pred'+_gf_pred_imp`i'_`l'
							}
						}
			...
			* fit parametric models and impute according to parameter estimates
			forvalues i=1/`imp_nvar' {
				...
				if "`imp_cmd`i''"=="mlogit" | "`imp_cmd`i''"=="ologit" {
					...
					qui gen double `cumulative_pred'=0
					forvalues l=1/`maxl' {
						qui replace `imp_var`i''=`out_mlogit_`l'' if `u_for_mlogit'>=`cumulative_pred' & `u_for_mlogit'<(`cumulative_pred'+_gf_pred_imp`i'_`l') & `imp_imp_var`i''==.
						qui replace `cumulative_pred'=`cumulative_pred'+_gf_pred_imp`i'_`l'
					}
				}
				if "`imp_cmd`i''"!="regress" & "`imp_cmd`i''"!="logit" & "`imp_cmd`i''"!="mlogit" & "`imp_cmd`i''"!="ologit" {
...
*****************************************************************************************************************************************************************************
						if "`command`i''"=="logit" | "`command`i''"=="regress" {
							...
							cap drop _gf_p*
							qui predict _gf_p1-_gf_p`maxcat'
						}
*****************************************************************************************************************************************************************************
						if "`command`i''"=="logit" {
...
*****************************************************************************************************************************************************************************
						if ("`command`i''"=="mlogit" | "`command`i''"=="ologit") {
							...
							forvalues cat=1(1)`maxcat' {
								...
								qui replace `cum_p2'=`cum_p2'+_gf_p`cat'
...
c_local _gf_check_delete `_gf_chk_del'
c_local _gf_check_print `_gf_chk_prt'
c_local _gf_check_save `_gf_chk_sav'
...
capture program drop _gformula_display_stats
program define _gformula_display_stats
version 16.0
set varabbrev off
set more off
...
program define _gformula_detangle
version 16.0
set varabbrev off
...
forvalues i=1/`ncl' {
	...
	while "`1'"!="" {
		* Inlined chkin logic
		local _gf_k: list posof "`1'" in rhs
		if `_gf_k' == 0 {
...
program define _gformula_formatline, rclass
version 16.0
set varabbrev off
