...
PREREQUISITES:
	Run gformula first. gformtab reads from e() results:
	- e(tce), e(nde), e(nie), e(pm), e(cde) - point estimates
	- e(se_tce), e(se_nde), e(se_nie), e(se_pm), e(se_cde) - standard errors
	- e(ci_normal), e(ci_percentile), e(ci_bc), e(ci_bca) - CI matrices
	- e(cmd) must equal "gformula", e(analysis_type) must equal "mediation"
...
quietly {
	...
	* Check if gformula mediation results exist in e()
	if "`e(cmd)'" != "gformula" | "`e(analysis_type)'" != "mediation" {
		noisily display as error "No gformula mediation results found in e()"
		noisily display as error "Run {bf:gformula} with {bf:mediation} option first"
		exit 119
	}

	* Check for OCE mediation type (not supported — returns tce_1 etc. instead of tce)
	if "`e(mediation_type)'" == "oce" {
		noisily display as error "gformtab does not support OCE mediation results"
		noisily display as error "OCE returns level-specific effects (tce_1, nde_1, etc.)"
		exit 198
	}
	...
	* Point estimates from e() convenience scalars
	local tce = e(tce)
	local nde = e(nde)
	local nie = e(nie)
	local pm = e(pm)
	capture local cde = e(cde)
	if _rc != 0 local cde = .

	* Standard errors from e(se) matrix by column name
	tempname se_mat
	capture matrix `se_mat' = e(se)
	if _rc != 0 {
		noisily display as error "SE matrix not found in e(se)"
		exit 111
	}
	local se_tce = `se_mat'[1, colnumb(`se_mat', "tce")]
	local se_nde = `se_mat'[1, colnumb(`se_mat', "nde")]
	local se_nie = `se_mat'[1, colnumb(`se_mat', "nie")]
	local se_pm = `se_mat'[1, colnumb(`se_mat', "pm")]
	if colnumb(`se_mat', "cde") != . {
		local se_cde = `se_mat'[1, colnumb(`se_mat', "cde")]
	}
	else {
		local se_cde = .
	}

	* Get confidence intervals from e() matrix
	tempname ci_mat
	capture matrix `ci_mat' = e(ci_`ci')
	if _rc != 0 {
		noisily display as error "CI matrix e(ci_`ci') not found"
		noisily display as error "Available CI types depend on gformula bootstrap options"
		exit 111
	}

	* Extract CI bounds by column name
	* gformula CI matrices are 2 x N: row 1 = lower, row 2 = upper
