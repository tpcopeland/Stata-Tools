...
{pstd}Required: Run {cmd:gformula} first. {cmd:gformtab} reads results from {cmd:e()} scalars and matrices.{p_end}
...
{p 4 8 2}Run {cmd:gformula} with the {cmd:mediation} option before using {cmd:gformtab}. The {cmd:oce} mediation type is not supported; {cmd:gformtab} handles standard mediation results ({cmd:obe}, {cmd:linexp}, {cmd:specific}, or baseline-based). The command reads:{p_end}
...
{phang2}{cmd:. * Run gformula: SNRI vs SSRI → adherence → CV event}{p_end}
{phang2}{cmd:. gformula cv_event adherence treated index_age female, ///}{p_end}
