...
psdash is a command family for propensity-score overlap, covariate balance, weight stability, and common-support diagnostics. It can read supported estimation or dataset contracts automatically, or work from explicitly supplied treatment, propensity-score, and weight variables.
...
combined runs overlap, balance, weight, and support panels on one common complete-case sample and returns an overall PASS or FAIL verdict with the analysis-sample ledger.
...
| combined | r(verdict), r(n_warnings), r(warnings), r(n_panels), r(N_requested), r(N_analysis), r(n_common_excluded), r(overlapmax), r(essmin), r(imbalmax), r(report), and source/estimand metadata. |
| detect | r(source), r(treatment), r(psvar), r(covariates), r(wvar), r(estimand), r(n_covariates), r(multigroup), r(longitudinal), r(K), r(levels), and r(reference). |
...
- **v1.3.0** (14 Jun 2026): Added psdash detect, combined, dryrun, machine-readable combined verdicts and thresholds, report(), multi-strategy Love plots, distributional balance plots, smdmatrix(), support, compare, and Excel export parity.
...
- **v1.0.2** (17 May 2026): Rejected invalid manual estimands, added multi-group treatment-level validation, isolated remaining QA installs, and made demo path handling relocatable with failure-safe cleanup.
