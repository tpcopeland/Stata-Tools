...
Balance reporting is deliberately richer than the `tebalance summarize` default. `psdash balance` computes raw and weighted standardized mean differences, variance ratios, Kolmogorov-Smirnov statistics, and a Love plot sorted by absolute SMD, with configurable thresholds and Excel export. Two-level covariates use the Bernoulli population variance `(b-a)^2 p(1-p)`; continuous adjusted variance ratios use the scale-invariant unbiased weighted variance, so multiplying every weight by a constant does not change the result. When a PS is available, it auto-generates IPTW weights for the requested `estimand()` (default ATE) and displays adjusted columns alongside raw columns, so the user sees immediately how much weighting resolves any imbalance — with `nowvar` to suppress weighting and `wvar()` to supply a pre-computed weight variable. Factor and interaction notation (`i.var`, `c.var`, `##`) is expanded into the fitted design columns — one indicator per non-base level, one product per interaction cell — whether supplied through `covariates()` or auto-detected from a fitted `logit`/`probit`/`mlogit`/`teffects` model. Balance is assessed on those design columns rather than on the underlying integer category codes, so categorical and joint-distribution imbalance is not hidden.

The weights subcommand is the complement. `psdash weights` reports mean, SD, range, percentiles, effective sample size, and extreme-weight counts, with on-the-fly `trim(#)`, `truncate(#)`, and `stabilize` modifications exposed through `generate(name)` so the modified weights are kept as a new variable rather than overwriting the original. `psdash support` assesses binary common support via manual thresholds or the Crump et al. (2009) optimal-trimming rule. For multiple treatments, `threshold()` retains a unit only when every component of its generalized propensity-score vector meets the requested practical-positivity floor. Multi-group overlap/support graphs show one panel per GPS component, comparing that same component across every observed treatment group; `r(gps_means)` stores the corresponding K-by-K table. The older own-arm-score range remains descriptive for compatibility but never drives the multi-group verdict. All subcommands store results in `r()` and the dashboard output lines use clear status labels plus a "Consider:" action line when follow-up is warranted.
...
|------------|--------------------|--------|
| `overlap` | `r(N)`, `r(overlap_lower)`, `r(overlap_upper)`, `r(n_outside)`, `r(pct_outside)`, `r(auc)`, `r(treatment)`, `r(psvar)`, `r(source)` | multi-group: `r(gps_means)` |
...
| `support` | `r(lower_bound)`, `r(upper_bound)`, `r(n_outside)`, `r(pct_outside)`, `r(trim_lower)`, `r(trim_upper)`, `r(n_trimmed)`, `r(N_remaining)`, `r(crump_alpha)`, `r(source)`; with `compare`: `r(*_pre)`/`r(*_post)` | multi-group: `r(gps_means)` |
...
- **v1.6.1** (29 Jul 2026): Method and efficiency correction. Binary balance now uses `(b-a)^2 p(1-p)` rather than Stata's sample-variance inflation, and adjusted continuous variance ratios use the scale-invariant unbiased weighted variance instead of normalized `aweight` variance. Exact PS boundaries now enter the machine-readable findings contract, and `balance` rejects undefined auto-generated weights rather than silently dropping those rows. Multi-treatment overlap/support now compare every GPS component across every observed treatment group, return the K-by-K `r(gps_means)` table, and reserve the legacy observed-arm scalarization for descriptive output only. Balance reuses sorted empirical CDFs and reference-group summaries; Crump trimming uses one sorted cumulative-sum pass rather than repeated full-data scans. QA adds old-code-failing method regressions, direct parity against R `cobalt`, corrected Python/known-answer oracles, strict bootstrap propagation, and arithmetic `RESULT:` contracts.
- **v1.6.0** (26 Jul 2026): Independent-audit remediation. **Fixes a silent propensity-score inversion**: with a binary 0/1 treatment, `psvars()` took its first element as the propensity score, but under the package's documented ascending-level convention that element is P(A=0|X), so every binary weight and diagnostic ran on an inverted score at `rc=0`. `psvars()` for a binary treatment now maps by treatment level, validates range and sum-to-1, and rejects more than two variables. Multi-group `psdash overlap` now reports the full-vector generalized-positivity block (`r(min_gps)`, `r(n_gps_violate)`, `gpsfloor()`) and raises a finding on it, instead of judging multi-arm overlap on the observed-arm min-max rule alone — the exact failure it previously reported as "Good". `stabilize` is refused for auto-generated `estimand(att)`/`estimand(atc)` weights, which are not on the 1/PS scale the ATE stabilization formula assumes. Untrimmed multi-group `support, generate()` now marks the GPS-positivity region rather than the observed-arm interval the same output labels informational. Multi-group trimming is reported and returned as the one-sided GPS floor it is (`r(trim_upper)` no longer returned for multi-group). Longitudinal result matrices are keyed by period value rather than by position. `balance` now names covariates with missing values and reports `r(n_cov_incomplete)`/`r(n_cov_min)`. `psdash combined` gains `gpsfloor()` and forwards it to both the overlap and support panels. **Corrects a citation error**: the help attributed the unweighted-pooled-SD SMD denominator to Austin and Stuart (2015), whose section 4.1.1 in fact replaces *each* sample estimate — variances included — with its weighted equivalent. The common-scale denominator is the `cobalt` convention; psdash keeps it deliberately (it makes `strategies()` overlays comparable) and now documents the divergence instead of miscrediting it. No numbers changed. Help corrections to `r(n_extreme)` (lower cutoff, not upper), combined `r(n_warnings)` (total findings, not flagged panels), and the fixed max/mean dominance threshold. Added `test_rb12_kimi_audit.do` (16 regressions, 14 of which fail on 1.5.0).
