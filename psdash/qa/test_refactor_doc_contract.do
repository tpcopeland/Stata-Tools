...
local ++test_count
display as text _n "--- D1: README presents demo graph images, not embedded console transcripts ---"
capture noisily {
    * Demo output is graph images generated by demo_psdash.do; console
    * transcripts are no longer embedded or linked (curated console markdown
    * was retired, see README Version History).
    assert strpos(fileread("`pkg_dir'/README.md"), "Demo output is generated from") > 0
    assert strpos(fileread("`pkg_dir'/README.md"), "demo/overlap_density.png") > 0
    assert strpos(fileread("`pkg_dir'/README.md"), "demo/mg_overlap_density.png") > 0
