...
{synopt:{opt boxo:pts(string)}}pass-through options to the box whisker line{p_end}
...
{synopt:{opt vert:ical}}vertical orientation{p_end}
...
{synopt:{opt plotr:egion(string)}}plot region options{p_end}
{synopt:{opt graphr:egion(string)}}graph region options{p_end}
...
{pstd}
Only one numeric variable may be specified. To compare multiple variables,
{cmd:reshape long} the data and use {opt over()} to distinguish them.
...
placed at positions 1*gap, 2*gap, 3*gap, etc. Default is 1.0. Increase to
add more space between groups; the cloud and box widths remain fixed.
...
{pstd}
{bf:Example 9: Element styling via pass-through options}

{phang2}{stata `"raincloud mpg, over(foreign) cloudopts(lwidth(medium)) pointopts(msymbol(d) msize(tiny)) boxopts(lwidth(thick))"':. raincloud mpg, over(foreign) cloudopts(lwidth(medium)) pointopts(msymbol(d) msize(tiny)) boxopts(lwidth(thick))}{p_end}
