...
* Single source of truth generated by data/generate_r_benchmarks.R.
preserve
import delimited using "`qa_dir'/data/r_benchmarks.csv", clear varnames(1) asdouble
forvalues i = 1/`=_N' {
    local key = metric[`i']
    local `key' = value[`i']
}
...
if `fail_count' > 0 {
    display "RESULT: crossval_gcomp tests=`test_count' pass=`pass_count' fail=`fail_count' status=FAIL"
    display as error "FAIL"
    exit 1
}
else {
    display "RESULT: crossval_gcomp tests=`test_count' pass=`pass_count' fail=`fail_count' status=PASS"
