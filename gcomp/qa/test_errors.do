...
if `fail_count' > 0 {
    display "RESULT: test_errors tests=`test_count' pass=`pass_count' fail=`fail_count' status=FAIL"
    display as error "FAIL"
    exit 1
}
else {
    display "RESULT: test_errors tests=`test_count' pass=`pass_count' fail=`fail_count' status=PASS"
