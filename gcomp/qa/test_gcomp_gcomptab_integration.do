/*******************************************************************************
* test_gcomp_gcomptab_integration.do
*
* Purpose: Integration test that runs gcomp on synthetic data, then pipes
*          results to gcomptab, and validates Excel output with check_xlsx.py
*
* Tests:
*   1. gcomp mediation (OBE) → gcomptab with normal CIs
*   2. gcomp mediation (OBE) → gcomptab with percentile CIs
*   3. gcomp mediation (OBE+CDE) → gcomptab with control()
*   4. e() results survive through gcomptab call
*   5. estimates store works after gcomp (eclass proof)
*   6. Excel output validated with check_xlsx.py
*
* Prerequisites:
*   - gcomp.ado in gcomp/
*   - gcomptab.ado in gcomp/
*   - python3 with openpyxl
*
* Run: stata-mp -b do ../../_devkit/_testing/test_gcomp_gcomptab_integration.do
*
...
run "${STATA_TOOLS_ROOT}/gcomp/gcomp.ado"
adopath ++ "${STATA_TOOLS_ROOT}/gcomp"
capture program drop gcomptab
run "${STATA_TOOLS_ROOT}/gcomp/gcomptab.ado"
...
* =============================================================================
* TEST 1: gcomp OBE → gcomptab with normal CIs
* =============================================================================
local ++test_count
display as text _n "TEST `test_count': gcomp OBE → gcomptab (normal CI)"
...
capture noisily {
	...
	* Export to Excel
	gcomptab, xlsx("${DATA_DIR}/_test_integration_normal.xlsx") ///
		sheet("Mediation OBE") ///
		title("Table 1. Causal Mediation Analysis (OBE)")

	confirm file "${DATA_DIR}/_test_integration_normal.xlsx"
	display as result "  PASSED: gcomp → gcomptab pipeline works (normal CI)"
	local ++pass_count
}
if _rc {
...
* =============================================================================
* TEST 2: gcomp OBE → gcomptab with percentile CIs
* =============================================================================
local ++test_count
display as text _n "TEST `test_count': gcomp OBE → gcomptab (percentile CI)"
display as text "{hline 50}"

capture noisily {
	* Re-run gcomp with all CI types
	gcomp y m x c, outcome(y) mediation obe ///
		exposure(x) mediator(m) ///
		commands(m: logit, y: logit) ///
		equations(m: x c, y: m x c) ///
	...
	* Check all CI matrices exist
	...
	gcomptab, xlsx("${DATA_DIR}/_test_integration_pctile.xlsx") ///
	...
	display as result "  PASSED: gcomptab with percentile CIs"
	local ++pass_count
}
if _rc {
	display as error "  FAILED: Error code " _rc
...
* =============================================================================
* TEST 3: gcomp OBE + control() → gcomptab with CDE
* =============================================================================
local ++test_count
display as text _n "TEST `test_count': gcomp OBE+CDE → gcomptab"
display as text "{hline 50}"
...
* =============================================================================
* TEST 4: e() results persist after gcomptab
* =============================================================================
local ++test_count
display as text _n "TEST `test_count': e() persists after gcomptab"
display as text "{hline 50}"

capture noisily {
	* gcomptab is rclass — should NOT clear e()
	* e() should still have gcomp results from TEST 3
	assert "`e(cmd)'" == "gcomp"
	assert "`e(analysis_type)'" == "mediation"
	confirm scalar e(tce)
	confirm matrix e(b)

	display as result "  PASSED: e() results persist after gcomptab call"
	local ++pass_count
}
if _rc {
	display as error "  FAILED: Error code " _rc
...
* =============================================================================
* TEST 10: gcomptab error when e() is from wrong command
* =============================================================================
local ++test_count
display as text _n "TEST `test_count': gcomptab rejects non-gcomp e()"
display as text "{hline 50}"
...
capture noisily {
	* Run a regression to replace e()
	...
	capture gcomptab, xlsx("${DATA_DIR}/_test_should_not_exist.xlsx") sheet("Error")
	assert _rc == 119

	display as result "  PASSED: gcomptab rejects non-gcomp e() results"
	local ++pass_count
}
if _rc {
	display as error "  FAILED: Error code " _rc
...
* =============================================================================
display as text _n "{hline 70}"
display as text "GCOMP + GFORMTAB INTEGRATION TEST SUMMARY"
display as text "{hline 70}"
