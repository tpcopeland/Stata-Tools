...
**# V9: Logit on 3-level variable is rejected
local ++test_count
capture noisily {
    _make_med_data
    gen byte m3 = mod(_n, 3)
    capture noisily gcomp y m3 x c, outcome(y) mediation obe ///
...
if _rc == 0 {
    display as result "  PASS: V9 logit on 3-level variable rejected rc=459"
    local ++pass_count
}
else {
...
display as text "=============================================="
if `fail_count' > 0 {
    display "RESULT: test_gcomp_validation tests=`test_count' pass=`pass_count' fail=`fail_count' status=FAIL"
    exit 1
}
display "RESULT: test_gcomp_validation tests=`test_count' pass=`pass_count' fail=`fail_count' status=PASS"
