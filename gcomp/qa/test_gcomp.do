* test_gcomp.do - Phase 1 verification tests for gcomp refactoring
* Tests: load, basic mediation, bug regressions, no globals
* Run: stata-mp -b do ../../_devkit/_testing/test_gcomp.do
...
capture run "gcomp/gcomp.ado"
if _rc != 0 {
	display as error "TEST 1 FAILED: gcomp.ado failed to load (rc=`=_rc')"
	local ++failures
}
else {
	display as text "TEST 1 PASSED: gcomp.ado loads without error"
}
...
if `rc1' != 0 | `rc2' != 0 | `rc3' != 0 | `rc4' != 0 {
	display as error "TEST 2 FAILED: Not all programs defined (gcomp=`rc1' bootstrap=`rc2' detangle=`rc3' formatline=`rc4')"
	local ++failures
}
else {
...
* ============================================================
* TEST 3: _gcomp_detangle works (replaces SSC detangle)
* ============================================================
local ++tests
clear
set obs 10
...
capture _gcomp_detangle "m: logit, y: logit" command "m y"
local det_rc = _rc
if `det_rc' != 0 {
	display as error "TEST 3 FAILED: _gcomp_detangle failed (rc=`det_rc')"
	local ++failures
}
else {
	if "${S_1}" != "logit" | "${S_2}" != "logit" {
		display as error "TEST 3 FAILED: _gcomp_detangle wrong results (S_1=${S_1} S_2=${S_2})"
		local ++failures
	}
	else {
		display as text "TEST 3 PASSED: _gcomp_detangle works correctly"
	}
}

* ============================================================
* TEST 4: _gcomp_formatline works (replaces SSC formatline)
* ============================================================
local ++tests
capture _gcomp_formatline, n("x c m y some_long_variable another") maxlen(20)
local fmt_rc = _rc
if `fmt_rc' != 0 {
	display as error "TEST 4 FAILED: _gcomp_formatline failed (rc=`fmt_rc')"
	local ++failures
}
else {
	if r(lines) < 1 {
		display as error "TEST 4 FAILED: _gcomp_formatline returned 0 lines"
		local ++failures
	}
	else {
		display as text "TEST 4 PASSED: _gcomp_formatline works correctly (`=r(lines)' lines)"
	}
}
...
if `gf_rc' != 0 {
	display as error "TEST 5 FAILED: gcomp mediation failed (rc=`gf_rc')"
	local ++failures
}
else {
	display as text "TEST 5 PASSED: gcomp mediation analysis completed"
}
...
if `gf_rc' == 0 {
	...
	display as error "TEST 6 SKIPPED: gcomp did not run"
	local ++failures
}
...
if `gf_rc' == 0 {
	local _pass = 1
	if "`e(cmd)'" != "gcomp" {
		display as error "TEST 6b FAILED: e(cmd)='`e(cmd)'', expected 'gcomp'"
		local _pass = 0
	}
	...
	if `_pass' {
		display as text "TEST 6b PASSED: e(cmd)=gcomp, e(analysis_type)=mediation"
	}
	else {
		local ++failures
	}
	...
	display as error "TEST 6b SKIPPED: gcomp did not run"
	local ++failures
}
...
if `gf_rc' == 0 {
	...
	display as error "TEST 6c SKIPPED: gcomp did not run"
	local ++failures
}
...
if `gf_rc' == 0 {
	...
	display as error "TEST 6d SKIPPED: gcomp did not run"
	local ++failures
}
...
copy "gcomp/gcomp.ado" `temp1', replace
...
capture noisily gcomp y m x c, outcome(y) mediation oce ///
	exposure(x) mediator(m) ///
	commands(m: logit, y: logit) ///
	equations(m: x c, y: m x c) ///
