...
capture noisily {
    tempfile marker
    shell bash -lc 'cd "$1" && if grep -Fq "codescan Version 2.0.4  2026/06/30" codescan.ado && grep -Fq "codescan_describe Version 2.0.4  2026/06/30" codescan_describe.ado && grep -Fq "{* *! version 2.0.4  30jun2026}" codescan.sthlp && ! grep -Fq "*! version" codescan_describe.sthlp && grep -Fq "**Version 2.0.4** | 2026-06-30" README.md && grep -Fq "d Distribution-Date: 20260630" codescan.pkg; then echo PASS > "$2"; else echo FAIL > "$2"; fi' bash "`pkg_dir'" "`marker'"
