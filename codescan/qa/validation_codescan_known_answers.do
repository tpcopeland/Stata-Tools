...
**# Score, Hierarchy, and Sensitivity
...
capture noisily {
    ...
    codescan dx1, define(dm2 "E11" | chf "I50" | renal "N18" | metastatic "C7[7-9]") ///
...
if _rc == 0 {
    display as result "  PASS: Charlson score hand-computed"
    local ++pass_count
}
else {
    display as error "  FAIL: Charlson score hand-computed (error `=_rc')"
    local ++fail_count
}
...
capture noisily {
    ...
    codescan dx1, define(dm_uncomp "E11" | dm_comp "E102") id(pid) collapse ///
        hierarchy(dm_comp > dm_uncomp) score(custom) replace
}
if _rc == 198 {
    display as result "  PASS: score(custom) without codefile is rejected"
    local ++pass_count
}
else {
    display as error "  FAIL: score(custom) without codefile should be rejected (error `=_rc')"
    local ++fail_count
}
...
capture noisily {
    tempfile weights
    local weights_dta "`weights'.dta"
    preserve
    clear
    input str16 name str16 pattern str16 exclusion str32 label double weight
    "dm_uncomp" "E11"  "" "Diabetes uncomplicated" 1
    "dm_comp"   "E102" "" "Diabetes complicated" 2
    end
    save "`weights_dta'", replace
    restore
    ...
    codescan dx1, codefile("`weights_dta'") id(pid) collapse ///
        hierarchy(dm_comp > dm_uncomp) score(custom)
    sort pid
    ...
    assert dm_comp[2] == 0
    assert _score[2] == 1
    assert dm_uncomp[3] == 0
    assert dm_comp[3] == 1
    assert _score[3] == 2
    assert _score[4] == 0
}
if _rc == 0 {
    display as result "  PASS: custom score with hierarchy hand-computed"
    local ++pass_count
}
else {
    display as error "  FAIL: custom score with hierarchy hand-computed (error `=_rc')"
    local ++fail_count
}
