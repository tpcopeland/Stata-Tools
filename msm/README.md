...
The weighted SMD substitutes weighted means *and* weighted variances, following Austin and Stuart (2015, §4.1.1). The raw and weighted columns are therefore each standardized on their own sample's SD rather than sharing one denominator, so read the percentage change as a direction and judge the weighted column against the threshold on its own terms.
