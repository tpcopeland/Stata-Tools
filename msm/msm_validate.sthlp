...
{pstd}
Run {cmd:msm_validate} after {cmd:msm_prepare} and before {cmd:msm_weight}. It is the quality
gate that stands between your raw data and the weighting step. You can run it
as many times as you like; it does not modify the dataset.
...
{phang2}3. {bf:Outcome is terminal} {hline 2} no rows exist after the outcome event. If a person has
the outcome at period 5, they should not have rows at period 6+. Post-event
rows would contaminate the estimation sample. Default: warning.{p_end}
...
{phang2}5. {bf:Missing data} {hline 2} checks id, period, treatment, outcome, censoring, and all
covariates for missing values. Missing values in the core variables can cause
silent observation loss in the weight models. Default: warning.{p_end}
...
{phang2}8. {bf:Treatment history patterns} {hline 2} classifies individuals as always-treated,
never-treated, or switchers and reports the distribution. Not a pass/fail
check {hline 1} purely descriptive.{p_end}

{phang2}9. {bf:Censoring patterns} {hline 2} checks that censoring is terminal
(no rows after censoring). Default: warning.{p_end}

{phang2}10. {bf:Positivity by period} {hline 2} both treatment values exist in every period. If any
period has 100% treated or 100% untreated, the weight model will fail for that
period (positivity violation). Default: warning. Reports the treatment
prevalence range across periods.{p_end}
