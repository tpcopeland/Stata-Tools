* validation_msm.do — Combined MSM Validation Suite (V1–V11)
...
* MSM COMBINED VALIDATION SUITE (V1-V11)
...
* ============================================================
* V9 (cross-language parity) REMOVED from this suite.
*
* It was a verbatim duplicate of qa/crossval_msm.do (the canonical cross-
* language suite, run in the crossval/full lanes). The duplication caused the
* two copies to diverge: the Q09 oracle fixes -- non-collapsibility smoke checks
* replacing the invalid marginal-vs-conditional test, and row-level weight
* parity replacing weak correlation gates -- were applied to crossval_msm.do
* while this embedded copy kept the old, invalid assertions. Per the standard
* QA layout cross-language parity belongs in its own crossval lane, and this
* Stata-validation lane must run without R/Python (audit Q03). See crossval_msm.do.
* ============================================================
...
local ++test_count
capture {
    * Own setup (Q01: no longer reuses test 10.3's fit) -- a logistic MSM so
    * the bias-factor formula has a fitted model to attach to.
    clear
    set obs 2000
    gen long id = _n
    gen int period = 0
    set seed 10401
    gen byte treatment = runiform() < 0.5
    gen byte outcome = runiform() < invlogit(-3 + ln(2) * treatment)
    msm_prepare, id(id) period(period) treatment(treatment) outcome(outcome)
    gen double _msm_weight = 1
    gen double _msm_tw_weight = 1
    _msm_qa_register_weights
...
* ============================================================
* V12: REGRESSION VALIDATIONS (Codex review 2026-04-01)
* Validates: strict mode promotion, extrapolation guard,
*            stale-weight cleanup
* ============================================================

* --- 12.1: strict mode escalates check 6 (small period N) ---
local ++test_count
capture noisily {
    clear
    set seed 54321
    * Small dataset: 20 individuals, 3 periods => some periods have < 10
    set obs 20
    gen id = _n
    expand 3
    ...
    gen cov1 = rnormal()
    * Drop rows to make period 2 small
    bysort period: drop if period == 2 & _n > 5
...
if _rc == 0 {
    ...
    display as error "  FAIL 12.1: strict check 6 escalation (rc=`=_rc')"
    local ++fail_count
    local failed_tests "`failed_tests' 12.1"
}

* --- 12.2: strict mode escalates check 7 (zero-variation covariate) ---
local ++test_count
capture noisily {
    use "`pkg_dir'/msm_example.dta", clear
    replace biomarker = 0
...
if _rc == 0 {
    ...
    display as error "  FAIL 12.2: strict check 7 escalation (rc=`=_rc')"
    local ++fail_count
    local failed_tests "`failed_tests' 12.2"
}

* --- 12.3: msm_predict rejects extrapolation by default ---
local ++test_count
capture noisily {
    use "`pkg_dir'/msm_example.dta", clear
    msm_prepare, id(id) period(period) treatment(treatment) ///
    ...
    * Within-range should work
    msm_predict, times(5) samples(10)
    assert r(n_times) == 1
...
if _rc == 0 {
    ...
    display as error "  FAIL 12.3: extrapolation guard (rc=`=_rc')"
    local ++fail_count
    local failed_tests "`failed_tests' 12.3"
}

* --- 12.4: msm_predict extrapolate option overrides guard ---
local ++test_count
capture noisily {
    use "`pkg_dir'/msm_example.dta", clear
    msm_prepare, id(id) period(period) treatment(treatment) ///
...
if _rc == 0 {
    display as result "  PASS 12.4: extrapolate option allows beyond-range predictions"
    local ++pass_count
}
else {
    display as error "  FAIL 12.4: extrapolate option (rc=`=_rc')"
    local ++fail_count
    local failed_tests "`failed_tests' 12.4"
}

* --- 12.5: msm_prepare drops stale weight variables ---
local ++test_count
capture noisily {
    use "`pkg_dir'/msm_example.dta", clear
    msm_prepare, id(id) period(period) treatment(treatment) ///
    ...
    * Weight variable should be gone
    capture confirm variable _msm_weight
    assert _rc != 0

    * _msm_weighted characteristic should be cleared
    local wflag : char _dta[_msm_weighted]
    assert "`wflag'" == ""
}
if _rc == 0 {
    display as result "  PASS 12.5: re-prepare drops weight variables and clears flag"
    local ++pass_count
}
else {
    display as error "  FAIL 12.5: stale weight cleanup (rc=`=_rc')"
    local ++fail_count
    local failed_tests "`failed_tests' 12.5"
}

* --- 12.6: msm_report skips weight section after re-prepare ---
local ++test_count
capture noisily {
    use "`pkg_dir'/msm_example.dta", clear
    msm_prepare, id(id) period(period) treatment(treatment) ///
    ...
    * Re-prepare invalidates weights
    msm_prepare, id(id) period(period) treatment(treatment) ///
        outcome(outcome) censor(censored) ///
        covariates(biomarker comorbidity) ///
        baseline_covariates(age sex)
...
if _rc == 0 {
    ...
    display as error "  FAIL 12.6: msm_report after re-prepare (rc=`=_rc')"
    local ++fail_count
