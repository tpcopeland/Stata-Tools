#!/usr/bin/env python3
"""Fail closed when a Stata help-table description exceeds Viewer width."""
...
STYLE_RE = re.compile(
    r"\{(?:cmd|cmdab|it|bf|hi|res|text|err|error|input|ul|sf|sub|sup)"
    r"\s*:\s*([^{}]*)\}"
)
...
def rendered_text(fragment: str) -> str:
    """Return the visible text used to measure a synopt description."""
    previous = None
    while previous != fragment:
        previous = fragment
        fragment = ABBREV_RE.sub(lambda match: match.group(1).replace(":", ""), fragment)
...
def synopt_rows(lines: list[str]) -> list[tuple[int, str, bool]]:
    """Join source continuations into logical synopt rows."""
    ...
    while index < len(lines):
        ...
        while not has_p_end and cursor < len(lines):
            ...
            cursor += 1
        rows.append((start + 1, " ".join(part.strip() for part in parts), has_p_end))
...
def check_file(path: Path) -> list[str]:
    ...
    for line_number, description, has_p_end in synopt_rows(lines):
        if not has_p_end:
            ...
            continue
        cap = VIEWER_LAST_COL - (
            synoptset_at[line_number - 1] + DESCRIPTION_INDENT
        )
...
def main() -> int:
    ...
    issues = [] if paths else ["no .sthlp files found"]
    for path in paths:
        if not path.is_file():
